...
************************************************************************************************************
*rdplot seat_next1 margin if  abs(margin)<0.05, p(1) nbins(10 10) graph_options(xtitle("Win margin (t)") title(Candidate Winning Seat in Election t + 1) plotregion(lcolor(white) ilcolor(white)) graphregion(fcolor(white)) yscale(range(0 1)) ylabel(0(0.1)1) ymtick(0(0.1)1) legend(off)) ci(95)
*rdplot seat_next1 margin if  abs(margin)<0.05, p(1) graph_options(xtitle("Win margin (t)") title(Candidate Winning Seat in Election t + 1) plotregion(lcolor(white) ilcolor(white)) graphregion(fcolor(white)) yscale(range(0 1)) ylabel(0(0.1)1) ymtick(0(0.1)1) legend(off)) ci(95) bandwidth(es)
...
foreach var in seat_next1 seat_next2 seat_next3 seat_next4 {
...
(line fit`var' margin if margin<0, sort lcolor(black) lwidth(medium)) (line fit_`var' margin if margin>0, sort lcolor(black) lwidth(medium)) (scatter mean`var' midbin, msize(medium) mlwidth(thin) mcolor(black) msymbol(circle_hollow)) , ///
 ...
 cd figures/gph/
 graph combine _seat_next1.gph _seat_next2.gph _seat_next3.gph _seat_next4.gph, plotregion(lcolor(white) ilcolor(white)) graphregion(fcolor(white)) scheme(s1mono) 
 graph export ../FigureA9.eps, replace
 cd ../../
