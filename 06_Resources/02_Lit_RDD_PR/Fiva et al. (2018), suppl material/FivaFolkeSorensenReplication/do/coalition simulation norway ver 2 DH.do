	...
	gen vs_sum=0
		...
		while felmax>0 {
			...
			foreach parti in `namelist'{
				...
				gen j`parti'=`parti'_vs_sim*(1-p`parti')/2 +`parti'_vs_sim*p`parti'/(2+2*`parti'_s_sim)
				gen j2`parti'=`parti'_vs_sim/p`parti'*(p1`parti')/2 +`parti'_vs_sim/p`parti'*(1-p1`parti')/(2+2*(`parti'_s_sim-1))
					}
...
do "data prep sim coal norway DH"
coalprepDH "RV SV DNA V SP KRF H FRP PP MDG Indep1 Indep2 Indep3 Indep4 Indep5 Indep6 Other1 Other2 Other3 Other4 Other5 JointL JointR1 JointR2"  "RV SV DNA MDG JointL" 

clear
set obs 1
gen x=1
save "../dta/val med simuleringar DH.dta", replace
use "../dta/hel kommun grunddata coal DH.dta" 
...
use "../dta/hel kommun grunddata coal DH.dta" , clear 
keep if _n==`i'
...
save tempdh, replace
use  "../dta/val med simuleringar DH.dta"
append using tempdh
save "../dta/val med simuleringar DH.dta", replace
use "../dta/hel kommun grunddata coal DH.dta" 
}
