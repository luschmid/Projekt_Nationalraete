...
save example_partymarg1.dta, replace
...
use example_all.dta, clear
	...
	di "`h'"
		keep if cand!=`h'
		drop cand
		egen cand_other=rank(-votes),  unique // generate rank of all other quotients except party j 
...
/*joinby party using example_partymarg1.dta
rename can cand_h
rename votes votes_h
save example_partymarg2.dta, replace

use example.dta, clear
rename cand cand_g
rename votes votes_g
joinby party using example_partymarg2.dta
keep if cand_h == number
g votesdiff = votes_g - votes_h
egen votemargin = rowmin(partymarg votesdiff)

replace partymarg = . if cand_g!=number
collapse (min) partymarg (max) votemargin, by(cand_g party votes_g alliance)
rename cand_g cand
rename votes_g votes
sort party cand


reshape wide partymarg, i(party) j(number)

save example_partymarg_wide.dta, replace

use example.dta, clear

merge m:1 party alliance using example_partymarg_wide.dta, gen(merge1)

reshape wide votes, i(party) j(cand)

save example_partymarg_widewide.dta, replace

use example.dta, clear

merge m:1 party alliance using example_partymarg_widewide.dta, gen(merge2)

forvalues i = 1(1)`n'{
gen votesdiff`i'=votes-votes`i'
}

egen margin=rowmin(pm_1 pm_2)


//erase example_pm* help_others example_partymarg_wide* 
