...
data_ch <- readstata13::read.dta13("./02_Processed_data/nationalraete_1931_2015.dta") %>%
  rename(votes_j=pvotes,votes_h=votes,party=list) %>%
  select(ID,name,firstname,year, canton,party,alliance,suballiance,votes_h,votes_j,elected) %>%
  mutate(alliance=ifelse(alliance %in% c("","."),party,alliance))%>%
  mutate(suballiance=ifelse(suballiance %in% c("","."),party,suballiance))


# (ii) Introduce "extra candidate" to account for empty party votes

data_ch_votes_j <- data_ch %>% group_by(year,canton,party) %>%
  dplyr::summarize(votes_party=sum(votes_h,na.rm=T),
                   votes_j=first(votes_j),
                   alliance=first(alliance),
                   suballiance=first(suballiance),
                   votes_j=first(votes_j),
                   votes_h=votes_j-votes_party) %>%
  select(year,canton,party,votes_h,votes_j,alliance,suballiance) %>%
  filter(votes_h>=0) %>%
  mutate(ID="Additional candidate",elected=0,firstname="Additional candidate",name="Additional candidate")
  

data_ch <- rbind(data_ch,data_ch_votes_j)


# (iii) Build numeric party, alliance, and suballiance variables as well as number of seats


data_ch_party_num <- data_ch %>% distinct(year,canton,party) %>% mutate(party_num=row_number())
data_ch_alliance_num <- data_ch %>% distinct(year,canton,alliance) %>% mutate(alliance_num=row_number())
data_ch_suballiance_num <- data_ch %>% distinct(year,canton,suballiance) %>% mutate(suballiance_num=row_number())

data_ch_seats <- data_ch %>% group_by(year,canton) %>%
  dplyr::summarize(seats=sum(elected,na.rm=T),
                   votes_canton=sum(votes_h,na.rm=T))

data_ch <- data_ch %>% left_join(data_ch_party_num,by=c("canton","party","year")) %>%
                       left_join(data_ch_alliance_num,by=c("canton","alliance","year")) %>%
                       left_join(data_ch_suballiance_num,by=c("canton","suballiance","year")) %>%
                       left_join(data_ch_seats,by=c("canton","year"))%>%
                       mutate(votes_h_share=votes_h/votes_canton) %>%
                       arrange(canton,year,party_num)



# Check: do votes_h_share sum up to 1?
check <- data_ch %>% group_by(canton,year) %>% summarize(test=sum(votes_h_share))
summary(check$test)
# Result: yes!
...
GetPartyVotes <- function(data){
  ...
  data_votes_j <- data %>% group_by(year,canton,party_num) %>%
    select(-votes_j) %>%
    dplyr::summarize(votes_j=sum(votes_h,na.rm=T),party=first(party),alliance_num=first(alliance_num),
                     suballiance_num=first(suballiance_num),seats=first(seats))  %>% ungroup()
  return(data_votes_j)
  #data <- data %>% select(-votes_j) %>% left_join(data_votes_j,by=c("canton","party_num","year")) 
...
# Input: data with party name and party votes
  ...
  colnames(data) <- c("party",colnames(data)[2:4])
...
GetElected_DHondt <- function(m=10000,data,control_seat_allocation=0){
  ...
  suballiance_link <- partyvotes %>% 
                         group_by(suballiance_num) %>%
                         summarize(alliance_num=first(alliance_num))
  
  # (b) Distribute seats to alliances
  VotesAlliance <- CollapseVotes(partyvotes, aggregation_level = alliance_num) %>% as_tibble %>%
                   mutate(seats=partyvotes$seats[1],toplevel=1)
  AllianceSeats <- GetNumberSeats(VotesAlliance) %>% rename(alliance_num=party)
  
  # (c) Distribute seats of alliances to suballiances
  VotesSuballiance <- CollapseVotes(partyvotes, aggregation_level = suballiance_num) %>%
                      as_tibble() %>%
                      left_join(suballiance_link,by=c("suballiance_num")) %>%
                      left_join(AllianceSeats,by=c("alliance_num")) %>%
                      mutate(seats=ifelse(is.na(seats),0,seats)) %>%
                      rename(toplevel=alliance_num)
  
  SuballianceSeats <- GetNumberSeats(VotesSuballiance) %>% rename(suballiance_num=party)
  
  # (d) Distribute seats of suballiances to parties
  VotesParty <- partyvotes %>% select(-seats)%>%
    as_tibble() %>%
    left_join(SuballianceSeats,by=c("suballiance_num")) %>%
    mutate(seats=ifelse(is.na(seats),0,seats)) %>%
    select(party_num,votes_j,seats,suballiance_num)%>%
    rename(toplevel=suballiance_num)
  
  PartySeats <- GetNumberSeats(VotesParty) %>% rename(party_num=party)
  
  # (e) Get individual election result
  data_final <- data %>% select(-seats) %>% filter(ID!="Additional candidate") %>% 
    left_join(PartySeats,by=c("party_num"))%>%
    mutate(seats=ifelse(is.na(seats),0,seats)) %>%
    group_by(year,canton,party_num)%>%
    mutate(votes_h_rank=rank(-votes_h,ties.method = "random" )) %>%
    ungroup() %>%
    mutate(elected=ifelse(votes_h_rank<=seats,1,0)) %>%
    select(ID,party_num,year,elected) %>%
...
CheckConvergence <- function(data1,data2){
  ...
  data_check <- data1 %>% left_join(data2,by=c("ID","year","party_num")) %>%
    select(ID,starts_with("votes_h"),starts_with("elected_sum"),party_num,elected,firstname,name,birthyear)
  ...
  for (k in 1:length(party_num_all)){
    ...
    data_check_party$votes_h_rank <- data.table::frank(data_check_party,-votes_h,-elected,firstname,name,birthyear)  
    ...
    if (check_convergence==1){
      ...
      while (convergence_indi!=1){
        ...
        df_out <- GetElectedSum(m,data,M) %>% dplyr::rename(elected_sum_new=elected_sum,
                                                            M_new=M)
        df_out_all <-df_out_all %>% left_join(df_out,by=c("ID","year", "party_num")) %>%
          mutate(M=M+M_new,
...
KotakorpiEtAl2007Loop <- function(canton_input,data,control_seat_allocation=0,check_convergence=1) {
  ...
  print(paste0("Canton: ",canton_input))
  data_cant <- data %>% filter(canton==canton_input)
  years <- as.character(levels(as.factor(data_cant$year)))
  for (yr in years){
    data_cant_year <- data_cant %>% filter(year==yr)
    print(paste0("Year: ",yr))
    temp <- KotakorpiEtAl2007(data=data_cant_year,control_seat_allocation=control_seat_allocation,check_convergence=check_convergence)
    if (yr==years[1]){
      fwrite(temp, file=paste0("./02_Processed_data/13_Running_variable/kotakorpietal2017_ch",canton_input,".csv"),  append=F)
    }
    else{
      fwrite(temp, file=paste0("./02_Processed_data/13_Running_variable/kotakorpietal2017_ch",canton_input,".csv"),  append=T)
    }
  }
}
...
CheckProcedure <- function(data){
  ...
  lapply(cans,KotakorpiEtAl2007Loop,data,control_seat_allocation=1,check_convergence=0)
  ...
  df <-  do.call(rbind, lapply(paste0("./02_Processed_data/13_Running_variable/",files), function(x) read.csv(x, stringsAsFactors = FALSE))) %>%
    select(ID,year,elected_sum)
  lapply(paste0("./02_Processed_data/13_Running_variable/",files), function(x) file.remove(x))
  
  return(data %>% left_join(df,by=c("ID","year")) %>% select(ID,year,firstname,name,,elected,elected_sum))
}
...
# test functions
# # 
# data_test <- data_ch %>% filter(year==1995 & canton=="ZH")
# data_test_out <- GetElected(m=10000,data_test,control_seat_allocation=1)
# 
# data_test_check <- data_test %>% left_join(data_test_out,by=c("ID")) %>% 
#                    mutate(elected_diff=elected.x-elected.y)
# 
# summary(data_test_check$elected_diff)  
# 
# data_test_check %>% filter(elected_diff!=0)
#   
...
#data_ch_canton <- data_ch %>% group_by(canton,year) %>% summarize(votes_h_min=max(votes_h))
#df_out <- CheckProcedure(data_ch) %>% mutate(diff=elected-elected_sum)
# summary(df_out$diff)
