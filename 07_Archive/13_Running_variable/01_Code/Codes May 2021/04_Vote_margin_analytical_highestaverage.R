...
PartyMargin <- function(party, data_input, no_seats,method="dHondt",
                        margin_type="add") {
# margin_type: "add" for only adding votes (as in Lüchinger, Schelker, 
...
GetRVSimulation_Candidate_SubAdd <- function(data_input, 
                                                     n, 
                                                     convcrit,
                                                     method="dHondt",
                                                     system="open") {
      ...
      for (minusj in parties_all[parties_all!=j]){
        ...
        for (i in 1:n) {
            while (seats<i) {
              votessim <- votessim*2
              data_input_party$votes_j[data_input_party$party==minusj] <- 
              data_input_party$votes_j[data_input_party$party==minusj]-votessim
              if (method=="SainteLague" | method=="dHondt"){
                seats <- GetSeatsHighestAverage(party_input=j,
                                                data_input=data_input_party, 
                                                no_seats=n,
...
GetRVSimulation_Candidate_Loop <- function(data_input, 
  ...
  #             "Votes_Subracted_Or_Added" for easiest ways as in 
  #             Fiva & Smith (2018)
  #---------------------------------------------------------------------------  
  df_out <- data.frame()
  years_all <- levels(as.factor(as.character(data_input$year)))   
  ...
  for (yr in years_all) {
    ...
    for (district in districts_all) {
      ...
      if (type== "Votes_Subracted_Or_Added"){
        df_out <- rbind(df_out, GetRVSimulation_Candidate(data_input=df2, 
                                                                 n=sum(df2$elected), 
                                                                 convcrit=convcrit,
                                                                 method=method,
                                                                 system=system))
        df_out <- rbind(df_out, GetRVSimulation_Candidate_SubAdd(data_input=df2, 
                                                                         n=sum(df2$elected), 
                                                                         convcrit=convcrit,
                                                                         method=method,
                                                                         system=system))
