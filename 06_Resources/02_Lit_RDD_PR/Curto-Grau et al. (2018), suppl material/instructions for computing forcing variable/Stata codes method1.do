...
** Code to create the forcing variable based on Hypothesis 1 (all the votes lost by the regional incumbent's bloc go to abstention)
*******************************************************************
...
use "file with electoral results and seats", clear	
...
replace cl_1=vloc0/(eloc+1) if (right==0 & id==1)
replace cr_1=vloc0/(eloc+1) if (left==0 & id==1)
sort mun_code
drop clmax crmax
by mun_code: egen clmax=max(cl_1)
by mun_code: egen crmax=max(cr_1)
...
replace crmax=0 if crmax==.
*********************************
** compute the distance if there's a left-wing majority in the city council (note that deltai=votes that each left-w party loses, while delta=total votes lost by the left)
*********************************
gen x1=(cl-crmax)*sloc if majleft0==1
gen delta_00=(x1/alpha_l)+1  if majleft0==1
by mun_code: egen delta0=min(delta_00) if majleft0==1
gen delta1=-delta0 if majleft0==1
gen delta1i=delta1*alpha_l if majleft0==1
*********************************
** compute the distance if there's a right-wing majority in the city council(note that betai=votes that each right-w party loses, while beta=total votes lost by the right)
*********************************
gen y1=(cr-clmax)*sloc if majright0==1
gen beta_00=(y1/alpha_r)+1   if majright0==1
by mun_code: egen beta0=min(beta_00)  if majright0==1
gen beta1=-beta0 if majright0==1
gen beta1i=beta1*alpha_r if majright0==1
** round beta1i and delta1i 
replace delta1i=floor(delta1i)
replace beta1i=floor(beta1i)
** check that x1 and y1 are positive and delta1 and beta1 are negative
sum x1 y1 delta1 beta1
*********************************
** new distribution of votes
*********************************
gen vloc1=vloc0
replace vloc1=vloc0+delta1i if delta1i!=. & majleft0==1
replace vloc1=vloc0+beta1i if  beta1i!=. & majright0==1
...
sort mun_code
by mun_code: egen vtot1=sum(vloc1)
gen sval1=vloc1/(vtot1+vblank)
save "file with electoral results and seats1", replace
*********************************
** new distribution of seats
*********************************
** once delta and beta are added, with the new allocation of votes generate the new allocation of seats
qui v2seats vloc if sval>0.05, party(pol_party) formula(dhondt) district(mun_code) size(stotal)  ///
	save("new file seats.dta") preserve(mun_code pol_party)
** merge previous database ("file with electoral results and seats1_method2") with the database with the new allocation of seats ("new file seats.dta"), then:
use "file with electoral results and seats1", clear	
*********************************
** Checks
*********************************
...
*********************************
** Computing the FINAL DISTANCE (forcing variable, labelled "distance")
*********************************
...
gen distance=-delta if majleft0==1 & ul_right==1
...
replace distance=delta if majleft0==1 &  ul_right==0
...
replace distance=-beta if majleft0==0 &  ul_right==0
...
replace distance=beta if majleft0==0 & ul_right==1
