...
foreach parti in `namelist'{
	gen `parti'_distp=mindiff`parti'p
	gen `parti'_distn=mindiff`parti'n
	gen `parti'_rank_distp=.
	gen `parti'_rank_distn=.
}
gen dist_p=1000
gen dist_n=1000
...
forvalues n=1(1)8{
	foreach parti in `namelist'{
		replace dist_p = `parti'_distp if `parti'_distp<dist_p
		replace dist_n = `parti'_distn if `parti'_distn<dist_n
	}
	foreach parti in `namelist'{
		replace `parti'_rank_distp=`n' if `parti'_distp==dist_p & `parti'_distp!=.
		replace `parti'_distp=. if `parti'_distp==dist_p & `parti'_distp!=.
		replace `parti'_rank_distn=`n' if `parti'_distn==dist_n & `parti'_distn!=.
		replace `parti'_distn=. if `parti'_distn==dist_n & `parti'_distn!=.
	}
}
...
gen sum_n`lim'=0 
   foreach parti in `namelist'{
			...
			replace sum_p`lim'= sum_p`lim' +`parti'diffpcalvs`lim'
			gen `parti'diffncalvs`lim'=  (mindiff`parti'n)<0.0`lim'
			replace sum_n`lim'= sum_n`lim' +`parti'diffncalvs`lim'
		...
		egen sum_min`lim'= rowmin(sum_p`lim' sum_n`lim')
} 
...
foreach lim in `namelist2' {   
   foreach parti in `namelist'{
			...
			replace `parti'diffcalvs`lim' =0 if `parti'_rank_distn > sum_min`lim' & `parti'_rank_distp > sum_min`lim'
