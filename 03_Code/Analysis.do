...
log using "$path\04_Results\04_Political_Rents\results_$S_DATE.smcl", replace
...
sum year if year==1947 | year == 1951 // Table 2 Note
...
foreach outcome of varlist ///
	...
	* Control mean: sum `outcome'_T0  if elected == 0
	...
	local elected_effect_`outcome'=_b[elected]
	local level_elected_`outcome'=_b[elected]+_b[_cons]
	local level_notelected_`outcome'=_b[_cons]
		...
		* Population 
		...
		gen weight = 4/count
			...
			* Control mean: sum `outcome' [iweight=weight] if ellegperiod == 0
			...
			local pop_effect_`outcome'=_b[ellegperiod]
			local level_pop_`outcome'=_b[_cons]
		restore
			...
			coefplot ///
			(matrix(coef_est_ob), ci(CI95_est_ob) mcolor(black) ciopts(lc(black) lw(medthin)) offset(-0.1)) ///
			(matrix(coef_est_ob), ci(CI90_est_ob) mcolor(black) ciopts(lc(black) lw(medthick)) offset(-0.1)) ///		
			(matrix(coef_est_ob_cov), ci(CI95_est_ob_cov) mcolor(black) ciopts(lc(black) lw(medthin)) offset(0)) ///
			(matrix(coef_est_ob_cov), ci(CI90_est_ob_cov) mcolor(black) ciopts(lc(black) lw(medthick)) offset(0)) ///
			(matrix(coef_est_fb), ci(CI95_est_fb) mcolor(ebblue) ciopts(lc(ebblue) lw(medthin)) offset(0.1)) ///
			(matrix(coef_est_fb), ci(CI90_est_fb) mcolor(ebblue) ciopts(lc(ebblue) lw(medthick)) offset(0.1)) ///		
			(matrix(coef_est_fb_cov), ci(CI95_est_fb_cov) mcolor(ebblue) ciopts(lc(ebblue) lw(medthin)) offset(0.2)) ///
			(matrix(coef_est_fb_cov), ci(CI90_est_fb_cov) mcolor(ebblue) ciopts(lc(ebblue) lw(medthick)) offset(0.2)) ///
			(matrix(coef_cand), ci(CI95_cand) mcolor(lavender) ciopts(lc(lavender) lw(medthin)) offset(-0.2)) ///
			(matrix(coef_cand), ci(CI90_cand) mcolor(lavender) ciopts(lc(lavender) lw(medthick)) offset(-0.2)) ///
			(matrix(coef_pop), ci(CI95_pop) mcolor(purple) ciopts(lc(purple) lw(medthin)) offset(-0.3)) ///
			(matrix(coef_pop), ci(CI90_pop) mcolor(purple) ciopts(lc(purple) lw(medthick)) offset(-0.3)), ///
...
di "Probability of holding directorship positions for politicians: " `level_elected_i_all_c1'
di "Number of directorship positions for politicians: " `level_elected_n_all_sum_c1'
di "Probability of holding directorship positions for population: " `level_pop_i_all_c1' 
di "Number of directorship positions for population: " `level_pop_n_all_sum_c1'
di "Probability of holding directorship positions for not elected candidates: " `level_notelected_i_all_c1' 
di "Number of directorship positions for not elected candidates: " `level_notelected_n_all_sum_c1'

di "Higher probability of holding directorship positions politicians vs. population: " `pop_effect_i_all_c1'
di "Higher probability of holding directorship positions politicians vs. population: " `pop_effect_n_all_sum_c1'
di "Higher probability of holding directorship positions elected vs. non-elected: " `elected_effect_i_all_c1'
di "Higher probability of holding directorship positions elected vs. non-elected: " `elected_effect_n_all_sum_c1'
...
foreach outcome in ///
	...
	local i = 1
		...
		foreach var of varlist `outcome'_TL1  `outcome'_T0 `outcome'_TF1 { 
			...
			local e_first = (`i'-1)*4+1
			local e_second = (`i'-1)*4+2
			local e_third = (`i'-1)*4+3
			local e_fourth = (`i'-1)*4+4
		...
		* Export tables
		...
		esttab e1 e2 e3 e4 e5 e6 e7 e8 e9 e10 e11 e12  ///
...
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_center.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_early.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_late.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_left.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_men.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_right.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_trim.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_women.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_uni.tex"
capture erase "$path_ol/tables/app_c1_poly_2_kern_tri.tex"
capture erase "$path_ol/tables/app_c1_poly_1_kern_tri_first.tex"
capture erase "$path_ol/tables/app_c2_poly_1_kern_tri.tex"
...
* (b) Robust for number of directorships
rdrobust `var' votemargin_rel, p(1) kernel(tri) ///
...
* (c) Conventional for fixed bandwidth
rdrobust `var' votemargin_rel, p(1) kernel(tri) ///
	vce(cluster ID_num) all h(0.005) rho(1)
...
mat rrates[1,2] =`n_ob_5'/`n_ob_sum'*100
mat rrates[2,2] =`n_ob_10'/`n_ob_sum'*100
mat rrates[1,3] =`fb_5'/`fb_sum'*100
mat rrates[2,3] =`fb_10'/`fb_sum'*100

mat rownames rrates = "5\%" "10\%"
mat colnames rrates = "\hspace{0.1cm}" "\hspace{0.2cm}" "\hspace{0.3cm}" 
esttab matrix(rrates) using "$path_ol/tables/balance_table_rr.tex",  ///
	nomti nonum append fragment
	
*************************************
* (F) Graph for first-time candidates
*************************************


local bw_half 0.005
rdplot i_all_c1_T0 votemargin_rel  ///
	if -`bw_half' <= votemargin_rel & votemargin_rel <= `bw_half' & ///
	first_election==1, ///
	p(1) h(`bw_half') kernel(triangular) binselect(qs) ///
	graph_options(ylabel(, angle(horizontal) gsty(dot)) ///
	graphregion(fcolor(white) lcolor(white)) legend(off)) 
	
graph export "$path_ol\figures\rdd_graph_i_all_c1_T0_first_time.pdf", as(pdf) replace

local bw_half 0.005
rdplot n_all_sum_c1 votemargin_rel  ///
	if -`bw_half' <= votemargin_rel & votemargin_rel <= `bw_half' & ///
	first_election==1, ///
	p(1) h(`bw_half') kernel(triangular) binselect(qs) ///
	graph_options(ylabel(, angle(horizontal) gsty(dot)) ///
...
graph export "$path_ol\figures\rdd_graph_n_all_sum_c1_first_time.pdf", as(pdf) replace

	
************************************
* (G) Information about early years
***********************************
