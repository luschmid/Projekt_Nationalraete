...
local vars1 candidateid_ lastname_ firstname_ listname_ score_  town_ gender_ job_ birthyear_ LeftRight_ canton_
local vars2 candidateid_*  lastname* firstname* listname*   score*  town*  gender* job* birthyear* LeftRight* canton*
// variables to keep (equal var1 and var2 but with candidateid)
local vars3  candidateid* score* lastname* firstname* listname*  town* gender* job* birthyear* LeftRight* canton*
...
//local threshold 0.6
	...
	* (iii) Merge all fuzzy merge and bring it to long format
	...
	order score_ candidateid candidateid_ yearNew firstname_ lastname_ birthyear   yearNew  
	...
	*/
	...
	local keepErfasser candidateid_threshold_min_sorted indicator candidateid_threshold_opt candidateid_threshold_min candidateid_threshold_max candidateid_ Vorname Nachname id_Stata id_recoded unsicher Kommentar Geschlecht Geburtsjahr Listenbezeichnung Gemeinde Wahljahr Beruf
	order `keepErfasser'
	keep `keepErfasser'
