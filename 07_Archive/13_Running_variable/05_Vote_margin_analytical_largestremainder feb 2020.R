...
GetVotesReq <- function(votes, n, j, minusj) {
  ...
  for (deltai in (-n:(n-1))) {
    #print(paste("deltai:",deltai,sep=" "))
    votes_req_j <- mpfr((votes[minusj] * n + deltai * votes_others) / (n - deltai),128)
    votes_mod <- votes
    votes_mod[j] <- as.numeric(votes_req_j)
    i <- as.numeric(GetIntegerHare(c(votes_mod),n)[j])+1
  ...
  out_df <- matrix(c(votes_req_all,i_all,rep(j,(2*n)),rep(minusj,(2*n)),c(-(n-1):(n))),ncol=5,nrow=(2*n))
...
GetVotesReqAll <- function(votes, n) {
  ...
  for (u in 1:dim(VR)[1]) {
    ...
    VR[u,6] <- n - sum(GetIntegerHare(votes_mod, n)) # free seats after round 1
    VR[u,7] <- rank(-GetRemainderHare(votes_mod, n)[-j])[minusj_revelant] # rank -j
    VR[u,8] <- GetRatioHare(votes_mod, n)[j]-floor(GetRatioHare(votes_mod, n)[j]) # remainderj
    VR[u,9] <- GetRatioHare(votes_mod, n)[minusj]-floor(GetRatioHare(votes_mod, n)[minusj]) # remainderminusj
  }
