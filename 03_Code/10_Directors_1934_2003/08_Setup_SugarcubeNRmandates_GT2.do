...
keep if _merge == 3
gen source = "GT2RL"
rename cname_orig Firm
replace Firm = "none" if firmname == "none"
rename firmlocation Firmlocation
keep NRid year Firm Firmlocation source
sort NRid year Firm
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge3.dta", replace
restore
...
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge2.dta", replace
restore

// prepare for manual coding: RL subset
keep if _merge == 1
gen source = "RL"
keep NRid canton year name firstname gdename cname_orig firmname firmlocation firstname_bus name_bus gdename_bus source 
rename name name_nr
...
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge1.dta", replace

// combine non-matched observations for manual evaluation & linkage
append using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge2.dta"

replace firm_orig = "none" if firmname == "none"

erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge1.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge2.dta"
...
Procedure: 
i) extract links that only exist in RL data (false links)
ii) extract manually coded links between RL and GT2 
iii) extract non-linked GT2 observations and clean non-AG's -- > missing links in RL (GT only)
*/ 

// cut imported dataset into RL and GT2 part and merge on NRid year linkIDs

preserve
keep if source == "GT2"
for var *: rename X X_GT2
rename NRid_GT2 NRid
rename year_GT2 year
rename linkIDs_GT2 linkIDs
drop NROddBlocks_GT2
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_links_tmp.dta", replace
restore

keep if source == "RL"
drop NROddBlocks

* i) extract links that only exist in RL data 
preserve 
keep if linkIDs == .
rename firm_orig Firm
rename firmlocation Firmlocation
keep NRid year Firm Firmlocation source firstname name gdename
rename name Name 
rename firstname Firstname 
rename gdename Gdename 
rename year Year
rename source Source
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_only.dta", replace
restore

* ii) extract manually coded links between RL and GT2 
keep if linkIDs != .

merge 1:1 NRid year linkIDs using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_links_tmp.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_links_tmp.dta"

preserve
keep if _merge == 3
rename firm_orig_GT2 Firm
rename firmlocation Firmlocation
replace source = "manualGT2RL"
keep NRid year Firm Firmlocation source
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_manual-matches.dta", replace
restore

* iii) extract non-linked GT2 observations for cleaning of non-AG's (GT only)
keep if _merge == 2
keep  year NRid linkIDs source_GT2 out_GT2 firm_orig_GT2 firmlocation_GT2 firstname_GT2 name_GT2 gdename_GT2
rename source_GT2 source
rename out_GT2 out
order source year NRid out linkIDs firm_orig_GT2 firmlocation_GT2 firstname_GT2 name_GT2 gdename_GT2
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_only_tmp.dta", replace

* ---> manually eliminate non-AGs
preserve
// collapse on unique strings to code manually (non-AGs); preserve existing indicators
gen maybe = 1 if out == "?"
replace out = "" if out == "?"
destring out, replace
gen n = 1
gen minyear = year
gen maxyear = year
collapse (sum) n out maybe (min) minyear (max) maxyear, by(firm_orig_GT2)
tostring maxyear, replace
tostring minyear, replace
gen y_minmax = minyear + "-" + maxyear
order n maybe out firm_orig_GT2 y_minmax
drop minyear maxyear
replace out = 1 if out > 0
replace out = . if out == 0
replace maybe = 1 if maybe > 0
replace maybe = . if maybe == 0
sort firm_orig_GT2
/*
export excel using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_nonAG_coding.xlsx", first(var) replace
// --- > inactive as manual coding has been completed
*/ 
restore

/*
// compile a list of all unique firm names in original Sugarcube (exclusively AGs)
preserve
use "$path\02_Processed_data\10_Directors_1934_2003\comp_1934-2003_merge.dta", clear
keep year firmname
replace firmname = usubinstr(firmname, "ß", "ss",.)
replace firmname = usubinstr(firmname, "$", "s",.)
replace firmname = usubinstr(firmname, "£", " ",.)	
replace firmname = usubinstr(firmname, "^", " ",.)	
replace firmname = usubinstr(firmname, "'", " ",.)
replace firmname = usubinstr(firmname, "`", " ",.)
...
gen minyear = year
gen maxyear = year
collapse (sum) n (min) minyear (max) maxyear, by(firmname)
tostring maxyear, replace
tostring minyear, replace
gen y_minmax = minyear + "-" + maxyear
order n firmname y_minmax
drop minyear maxyear
sort firmname
export excel using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\Sugarcube_Firmnames_unique.xlsx", first(var) replace
restore
*/

// read-in coded non-AGs from GT2_only
import excel "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_nonAG_coding_in.xlsx", firstrow clear
replace out = 0 if AG_in_Sugarcube == 1
drop if out == 1
drop if maybe == 1
drop out AG_in_Sugarcube maybe y_minmax n

merge 1:m firm_orig_GT2 using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_only_tmp.dta"
drop out // variable "out" comes from "GT2_only.dta" and corresponds to coding before systematic manual coding
keep if _merge == 3
drop _merge
...
// GT2: AG's only, including NR-IDs
append using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge3.dta"
append using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_manual-matches.dta"
sort NRid year Firm
order source NRid year Firm Firmlocation 
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_tmp.dta", replace

// use all NR-ID and names from GT2
use "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\NR_ID_GT2_tmp.dta", clear
keep NRid year canton name firstname gdename gdenr_2018 Companyname_mand Companylocation_mand
sort NRid year
duplicates drop NRid year, force  // the dataset contains multiple entries for NRs with mandates
...
keep if _merge == 1
/*
export excel using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_nonAG_check2.xlsx", first(var) replace
* -- > no AGs _merge == 1
* -- > manual checks look good: no corrections necessary
* -- > inactive as manual coding has been completed
*/
restore 

drop Companyname_mand Companylocation_mand canton_mand gdenr_2018_mand
replace source = "GT2" if source == ""
replace source = "GT2RL" if source == "manualGT2RL"
replace Firm = "none" if Firm == ""
replace Firmlocation = "none" if Firm == "none"
rename name_mand Name 
rename firstname_mand Firstname 
rename gdename_mand Gdename 
rename year Year
...
// check if there are no entries with "none" and firm-entries
bysort NRid Year: gen n = _n
gen flag = 1 if Firm == "none" & n > 1
tab flag

// check Hans Werner Widrig: one mandate in 1998 & 1999
drop if NRid == "SG-1987-0089" & Year == 1998 & Firm == "none"
drop if NRid == "SG-1987-0089" & Year == 1999 & Firm == "none"
drop flag
drop n
...
sort NRid Year 
compress
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2.dta", replace

** Create matched GT2 & RL dataset: add RL_only observations
append using "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_only.dta"
...
gen FP = 0
gen FN = 0
replace TP = 1 if Source == "GT2RL"
replace FP = 1 if Source == "RL"
replace FN = 1 if Source == "GT2"

order NRid Year Name Firstname Gdename Firm Firmlocation Source TP FP FN

sort NRid Year Name Firstname Gdename Firm Firmlocation
compress
save "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2.dta", replace
...
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\GT2_only_tmp.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_only.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_manual-matches.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\11_Elected_Mandates_GT2\RL_GT2_merge3.dta"
