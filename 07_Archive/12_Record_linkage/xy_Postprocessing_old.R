...
bisnode_to_merge <- data_input %>%   
  ...
  mutate(sex_bis=ifelse(anrede=="m?nnlich","Male","Female"),
  ...
  select(id_1,ends_with("_bis"),rechtsform,gremium) 
...
# (vii) Read in manual checks by Oester and Marilley and consolidated by 
...
#
# Both connections seems to be correct connections
