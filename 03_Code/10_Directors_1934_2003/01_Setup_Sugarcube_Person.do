...
local date $S_DATE
log using "$path\02_Processed_data\10_Directors_1934_2003\Sugar_log_Pers_`date'", replace
...
*** Raw data: Read-in and corrections

** Import csv files (saved as UTF-8 with BOM), after corrections @Unilu for years before 1985
...
rmdir "$temp\SugarPersonsCheck_tmp"
restore
...
* Merge with original CID's, check functions
...
replace functions = functions_o if d_compID_funct != 0 & compid_diff == 0 // replace to original "functions" if CID were untouched by UniLU corrections

drop nbrcompIDs nbrfunctions d_compID_funct dupl_c compmerge firstname_merge lastname_merge companiesid_o functions_o nbrcompIDs_o nbrfunctions_o d_compID_funct_o dupl_o _merge compid_diff
...
foreach str in /*titlejob firstname lastname address address2 functions */ alternateaddress city {
...
replace city = "thonex" if city == "thssnex"
replace city = "bogis bossey" if city == "bogis ssossey"
replace city = "baulmes" if city == "ssaulmes"
replace city = "bole" if city == "bssle"
replace city = "bueren an der aare" if city == "buessen an der aare"
replace city = "brienz (be)" if city == "brienzsse" 
replace city = "untersiggenthal" if city == "untersiggssi." 
replace city = "sirnach" if city == "ssirnach" 
replace city = "biglen" if city == "ssiglen" 
replace city = "schaenis" if city == "schaenisss" 
replace city = "schaenis" if city == "schanis" 
replace city = "st. gallen" if city == "148251len"
replace city = "guemligen" if city == "guemiigg" 
replace city = "duedingen" if city == "duedinsen (guin)"
replace city = "kappel so" if city == "kappel ss0"
replace city = "unteraegeri" if city == "untsfs ed"
replace city = "oberaegeri" if city == "obersgeri" 
replace city = "bad ragaz" if city == "sad ragaz" 
replace city = "zuerich" if city == "zueseh" 
replace city = "st. gallen" if city == "st. gasen" 
replace city = "lausanne" if city == "lausannes"
replace city = "neuchatel" if city == "sjeuchatel"  
replace city = "unterentfelden" if city == "unterent elden"
...
* 3 Missing values in PID
/*
PID	year	firstname	lastname
.	1962	Louis	Plojoux
.	1962	Louis	Plojoux
.	1979	Alain	Le Comte du Colombier
*/

replace PID = 999999 if PID == .  // 2 missing PIDs
...
label var ID_dupl "Duplicate ID within one year (PID-year duplicate)"
...
replace gdename = usubinstr(gdename , "(st. g.)" , "(sg)",.)
...
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page gdename gdename_std1
...
replace gdename = ustrregexra(gdename,"9oll","9011")  // replace "9oll" in string (st. gallen)
...
preserve  // check matches: check all entries on whether or not it is actaully a PLZ4
keep if _merge_3 == 3
gen n = 1
...
replace wrong_3 = 1 if gdename_std1 == "aarbergg. 29 / schw. coiffeurv, fachsch., 3oll bern"
...
preserve  // preserve the cases that correctly match
keep if wrong_3 == . & _merge_3 == 3
...
drop if _merge_3 == 3 & (wrong_3 == . | wrong_3 == 2)   // drop all correct matches from step 3
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page gdename gdename_std1
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo3_notmerged.dta", replace


* 4a/b: Split gdename to find cities in longer strings
...
* Round 4 matches:
*     4a: Correct observations (hand-checked)
*     4b: identify matches from this round in data, extract them, reshape long (more than one potential match) including all information, add corrections (hand-checked)
...
// identify those PID-year-ID_dupl observations that are matched in 4a
gen help1 = 1 if _merge_4a == 3
bysort PID year ID_dupl gdename_std1: egen m4a = total(help1)
drop help1

// all those observations that are not affected by 4a
preserve 
keep if m4a == 0
duplicates drop PID year ID_dupl gdename_std1, force 
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page gdename gdename_std1 nomcap gdename_std4
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4a_notmerged.dta", replace
restore

keep if m4a > 0  // focus on those affected by 4a
drop if _merge_4a == 1
...
replace wrong4a = 1 if gdename_std1 == "neuhausen a. r, wohnbauges. der georg fischer ag, schaffhausen" & gdename_4a == "schaffhausen"
...
replace wrong4a = 1 if gdename_std1 == "uitikon a.a, verwaltungsges. fuer investment trusts (vit), zuerich" & gdename_4a == "zuerich"
replace wrong4a = 1 if gdename_std1 == "gasthaus vierlinden, unterboezberg" & gdename_4a == "unterboezberg"
...
replace wrong4a = 1 if gdename_std1 == "muri b., bern" & gdename_4a == "bern"
...
replace wrong4a = 1 if gdename_std1 == "widen, 882owaedenswll" & gdename_4a == "widen"
replace wrong4a = 1 if gdename_std1 == "neuchatel, route de 12, 252a la neuveville" & gdename_4a == "neuchatel"
...
replace wrong4a = 1 if gdename_std1 == "geneve cie d etudes et de participations industrielles, geneve" & gdename_4a == "geneve"
replace wrong4a = 1 if gdename_std1 == "muri, bern" & gdename_4a == "bern"
replace wrong4a = 1 if gdename_std1 == "curacao n.v, tcpl finance nv, curacao, zug branch, zug" & gdename_4a == "zug"
replace wrong4a = 1 if gdename_std1 == "langnau a. a, texsana reinigung hirschmatt luzern ag, luzern" & gdename_4a == "luzern"
replace wrong4a = 1 if gdename_std1 == "dorf, ursenbach" & gdename_4a == "dorf"
replace wrong4a = 1 if gdename_std1 == "winterthur» lebensversicherungs gesellschaft, winterthur" & gdename_4a == "winterthur"
...
replace wrong4a = 1 if gdename_std1 == "zuerich. drahtseilbahn interlaken heimwehfluh ag, interlaken" & gdename_4a == "interlaken"
...
replace wrong4a = 1 if gdename_std1 == "dorf, 614a zell lu" & gdename_4a == "dorf"
...
replace wrong4a = 1 if gdename_std1 == "la chaux de fonds s.i. cafe des sports s.a., la chaux de fonds" & gdename_4a == "la chaux de fonds"
replace wrong4a = 1 if gdename_std1 == "bruxelles b, bankfuer internationalen zahlungsausgleich, basel" & gdename_4a == "basel"
replace wrong4a = 1 if gdename_std1 == "baden gebr. wind bau und automalerei ag, baden" & gdename_4a == "baden"
...
replace wrong4a = 1 if gdename_std1 == "dorf, 61.44 zell lu" & gdename_4a == "dorf"
...
replace wrong4a = 1 if gdename_std1 == "kuessnacht a. r, villa sagirain, kuessnacht am rigi, schwyz (0.1)" & gdename_4a == "kuessnacht am rigi"
replace wrong4a = 1 if gdename_std1 == "stanztechnik, bettlach" & gdename_4a == "bettlach"
replace wrong4a = 1 if gdename_std1 == "dorf, 614.4 zell lu" & gdename_4a == "dorf"
...
replace wrong4a = 1 if gdename_std1 == "uetikon a.s, johann weibel ag, mechanische ziegelei, eschlikon" & gdename_4a == "eschlikon"
...
replace wrong4a = 1 if gdename_std1 == "neuhausen a.r, bruetsch leu ag. hoch und tiefbau, schaffhausen" & gdename_4a == "schaffhausen"
...
replace gdename = "uitikon am albis" if gdename_std1 == "uitikon a.a, verwaltungsges. fuer investment trusts (vit), zuerich" & gdename_4a == "zuerich"
...
replace gdename = "flawil" if gdename_std1 == "flawil , stahlbau und montagen ag altwil sg, gaiserwald" & gdename_4a == "gaiserwald"
...
replace gdename = "lugano" if gdename_std1 == "lugano, lnpraegnierwerke brittnau/wikon ag, brittnau" & gdename_4a == "brittnau"
replace gdename = "zell (lu)" if gdename_std1 == "dorf, 61.44 zell lu" & gdename_4a == "dorf"
replace gdename = "kuessnacht am rigi" if gdename_std1 == "kuessnacht a. r, villa sagirain, kuessnacht am rigi, schwyz (0.1)" & gdename_4a == "kuessnacht am rigi"
...
// observations corrected for second round geo-matching 4a
gen correct4a = .
replace correct4a = 1 if gdename_std1 == "neuhausen a. r, wohnbauges. der georg fischer ag, schaffhausen" & gdename_4a == "schaffhausen"
replace correct4a = 1 if gdename_std1 == "uitikon a.a, verwaltungsges. fuer investment trusts (vit), zuerich" & gdename_4a == "zuerich"
replace correct4a = 1 if gdename_std1 == "muri b., bern" & gdename_4a == "bern"
replace correct4a = 1 if gdename_std1 == "widen, 882owaedenswll" & gdename_4a == "widen"
replace correct4a = 1 if gdename_std1 == "muri, bern" & gdename_4a == "bern"
replace correct4a = 1 if gdename_std1 == "langnau a. a, texsana reinigung hirschmatt luzern ag, luzern" & gdename_4a == "luzern"
replace correct4a = 1 if gdename_std1 == "flawil , stahlbau und montagen ag altwil sg, gaiserwald" & gdename_4a == "gaiserwald"
replace correct4a = 1 if gdename_std1 == "zuerich. drahtseilbahn interlaken heimwehfluh ag, interlaken" & gdename_4a == "interlaken"
replace correct4a = 1 if gdename_std1 == "dorf, 614a zell lu" & gdename_4a == "dorf"
replace correct4a = 1 if gdename_std1 == "lugano, lnpraegnierwerke brittnau/wikon ag, brittnau" & gdename_4a == "brittnau"
replace correct4a = 1 if gdename_std1 == "dorf, 61.44 zell lu" & gdename_4a == "dorf"
replace correct4a = 1 if gdename_std1 == "kuessnacht a. r, villa sagirain, kuessnacht am rigi, schwyz (0.1)" & gdename_4a == "kuessnacht am rigi"
replace correct4a = 1 if gdename_std1 == "dorf, 614.4 zell lu" & gdename_4a == "dorf"
replace correct4a = 1 if gdename_std1 == "uetikon a.s, johann weibel ag, mechanische ziegelei, eschlikon" & gdename_4a == "eschlikon"
replace correct4a = 1 if gdename_std1 == "neuhausen a.r, bruetsch leu ag. hoch und tiefbau, schaffhausen" & gdename_4a == "schaffhausen"

// identify PID-year-ID_dupl combinations that are: a) wrong (notmerged_4a), b) correct after first round 4a, and c) need second round match 4a
gen nhelp = 1
bysort PID year ID_dupl gdename_std1 : egen nr_multi = total(nhelp)  // number of multiple options per PID-year-ID_dupl combination
bysort PID year ID_dupl gdename_std1 : egen nr_wrong4a = total(wrong4a)  // number of wrong matches
gen nomerge4a = 1 if nr_multi == nr_wrong4a & nr_wrong != 0 // if all matches are wrong  -- > indicator == 1  -- > no match after 4a

// a) those that are notmerged in 4a after corrections 
preserve
keep if nomerge4a == 1 & correct4a != 1
*duplicates report PID year ID_dupl gdename_std1
*duplicates drop PID year ID_dupl gdename_std1, force 
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page gdename gdename_std1 nomcap gdename_std4
append using "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4a_notmerged.dta"
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4a_notmerged.dta", replace
restore

// b) correct matches from 4a first round
preserve
keep if nomerge4a != 1 & correct4a != 1
drop if wrong4a == 1
duplicates drop PID year ID_dupl gdename_std1 gdename, force 
bysort PID year ID_dupl gdename_std1: gen options4a = _n
rename E_CNTR_4a GdeNr_E_CNTR
rename N_CNTR_4a GdeNr_N_CNTR

gen geo = 41
gen gdename_used = gdename
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page ///
gdename GdeNr_E_CNTR GdeNr_N_CNTR gdenr_2018 gdename_used geo options4a
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo41.dta", replace
restore

// c) matches for second round 4a
keep if correct4a == 1
drop gdenr_2018 
sort gdename
...
duplicates list PID year ID_dupl gdename 
duplicates drop PID year ID_dupl gdename, force 
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo41.dta", replace


// 4b: municipality names without cantonal info in () and other special characters (loop through duplicates-municipal-geo datasests)
use "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4a_notmerged.dta", clear
drop gdename
...
replace gdename = usubinstr(gdename, "  ", " ",.)
replace gdename = ustrtrim(gdename)
replace gdename = "" if nomcap != ""  // capital information extracted before (in parentheses) provides the information on wrong (firm) matches
drop gdename_new

// merge on gdenames without canton info, on sequence of non-duplicate geo-datasets
sort gdename year
forv n = 0(1)4 {
	merge m:1 gdename year using "$path\02_Processed_data\10_Directors_1934_2003\Gde_GdeNr2018_Centroid_1931-2018_dupl`n'.dta" // sub-datasets with unique version of duplicate municipal names (e.g., biel)
	drop if _merge==2
	rename _merge merge_4b`n'
	rename gdenr_2018 gdenr_2018_4b`n' 
	rename E_CNTR E_CNTR_4b`n'
	rename N_CNTR N_CNTR_4b`n'
	rename ctn ctn`n'
}
...
bysort PID year ID_dupl: gen n_obs_4 = _n
reshape long ctn merge_4b gdenr_2018_4b E_CNTR_4b N_CNTR_4b, i(PID year ID_dupl n_obs_4) j(n_obs_4b)

// There are two sorts of multiple matches per unique PID-year-ID_dupl gdename_std1 combination:
// 	1) those multiple observations per ID stemming from the split
//  2) those multiple observations per ID stemming from the muliple options of the municipal name without cantonal information

gen nhelp = 1 if merge_4b == 3
bysort PID year ID_dupl gdename_std1 : egen m4b = total (nhelp) // total number of matches per unique PID-year-ID_dupl combinations
drop nhelp

// subsample of not-merged 4b observations
preserve
keep if m4b == 0 // no merges occured
duplicates drop PID year ID_dupl gdename_std1, force  // this duplicate drop command also drops the additional observations created by the split in round 4
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4b_notmerged.dta", replace
restore 
...
gen nhelp = 1
bysort PID year ID_dupl gdename_std1: egen options4b = total(nhelp)  // number of options per unique PID-year-ID_dupl combination
drop nhelp

// produce list with unique gde-info for manual checks
preserve
gen n = -1
collapse (sum) n, by (gdename_std1 gdenr_2018_4b gdename_4b ctn)
order n 
sort n
save "$path\02_Processed_data\10_Directors_1934_2003\check_4b.dta", replace
restore

// Identify wrong matches in 4b
gen wrong4b = .
replace wrong4b = 1 if gdename_std1 == ".3941 biel" & gdename_4b == "biel" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "31 decembre, rue 37, 12oe0 geneve" & gdename_4b == "rue" & gdenr_2018_4b == 2097
replace wrong4b = 1 if gdename_std1 == "4.333 muenchwilen" & gdename_4b == "muenchwilen" & gdenr_2018_4b == 4746
replace wrong4b = 1 if gdename_std1 == "43(2 stein" & gdename_4b == "stein" & gdenr_2018_4b == 3360
replace wrong4b = 1 if gdename_std1 == "43(2 stein" & gdename_4b == "stein" & gdenr_2018_4b == 3005
replace wrong4b = 1 if gdename_std1 == "aemmetweg 2, wetzikon" & gdename_4b == "wetzikon" & gdenr_2018_4b == 4611
replace wrong4b = 1 if gdename_std1 == "aesch (basel)" & gdename_4b == "aesch" & gdenr_2018_4b == 1021
replace wrong4b = 1 if gdename_std1 == "aesch (bld)" & gdename_4b == "aesch" & gdenr_2018_4b == 1021
replace wrong4b = 1 if gdename_std1 == "aesch (zh)" & gdename_4b == "aesch" & gdenr_2018_4b == 1021
replace wrong4b = 1 if gdename_std1 == "aesch (zh)" & gdename_4b == "aesch" & gdenr_2018_4b == 2761
replace wrong4b = 1 if gdename_std1 == "arlesheim (d)" & gdename_4b == "arlesheim" & gdenr_2018_4b == 2763
replace wrong4b = 1 if gdename_std1 == "au 1150, 9056 ais" & gdename_4b == "au" & gdenr_2018_4b == 3231
replace wrong4b = 1 if gdename_std1 == "aubonne (ne)" & gdename_4b == "aubonne" & gdenr_2018_4b == 5422
replace wrong4b = 1 if gdename_std1 == "aubrigstr. 15, rueti" & gdename_4b == "rueti" & gdenr_2018_4b == 1631
replace wrong4b = 1 if gdename_std1 == "augartenstr. 36, andwil" & gdename_4b == "andwil" & gdenr_2018_4b == 4901
replace wrong4b = 1 if gdename_std1 == "av. tir fedederal 94, ecublens" & gdename_4b == "ecublens" & gdenr_2018_4b == 2072
replace wrong4b = 1 if gdename_std1 == "bad hamburg v.d.h. decsmholdingund finanz ag, buchs, buchs" & gdename_4b == "buchs" & gdenr_2018_4b == 83
replace wrong4b = 1 if gdename_std1 == "bad hamburg v.d.h. decsmholdingund finanz ag, buchs, buchs" & gdename_4b == "buchs" & gdenr_2018_4b == 4003
replace wrong4b = 1 if gdename_std1 == "bad hamburg v.d.h. decsmholdingund finanz ag, buchs, buchs" & gdename_4b == "buchs" & gdenr_2018_4b == 1125
replace wrong4b = 1 if gdename_std1 == "bad hamburg v.d.h. decsmholdingund finanz ag, buchs, buchs" & gdename_4b == "buchs" & gdenr_2018_4b == 3271
replace wrong4b = 1 if gdename_std1 == "baden (d)" & gdename_4b == "baden" & gdenr_2018_4b == 4021
replace wrong4b = 1 if gdename_std1 == "baerenfelsweg 14, aesch" & gdename_4b == "aesch" & gdenr_2018_4b == 1021
replace wrong4b = 1 if gdename_std1 == "bahnhofstr. 21, biel" & gdename_4b == "biel" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "bellerive (ge)" & gdename_4b == "bellerive" & gdenr_2018_4b == 5464
replace wrong4b = 1 if gdename_std1 == "bellevue (usa)" & gdename_4b == "bellevue" & gdenr_2018_4b == 6606
replace wrong4b = 1 if gdename_std1 == "berg (d)" & gdename_4b == "berg" & gdenr_2018_4b == 4891
replace wrong4b = 1 if gdename_std1 == "berg (d)" & gdename_4b == "berg" & gdenr_2018_4b == 3211
replace wrong4b = 1 if gdename_std1 == "berg , starnberger see d" & gdename_4b == "berg" & gdenr_2018_4b == 3211
replace wrong4b = 1 if gdename_std1 == "berg , starnberger see d" & gdename_4b == "berg" & gdenr_2018_4b == 4891
replace wrong4b = 1 if gdename_std1 == "berg, starnberger see d" & gdename_4b == "berg" & gdenr_2018_4b == 4891
replace wrong4b = 1 if gdename_std1 == "berg, starnberger see d" & gdename_4b == "berg" & gdenr_2018_4b == 3211
replace wrong4b = 1 if gdename_std1 == "bern cie du chemin de fer montreux oberland bernois, montreux (3, 54169)" & gdename_4b == "montreux" & gdenr_2018_4b == 5886
replace wrong4b = 1 if gdename_std1 == "bern. , ges. zur foerderung des wohnungsbaues fuer funktio" & gdename_4b == "bern" & gdenr_2018_4b == 351
replace wrong4b = 1 if gdename_std1 == "bolken (be)" & gdename_4b == "bolken" & gdenr_2018_4b == 2514
replace wrong4b = 1 if gdename_std1 == "bremgarten (d)" & gdename_4b == "bremgarten" & gdenr_2018_4b == 4063
replace wrong4b = 1 if gdename_std1 == "brunnenthal (d)" & gdename_4b == "brunnenthal" & gdenr_2018_4b == 2457
replace wrong4b = 1 if gdename_std1 == "bruxelles b, bank fuer internationalen zahlungsausgleich, basel (1500, 0)" & gdename_4b == "basel" & gdenr_2018_4b == 2701
replace wrong4b = 1 if gdename_std1 == "buchberg 341, thai" & gdename_4b == "buchberg" & gdenr_2018_4b == 2933
replace wrong4b = 1 if gdename_std1 == "buehl (d)" & gdename_4b == "buehl" & gdenr_2018_4b == 734
replace wrong4b = 1 if gdename_std1 == "burgdorf (d)" & gdename_4b == "burgdorf" & gdenr_2018_4b == 404
replace wrong4b = 1 if gdename_std1 == "busswil (be)" & gdename_4b == "busswil" & gdenr_2018_4b == 4761
replace wrong4b = 1 if gdename_std1 == "ch. de l ormet 14, ecublens" & gdename_4b == "ecublens" & gdenr_2018_4b == 2072
replace wrong4b = 1 if gdename_std1 == "ch. du stand 15b, ecublens" & gdename_4b == "ecublens" & gdenr_2018_4b == 2072
replace wrong4b = 1 if gdename_std1 == "ch. tortiquet 15, villeneuve" & gdename_4b == "villeneuve" & gdenr_2018_4b == 2044
replace wrong4b = 1 if gdename_std1 == "chapelle, route 40, gran lancy" & gdename_4b == "chapelle" & gdenr_2018_4b == 2066
replace wrong4b = 1 if gdename_std1 == "chapelle, route 40, gran lancy" & gdename_4b == "chapelle" & gdenr_2018_4b == 2009
replace wrong4b = 1 if gdename_std1 == "cherventino, gerra" & gdename_4b == "gerra" & gdenr_2018_4b == 5138
replace wrong4b = 1 if gdename_std1 == "collonges (f)" & gdename_4b == "collonges" & gdenr_2018_4b == 6211
replace wrong4b = 1 if gdename_std1 == "cottens, le moulin" & gdename_4b == "cottens" & gdenr_2018_4b == 2186
replace wrong4b = 1 if gdename_std1 == "couvet (ge)" & gdename_4b == "couvet" & gdenr_2018_4b == 6512
replace wrong4b = 1 if gdename_std1 == "der ey, kirchberg" & gdename_4b == "kirchberg" & gdenr_2018_4b == 3392
replace wrong4b = 1 if gdename_std1 == "dicks, rue 15, niederanven" & gdename_4b == "rue" & gdenr_2018_4b == 2097
replace wrong4b = 1 if gdename_std1 == "dicks, rue 15, niederanven l" & gdename_4b == "rue" & gdenr_2018_4b == 2097
replace wrong4b = 1 if gdename_std1 == "dorfstr. 36, kilchberg" & gdename_4b == "kilchberg" & gdenr_2018_4b == 2851
replace wrong4b = 1 if gdename_std1 == "endingen (d)" & gdename_4b == "endingen" & gdenr_2018_4b == 4305
replace wrong4b = 1 if gdename_std1 == "ettingerstr. 47, reinach" & gdename_4b == "reinach" & gdenr_2018_4b == 4141
replace wrong4b = 1 if gdename_std1 == "fischbach (tg)" & gdename_4b == "fischbach" & gdenr_2018_4b == 1129
replace wrong4b = 1 if gdename_std1 == "fleischbachstr. 9, reinach" & gdename_4b == "reinach" & gdenr_2018_4b == 4141
replace wrong4b = 1 if gdename_std1 == "forel (uvaux)" & gdename_4b == "forel" & gdenr_2018_4b == 2054
replace wrong4b = 1 if gdename_std1 == "forel (vd)" & gdename_4b == "forel" & gdenr_2018_4b == 2054
replace wrong4b = 1 if gdename_std1 == "forel(lavaux)" & gdename_4b == "forel" & gdenr_2018_4b == 2054
replace wrong4b = 1 if gdename_std1 == "frankfurt a m, electromation ag, basel ( 5.45)" & gdename_4b == "basel" & gdenr_2018_4b == 2701
replace wrong4b = 1 if gdename_std1 == "friesenheid, schmitten" & gdename_4b == "schmitten" & gdenr_2018_4b == 3514
replace wrong4b = 1 if gdename_std1 == "geneve ste de l immeuble medical champel malombre, geneve (0, 0548)" & gdename_4b == "geneve" & gdenr_2018_4b == 6621
replace wrong4b = 1 if gdename_std1 == "granges (vd)" & gdename_4b == "granges" & gdenr_2018_4b == 2328
replace wrong4b = 1 if gdename_std1 == "granges (vd)" & gdename_4b == "granges" & gdenr_2018_4b == 6248
replace wrong4b = 1 if gdename_std1 == "grone (d)" & gdename_4b == "grone" & gdenr_2018_4b == 6238
replace wrong4b = 1 if gdename_std1 == "hauptwil (sg)" & gdename_4b == "hauptwil" & gdenr_2018_4b == 4486
replace wrong4b = 1 if gdename_std1 == "hoeheweg 24, biel" & gdename_4b == "biel" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "hof niederried, 34.33 schwanden" & gdename_4b == "schwanden" & gdenr_2018_4b == 1631
replace wrong4b = 1 if gdename_std1 == "homburg (d)" & gdename_4b == "homburg" & gdenr_2018_4b == 4816
replace wrong4b = 1 if gdename_std1 == "honore labande, rue 2, monaco" & gdename_4b == "rue" & gdenr_2018_4b == 2097
replace wrong4b = 1 if gdename_std1 == "ifangstr. 17, weiningen" & gdename_4b == "weiningen" & gdenr_2018_4b == 4621
replace wrong4b = 1 if gdename_std1 == "ins (gr)" & gdename_4b == "ins" & gdenr_2018_4b == 496
replace wrong4b = 1 if gdename_std1 == "jegensdorferstr. 3b, zuzwil" & gdename_4b == "zuzwil" & gdenr_2018_4b == 3426
replace wrong4b = 1 if gdename_std1 == "jun., stein, am rhein" & gdename_4b == "stein" & gdenr_2018_4b == 3360
replace wrong4b = 1 if gdename_std1 == "jun., stein, am rhein" & gdename_4b == "stein" & gdenr_2018_4b == 3005
replace wrong4b = 1 if gdename_std1 == "jun., stein, am rhein" & gdename_4b == "stein" & gdenr_2018_4b == 4260
replace wrong4b = 1 if gdename_std1 == "kanalweg 4, kirchberg" & gdename_4b == "kirchberg" & gdenr_2018_4b == 3392
replace wrong4b = 1 if gdename_std1 == "kappel (sg)" & gdename_4b == "kappel" & gdenr_2018_4b == 2580
replace wrong4b = 1 if gdename_std1 == "kesswil (sg)" & gdename_4b == "kesswil" & gdenr_2018_4b == 4426
replace wrong4b = 1 if gdename_std1 == "kesswil (tg)" & gdename_4b == "kesswil" & gdenr_2018_4b == 4426
replace wrong4b = 1 if gdename_std1 == "kloesterliweg 6, oberhofen" & gdename_4b == "oberhofen" & gdenr_2018_4b == 4184
replace wrong4b = 1 if gdename_std1 == "koblenz (d)" & gdename_4b == "koblenz" & gdenr_2018_4b == 4310
replace wrong4b = 1 if gdename_std1 == "kuessnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil" & gdename_4b == "hergiswil" & gdenr_2018_4b == 1507
replace wrong4b = 1 if gdename_std1 == "kussnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil" & gdename_4b == "hergiswil" & gdenr_2018_4b == 1507
replace wrong4b = 1 if gdename_std1 == "la joux (be)" & gdename_4b == "la joux" & gdenr_2018_4b == 2113
replace wrong4b = 1 if gdename_std1 == "lajoux" & gdename_4b == "lajoux" & gdenr_2018_4b == 6750
replace wrong4b = 1 if gdename_std1 == "laufenburg (d)" & gdename_4b == "laufenburg" & gdenr_2018_4b == 4170
replace wrong4b = 1 if gdename_std1 == "le pommeret, le paquier" & gdename_4b == "le paquier" & gdenr_2018_4b == 6487
replace wrong4b = 1 if gdename_std1 == "lerchenstr. 12, 920 gossau (sg)" & gdename_4b == "gossau" & gdenr_2018_4b == 115
replace wrong4b = 1 if gdename_std1 == "lfangstr. 17, weiningen" & gdename_4b == "weiningen" & gdenr_2018_4b == 4621
replace wrong4b = 1 if gdename_std1 == "lindau (d)" & gdename_4b == "lindau" & gdenr_2018_4b == 176
replace wrong4b = 1 if gdename_std1 == "lussy (vd)" & gdename_4b == "lussy" & gdenr_2018_4b == 2116
replace wrong4b = 1 if gdename_std1 == "malans (sg)" & gdename_4b == "malans" & gdenr_2018_4b == 3954
replace wrong4b = 1 if gdename_std1 == "marbach (d)" & gdename_4b == "marbach" & gdenr_2018_4b == 3253
replace wrong4b = 1 if gdename_std1 == "marbach (d)" & gdename_4b == "marbach" & gdenr_2018_4b == 1010
replace wrong4b = 1 if gdename_std1 == "mauren (fl)" & gdename_4b == "mauren" & gdenr_2018_4b == 4891
replace wrong4b = 1 if gdename_std1 == "missoula, montana (usa)" & gdename_4b == "montana" & gdenr_2018_4b == 6253
replace wrong4b = 1 if gdename_std1 == "montet (vully)" & gdename_4b == "montet" & gdenr_2018_4b == 2089
replace wrong4b = 1 if gdename_std1 == "morgengab 537, mauren (fl)" & gdename_4b == "mauren" & gdenr_2018_4b == 4891
replace wrong4b = 1 if gdename_std1 == "muellheim (d)" & gdename_4b == "muellheim" & gdenr_2018_4b == 4831
replace wrong4b = 1 if gdename_std1 == "muenster (d)" & gdename_4b == "muenster" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "muenster (gr)" & gdename_4b == "muenster" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "muenster (lu)" & gdename_4b == "muenster" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "muri (be)" & gdename_4b == "muri" & gdenr_2018_4b == 4236
replace wrong4b = 1 if gdename_std1 == "muri (be) /" & gdename_4b == "muri" & gdenr_2018_4b == 4236
replace wrong4b = 1 if gdename_std1 == "neue jonastr. 81, rapperswil" & gdename_4b == "rapperswil" & gdenr_2018_4b == 310
replace wrong4b = 1 if gdename_std1 == "neuhausen a. r, schaffhauser strickmaschinenfabrik, schaffhausen (1.)" & gdename_4b == "schaffhausen" & gdenr_2018_4b == 2939
replace wrong4b = 1 if gdename_std1 == "nussbaumen (zh)" & gdename_4b == "nussbaumen" & gdenr_2018_4b == 4821
replace wrong4b = 1 if gdename_std1 == "oberdorf (basel)" & gdename_4b == "oberdorf" & gdenr_2018_4b == 1508
replace wrong4b = 1 if gdename_std1 == "oberdorf (basel)" & gdename_4b == "oberdorf" & gdenr_2018_4b == 2553
replace wrong4b = 1 if gdename_std1 == "oberdorf (bld)" & gdename_4b == "oberdorf" & gdenr_2018_4b == 1508
replace wrong4b = 1 if gdename_std1 == "oberdorf (bld)" & gdename_4b == "oberdorf" & gdenr_2018_4b == 2553
replace wrong4b = 1 if gdename_std1 == "oberdorf, s558 illhard" & gdename_4b == "oberdorf" & gdenr_2018_4b == 1508
replace wrong4b = 1 if gdename_std1 == "oberdorf, s558 illhard" & gdename_4b == "oberdorf" & gdenr_2018_4b == 2553
replace wrong4b = 1 if gdename_std1 == "oberdorf, s558 illhard" & gdename_4b == "oberdorf" & gdenr_2018_4b == 2892
replace wrong4b = 1 if gdename_std1 == "oberfeldring 18, arni" & gdename_4b == "arni" & gdenr_2018_4b == 602
replace wrong4b = 1 if gdename_std1 == "oberhofen (be)" & gdename_4b == "oberhofen" & gdenr_2018_4b == 4184
replace wrong4b = 1 if gdename_std1 == "oberhofen (tg)" & gdename_4b == "oberhofen" & gdenr_2018_4b == 4184
replace wrong4b = 1 if gdename_std1 == "oberkirch (d)" & gdename_4b == "oberkirch" & gdenr_2018_4b == 1095
replace wrong4b = 1 if gdename_std1 == "oberkirch (deutschland)" & gdename_4b == "oberkirch" & gdenr_2018_4b == 1095
replace wrong4b = 1 if gdename_std1 == "oberkirch (so)" & gdename_4b == "oberkirch" & gdenr_2018_4b == 1095
replace wrong4b = 1 if gdename_std1 == "oberurnen (sz)" & gdename_4b == "oberurnen" & gdenr_2018_4b == 1630
replace wrong4b = 1 if gdename_std1 == "oberwil (basel)" & gdename_4b == "oberwil" & gdenr_2018_4b == 4074
replace wrong4b = 1 if gdename_std1 == "oberwil (basel)" & gdename_4b == "oberwil" & gdenr_2018_4b == 4571
replace wrong4b = 1 if gdename_std1 == "oberwil (bid)" & gdename_4b == "oberwil" & gdenr_2018_4b == 4571
replace wrong4b = 1 if gdename_std1 == "oberwil (bid)" & gdename_4b == "oberwil" & gdenr_2018_4b == 4074
replace wrong4b = 1 if gdename_std1 == "pfaeffikon (sz)" & gdename_4b == "pfaeffikon" & gdenr_2018_4b == 177
replace wrong4b = 1 if gdename_std1 == "ponte tresa (i)" & gdename_4b == "ponte tresa" & gdenr_2018_4b == 5213
replace wrong4b = 1 if gdename_std1 == "port (gb)" & gdename_4b == "port" & gdenr_2018_4b == 745
replace wrong4b = 1 if gdename_std1 == "r. de bassenges 2, ecublens" & gdename_4b == "ecublens" & gdenr_2018_4b == 2072
replace wrong4b = 1 if gdename_std1 == "r. fortifications 8, villeneuve" & gdename_4b == "villeneuve" & gdenr_2018_4b == 2044
replace wrong4b = 1 if gdename_std1 == "rapperswil (zh)" & gdename_4b == "rapperswil" & gdenr_2018_4b == 310
replace wrong4b = 1 if gdename_std1 == "reinach (basel)" & gdename_4b == "reinach" & gdenr_2018_4b == 4141
replace wrong4b = 1 if gdename_std1 == "reinach (bid)" & gdename_4b == "reinach" & gdenr_2018_4b == 4141
replace wrong4b = 1 if gdename_std1 == "rheinfelden (brd)" & gdename_4b == "rheinfelden" & gdenr_2018_4b == 4258
replace wrong4b = 1 if gdename_std1 == "rheinfelden (d)" & gdename_4b == "rheinfelden" & gdenr_2018_4b == 4258
replace wrong4b = 1 if gdename_std1 == "rheinfelden (deutschland)" & gdename_4b == "rheinfelden" & gdenr_2018_4b == 4258
replace wrong4b = 1 if gdename_std1 == "rickenbach (d)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "rickenbach (d)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "rickenbach (d)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "rickenbach (d)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "rickenbach (sg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "rickenbach (sg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "rickenbach (sg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "rickenbach (sg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "rickenbach (tg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "rickenbach (tg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "rickenbach (tg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "rickenbach (tg)" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 4751
replace wrong4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "riedweg 10, biel" & gdename_4b == "biel" & gdenr_2018_4b == 6077
replace wrong4b = 1 if gdename_std1 == "rochefort (france)" & gdename_4b == "rochefort" & gdenr_2018_4b == 6413
replace wrong4b = 1 if gdename_std1 == "rolliweg 15, lengnau" & gdename_4b == "lengnau" & gdenr_2018_4b == 4312
replace wrong4b = 1 if gdename_std1 == "rosenweg 318, 3 524 niederwil (ag)" & gdename_4b == "niederwil" & gdenr_2018_4b == 4571
replace wrong4b = 1 if gdename_std1 == "rosenweg 318, 3 524 niederwil (ag)" & gdename_4b == "niederwil" & gdenr_2018_4b == 2554
replace wrong4b = 1 if gdename_std1 == "rossweg, schmitten" & gdename_4b == "schmitten" & gdenr_2018_4b == 2305
replace wrong4b = 1 if gdename_std1 == "rte arrufens 8, romont" & gdename_4b == "romont" & gdenr_2018_4b == 442
replace wrong4b = 1 if gdename_std1 == "rte arruffens 57, romont" & gdename_4b == "romont" & gdenr_2018_4b == 442
replace wrong4b = 1 if gdename_std1 == "rueti (sg)" & gdename_4b == "rueti" & gdenr_2018_4b == 118
replace wrong4b = 1 if gdename_std1 == "rueti (sg)" & gdename_4b == "rueti" & gdenr_2018_4b == 1631
replace wrong4b = 1 if gdename_std1 == "ruetistr. 12, 4.332 stein (ag)" & gdename_4b == "stein" & gdenr_2018_4b == 3360
replace wrong4b = 1 if gdename_std1 == "ruetistr. 12, 4.332 stein (ag)" & gdename_4b == "stein" & gdenr_2018_4b == 3005
replace wrong4b = 1 if gdename_std1 == "s.i. parc du bude e, geneve (0, 1377)" & gdename_4b == "geneve" & gdenr_2018_4b == 6621
replace wrong4b = 1 if gdename_std1 == "schmitten (albula)" & gdename_4b == "schmitten" & gdenr_2018_4b == 2305
replace wrong4b = 1 if gdename_std1 == "schmitten (d)" & gdename_4b == "schmitten" & gdenr_2018_4b == 3514
replace wrong4b = 1 if gdename_std1 == "schmitten (d)" & gdename_4b == "schmitten" & gdenr_2018_4b == 2305
replace wrong4b = 1 if gdename_std1 == "schongau (d)" & gdename_4b == "schongau" & gdenr_2018_4b == 1041
replace wrong4b = 1 if gdename_std1 == "schuetzengasse, altdorf" & gdename_4b == "altdorf" & gdenr_2018_4b == 2920
replace wrong4b = 1 if gdename_std1 == "schuetzenmattstr. 54, 4.051 basel" & gdename_4b == "basel" & gdenr_2018_4b == 2701
replace wrong4b = 1 if gdename_std1 == "schuetzenstr. 40, gossau" & gdename_4b == "gossau" & gdenr_2018_4b == 115
replace wrong4b = 1 if gdename_std1 == "schulweg 21, kirchberg" & gdename_4b == "kirchberg" & gdenr_2018_4b == 3392
replace wrong4b = 1 if gdename_std1 == "schwarzenbach (sg)" & gdename_4b == "schwarzenbach" & gdenr_2018_4b == 1081
replace wrong4b = 1 if gdename_std1 == "schwarzenberg (a)" & gdename_4b == "schwarzenberg" & gdenr_2018_4b == 1066
replace wrong4b = 1 if gdename_std1 == "seestr. 129, kilchberg" & gdename_4b == "kilchberg" & gdenr_2018_4b == 2851
replace wrong4b = 1 if gdename_std1 == "sonnenberg 224, holderbank" & gdename_4b == "holderbank" & gdenr_2018_4b == 4199
replace wrong4b = 1 if gdename_std1 == "spilistatt, 38181wald" & gdename_4b == "wald" & gdenr_2018_4b == 3036
replace wrong4b = 1 if gdename_std1 == "spilistatt, 38181wald" & gdename_4b == "wald" & gdenr_2018_4b == 120
replace wrong4b = 1 if gdename_std1 == "st martin (ne)" & gdename_4b == "st martin" & gdenr_2018_4b == 3603
replace wrong4b = 1 if gdename_std1 == "st. martin (d)" & gdename_4b == "st martin" & gdenr_2018_4b == 3603
replace wrong4b = 1 if gdename_std1 == "st. martin (jersey)" & gdename_4b == "st martin" & gdenr_2018_4b == 3603
replace wrong4b = 1 if gdename_std1 == "st. martin(jersey)" & gdename_4b == "st martin" & gdenr_2018_4b == 3603
replace wrong4b = 1 if gdename_std1 == "st. niklaus (so)" & gdename_4b == "st niklaus" & gdenr_2018_4b == 6292
replace wrong4b = 1 if gdename_std1 == "stationsstrasse, 947buchs, raefis" & gdename_4b == "buchs" & gdenr_2018_4b == 4003
replace wrong4b = 1 if gdename_std1 == "stationsstrasse, 947buchs, raefis" & gdename_4b == "buchs" & gdenr_2018_4b == 1125
replace wrong4b = 1 if gdename_std1 == "stationsstrasse, 947buchs, raefis" & gdename_4b == "buchs" & gdenr_2018_4b == 83
replace wrong4b = 1 if gdename_std1 == "staufen (d)" & gdename_4b == "staufen" & gdenr_2018_4b == 4210
replace wrong4b = 1 if gdename_std1 == "steinen (brd)" & gdename_4b == "steinen" & gdenr_2018_4b == 1373
replace wrong4b = 1 if gdename_std1 == "steinen (d)" & gdename_4b == "steinen" & gdenr_2018_4b == 1373
replace wrong4b = 1 if gdename_std1 == "storchenweg 6, oberwil" & gdename_4b == "oberwil" & gdenr_2018_4b == 4571
replace wrong4b = 1 if gdename_std1 == "sursee (ag)" & gdename_4b == "sursee" & gdenr_2018_4b == 1103
replace wrong4b = 1 if gdename_std1 == "sytirain, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "sytirain, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "sytirain, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "sytirain, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "sytiraln, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 1097
replace wrong4b = 1 if gdename_std1 == "sytiraln, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 225
replace wrong4b = 1 if gdename_std1 == "sytiraln, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2857
replace wrong4b = 1 if gdename_std1 == "sytiraln, rickenbach" & gdename_4b == "rickenbach" & gdenr_2018_4b == 2582
replace wrong4b = 1 if gdename_std1 == "teufen (freienstein) (zh)" & gdename_4b == "teufen" & gdenr_2018_4b == 3024
replace wrong4b = 1 if gdename_std1 == "thalheim (d)" & gdename_4b == "thalheim" & gdenr_2018_4b == 4117
replace wrong4b = 1 if gdename_std1 == "thonex (f)" & gdename_4b == "thonex" & gdenr_2018_4b == 6640
replace wrong4b = 1 if gdename_std1 == "thunstr. 61, muri (be)" & gdename_4b == "muri" & gdenr_2018_4b == 4236
replace wrong4b = 1 if gdename_std1 == "uitikon (zh)" & gdename_4b == "uitikon" & gdenr_2018_4b == 248
replace wrong4b = 1 if gdename_std1 == "vernate (i)" & gdename_4b == "vernate" & gdenr_2018_4b == 5230
replace wrong4b = 1 if gdename_std1 == "villette (ge)" & gdename_4b == "villette" & gdenr_2018_4b == 5613
replace wrong4b = 1 if gdename_std1 == "waldenburg (d)" & gdename_4b == "waldenburg" & gdenr_2018_4b == 2895
replace wrong4b = 1 if gdename_std1 == "waldkirch (d)" & gdename_4b == "waldkirch" & gdenr_2018_4b == 3444
replace wrong4b = 1 if gdename_std1 == "wangen (d)" & gdename_4b == "wangen" & gdenr_2018_4b == 1349
replace wrong4b = 1 if gdename_std1 == "wangen a. a, oberaareauische automobilkurse ag, wangen" & gdename_4b == "wangen" & gdenr_2018_4b == 200
replace wrong4b = 1 if gdename_std1 == "wangen a. a, oberaareauische automobilkurse ag, wangen" & gdename_4b == "wangen" & gdenr_2018_4b == 1349
replace wrong4b = 1 if gdename_std1 == "wangen a. a, oberaargauische automobilkurse ag, wangen" & gdename_4b == "wangen" & gdenr_2018_4b == 1349
replace wrong4b = 1 if gdename_std1 == "wangen a. a, oberaargauische automobilkurse ag, wangen" & gdename_4b == "wangen" & gdenr_2018_4b == 200
replace wrong4b = 1 if gdename_std1 == "wehr, baden (d)" & gdename_4b == "baden" & gdenr_2018_4b == 4021
replace wrong4b = 1 if gdename_std1 == "weingarten (d)" & gdename_4b == "weingarten" & gdenr_2018_4b == 4741
replace wrong4b = 1 if gdename_std1 == "weiningen (baden)" & gdename_4b == "weiningen" & gdenr_2018_4b == 4621
replace wrong4b = 1 if gdename_std1 == "weiningen (baden)" & gdename_4b == "weiningen" & gdenr_2018_4b == 251
replace wrong4b = 1 if gdename_std1 == "weissensteinstr. 146, oberdorf" & gdename_4b == "oberdorf" & gdenr_2018_4b == 2892
replace wrong4b = 1 if gdename_std1 == "weissensteinstr. 146, oberdorf" & gdename_4b == "oberdorf" & gdenr_2018_4b == 1508
replace wrong4b = 1 if gdename_std1 == "weston, mass. (usa) ciba geigy ag, basel (426, 1064)" & gdename_4b == "basel" & gdenr_2018_4b == 2701
replace wrong4b = 1 if gdename_std1 == "wiesen (d)" & gdename_4b == "wiesen" & gdenr_2018_4b == 3851
replace wrong4b = 1 if gdename_std1 == "wildberg (d)" & gdename_4b == "wildberg" & gdenr_2018_4b == 182
replace wrong4b = 1 if gdename_std1 == "winkel 7, 947gbuchs sg" & gdename_4b == "winkel" & gdenr_2018_4b == 72
replace wrong4b = 1 if gdename_std1 == "wohlen (be)" & gdename_4b == "wohlen" & gdenr_2018_4b == 4082
replace wrong4b = 1 if gdename_std1 == "zell (a)" & gdename_4b == "zell" & gdenr_2018_4b == 231
replace wrong4b = 1 if gdename_std1 == "zell (a)" & gdename_4b == "zell" & gdenr_2018_4b == 1150
replace wrong4b = 1 if gdename_std1 == "zell (d)" & gdename_4b == "zell" & gdenr_2018_4b == 231
replace wrong4b = 1 if gdename_std1 == "zell (d)" & gdename_4b == "zell" & gdenr_2018_4b == 1150

// correction of wrong entries in 4b
replace gdename = "geneve" if gdename_std1 == "31 decembre, rue 37, 12oe0 geneve"
replace gdename = "aesch bei birmensdorf" if gdename_std1 == "aesch (zh)"
replace gdename = "lancy" if gdename_std1 == "chapelle, route 40, gran lancy"
replace gdename = "ruederswil" if gdename_std1 == "hof niederried, 34.33 schwanden"
replace gdename = "stein am rhein" if gdename_std1 == "jun., stein, am rhein"
replace gdename = "st. gallen" if gdename_std1 == "kesswil (sg)"
replace gdename = "kuessnacht am rigi" if gdename_std1 == "kuessnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil"
replace gdename = "kuessnacht am rigi" if gdename_std1 == "kussnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil"
replace gdename = "lajoux" if gdename_std1 == "la joux (be)"
replace gdename = "muestair" if gdename_std1 == "muenster (gr)"
replace gdename = "muri bei bern" if gdename_std1 == "muri (be)"
replace gdename = "muri bei bern" if gdename_std1 == "muri (be) /"
replace gdename = "neuhausen am rheinfall" if gdename_std1 == "neuhausen a. r, schaffhauser strickmaschinenfabrik, schaffhausen (1.)"
replace gdename = "wigoltingen" if gdename_std1 == "oberdorf, s558 illhard"
replace gdename = "oberhofen am thunersee" if gdename_std1 == "oberhofen (be)"
replace gdename = "zullwil" if gdename_std1 == "oberkirch (so)"
replace gdename = "freienbach" if gdename_std1 == "pfaeffikon (sz)"
replace gdename = "wattwil" if gdename_std1 == "rickenbach (sg)"
replace gdename = "rickenbach bei wil" if gdename_std1 == "rickenbach (tg)"
replace gdename = "schwyz" if gdename_std1 == "rickenbachstrasse 155, rickenbach"
replace gdename = "waldkirch" if gdename_std1 == "rueti (sg)"
replace gdename = "jonschwil" if gdename_std1 == "schwarzenbach (sg)"
replace gdename = "grindelwald" if gdename_std1 == "spilistatt, 38181wald"
replace gdename = "feldbrunnen st niklaus" if gdename_std1 == "st. niklaus (so)"
replace gdename = "schwyz" if gdename_std1 == "sytirain, rickenbach"
replace gdename = "freienstein teufen" if gdename_std1 == "teufen (freienstein) (zh)"
replace gdename = "muri bei bern" if gdename_std1 == "thunstr. 61, muri (be)"
replace gdename = "thonex" if gdename_std1 == "villette (ge)"
replace gdename = "wangen an der aare" if gdename_std1 == "wangen a. a, oberaareauische automobilkurse ag, wangen"
replace gdename = "buchs sg" if gdename_std1 == "winkel 7, 947gbuchs sg"
replace gdename = "wohlen bei bern" if gdename_std1 == "wohlen (be)"

// observations not to be dropped before new geo-matching
gen correct4b = .
replace correct4b = 1 if gdename_std1 == "31 decembre, rue 37, 12oe0 geneve"
replace correct4b = 1 if gdename_std1 == "aesch (zh)"
replace correct4b = 1 if gdename_std1 == "chapelle, route 40, gran lancy"
replace correct4b = 1 if gdename_std1 == "hof niederried, 34.33 schwanden"
replace correct4b = 1 if gdename_std1 == "jun., stein, am rhein"
replace correct4b = 1 if gdename_std1 == "kesswil (sg)"
replace correct4b = 1 if gdename_std1 == "kuessnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil"
replace correct4b = 1 if gdename_std1 == "kussnacht a.r, schweiz. glasindustrie siegwart cie ag, hergiswil"
replace correct4b = 1 if gdename_std1 == "la joux (be)"
replace correct4b = 1 if gdename_std1 == "muenster (gr)"
replace correct4b = 1 if gdename_std1 == "muri (be)"
replace correct4b = 1 if gdename_std1 == "muri (be) /"
replace correct4b = 1 if gdename_std1 == "neuhausen a. r, schaffhauser strickmaschinenfabrik, schaffhausen (1.)"
replace correct4b = 1 if gdename_std1 == "oberdorf, s558 illhard"
replace correct4b = 1 if gdename_std1 == "oberhofen (be)"
replace correct4b = 1 if gdename_std1 == "oberkirch (so)"
replace correct4b = 1 if gdename_std1 == "pfaeffikon (sz)"
replace correct4b = 1 if gdename_std1 == "rickenbach (sg)"
replace correct4b = 1 if gdename_std1 == "rickenbach (tg)"
replace correct4b = 1 if gdename_std1 == "rickenbachstrasse 155, rickenbach"
replace correct4b = 1 if gdename_std1 == "rueti (sg)"
replace correct4b = 1 if gdename_std1 == "schwarzenbach (sg)"
replace correct4b = 1 if gdename_std1 == "spilistatt, 38181wald"
replace correct4b = 1 if gdename_std1 == "st. niklaus (so)"
replace correct4b = 1 if gdename_std1 == "sytirain, rickenbach"
replace correct4b = 1 if gdename_std1 == "teufen (freienstein) (zh)"
replace correct4b = 1 if gdename_std1 == "thunstr. 61, muri (be)"
replace correct4b = 1 if gdename_std1 == "villette (ge)"
replace correct4b = 1 if gdename_std1 == "wangen a. a, oberaareauische automobilkurse ag, wangen"
replace correct4b = 1 if gdename_std1 == "winkel 7, 947gbuchs sg"
replace correct4b = 1 if gdename_std1 == "wohlen (be)"

// identify PID-year-ID_dupl combinations that either a) remain unmatched (notmerged_4b),  b) have been matched correctly, and c) need second round matching
gen nhelp = 1
bysort PID year ID_dupl gdename_std1 : egen nr_multi = total(nhelp)  // number of multiple options per PID-year-ID_dupl
bysort PID year ID_dupl gdename_std1 : egen nr_wrong4b = total(wrong4b)  // number of wrong matches
gen nomerge4b2 = 1 if nr_multi == nr_wrong4b & nr_wrong4b != 0 // a) if all matches are wrong  -- > indicator == 1 -- > add to sample of non-merged4b
drop nhelp

// a) subsample of not-merged 4b observations
preserve
keep if nomerge4b2 == 1 & correct4b != 1
duplicates drop PID year ID_dupl gdename_std1, force  // this duplicate drop command also drops the additional observations created by the split in round 4
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page gdename gdename_std1 nomcap gdename_std4
append using "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4b_notmerged.dta"
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4b_notmerged.dta", replace
restore 

// b) keep correct matches of first round 4b
preserve 
keep if nomerge4b != 1 & correct4b != 1
drop if wrong4b == 1 // all wrong matches
duplicates drop PID year ID_dupl gdename_std1 gdename ctn, force 
rename E_CNTR_4b GdeNr_E_CNTR
rename N_CNTR_4b GdeNr_N_CNTR
rename gdenr_2018_4b gdenr_2018
drop options4b
bysort PID year ID_dupl gdename_std1: gen options4b = _n

gen geo = 42
gen gdename_used = gdename_4b
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page ///
gdename GdeNr_E_CNTR GdeNr_N_CNTR gdenr_2018 gdename_used geo options4b
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo42.dta", replace
restore 

// c) matches for second round of 4b
keep if correct4b == 1
duplicates drop PID year ID_dupl gdename_std1, force 
drop n_obs_4b
sort gdename
merge m:1 gdename year using "$path\02_Processed_data\10_Directors_1934_2003\Gde_GdeNr2018_Centroid_1931-2018_dupl0.dta" 
drop if _merge==2
rename _merge _merge_4b2
rename E_CNTR GdeNr_E_CNTR
rename N_CNTR GdeNr_N_CNTR 
drop options4b
bysort PID year ID_dupl gdename_std1: gen options4b = _n
...
use "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4b_notmerged.dta", clear
drop gdename
...
replace gdename = "hofstetten (so)" if gdename == "fiueh" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "fiueh so" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "flueh" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "flueh so" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "flueh/so" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "fluh" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "fluh so" & PLZ4 == 4114
...
replace gdename = "hofstetten (so)" if gdename == "hofsteffen" & PLZ4 == 4114
replace gdename = "hofstetten (so)" if gdename == "hofsteffen so" & PLZ4 == 4114
...
drop _merge
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page ///
countPLZ4 GdeNr_E_CNTR GdeNr_N_CNTR gdenr_2018 gdename_used geo
...
drop if _merge == 3
drop _merge
gen n_gde = 1  // for those gde. names for which more than one match is possible
...
// further round of manual coding (Geo_coding_gdename2 -- > iteration of "stillmissing")
...
replace gdename = "chateau d oex" if gdename == "chateau d œx"
replace gdename = "baden" if gdename == "baden (ag)"
replace gdename = "morrens (vd)" if gdename == "morrens"
replace gdename = "rueschlikon" if gdename == "rueschlikon (zh)"
replace gdename = "genthod" if gdename == "genthod (ge)"
replace gdename = "nendaz" if gdename == "beusson/nendaz"
replace gdename = "mellingen" if gdename == "bruggerstr. 17, mellingen ag"
replace gdename = "montreux" if gdename == "villa le flon, territet montreux"
replace gdename = "hilterfingen" if gdename == "huenibach hilterfingen"
replace gdename = "widen" if gdename == "widen ag"
replace gdename = "kuessnacht (sz)" if gdename == "kuessnacht sz"
replace gdename = "freienbach" if gdename == "pfaeffikon/freienbach"
replace gdename = "baar" if gdename == "allenwinden/baar"
replace gdename = "koeniz" if gdename == "wabern/koeniz"
replace gdename = "lutry" if gdename == "la conversion lutry"
replace gdename = "wohlen bei bern" if gdename == "hinterkappelen wohlen be"
replace gdename = "saanen" if gdename == "schoried"
replace gdename = "hausen (ag)" if gdename == "hausen b. brugg"
replace gdename = "affoltern am albis" if gdename == "affoltern a. a"
replace gdename = "duedingen" if gdename == "mariahilf"
replace gdename = "ependes (vd)" if gdename == "ependens vd"
replace gdename = "horn" if gdename == "horn tg"
replace gdename = "aeschi bei spiez" if gdename == "aeschi b. spiez"
replace gdename = "altstaetten" if gdename == "luechingen/altstaetten"
replace gdename = "seewen" if gdename == "mittlerer kreis 25, seewen, therwil"
replace gdename = "muri bei bern" if gdename == "guemligen/muri be"
replace gdename = "roethenbach im emmental" if gdename == "roethenbach i.e"
replace gdename = "schwyz" if gdename == "rickenbach/schwyz"
replace gdename = "montreux" if gdename == "les avants/montreux"
replace gdename = "illnau" if gdename == "ilinau effretikon"
replace gdename = "altstaetten" if gdename == "altstaetten (sg)"
replace gdename = "bulle" if gdename == "bulle (fr)"
replace gdename = "burgdorf" if gdename == "burgdorf (be)"
replace gdename = "winterthur" if gdename == "dr., winterthur"
replace gdename = "huttwil" if gdename == "huttwil (be)"
replace gdename = "meggen" if gdename == "meggen (lu)"
replace gdename = "mies" if gdename == "mies (vd)"
replace gdename = "prilly" if gdename == "prilly (vd)"
replace gdename = "bern" if gdename == "prof. dr., bern"
replace gdename = "sion" if gdename == "sion (vs)"
replace gdename = "worb" if gdename == "wattenwil (worb)"
* October 2020
replace gdename = "wuerenlos" if gdename == "wrenlos"
replace gdename = "moeriken wildegg" if gdename == "mriken (ag)"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, maschinenfabrik liechti co ag, langnau i. e. (0.5)"
replace gdename = "saignelegier" if gdename == "saignelgier"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, voegeli moser ag, langnau i. e. (0.05)"
replace gdename = "anniviers" if gdename == ", mission/ayer"
replace gdename = "bolligen" if gdename == "bolligen be"
replace gdename = "gsteig" if gdename == "gsteig b. gstaad"
replace gdename = "baar" if gdename == "neuaegeri/baar"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, bigler ag, langnau i. e. (0.1)"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, pferdemetzgerei horisberger ag, aarau (0.05)"
replace gdename = "muestair" if gdename == "mstair"
replace gdename = "oetwil am see" if gdename == "oetwil a.s, gadola ag, oetwil a.s. (0.25)"
replace gdename = "wangen an der aare" if gdename == "wangen a. a, obrecht co ag, wangen (0.8)"
replace gdename = "oberhelfenschwil" if gdename == "oberhelfenschwiler"
replace gdename = "hausen am albis" if gdename == "hausen a. a."
replace gdename = "hausen am albis" if gdename == "hausen a. a, magnet ag, hausen a. a. (0.2)"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, gebr. wuthrich ag, langnau i. e. (0.15)"
replace gdename = "affoltern am albis" if gdename == "affoltern a.a, ernst tanner ag, affoltern a.a. (0.1)"
replace gdename = "unterbaech" if gdename == "unterbch"
replace gdename = "crans montana" if gdename == ", corin/montana"
replace gdename = "gossau (zh)" if gdename == "grueningerstr. 295, ottikon"
replace gdename = "schoenenberg (zh)" if gdename == "schnenberg (zh)"
replace gdename = "fluehli" if gdename == "soerenberg/fluehli"
replace gdename = "zollikon" if gdename == "zollikerberg/zollikon"
replace gdename = "thal" if gdename == "staad/thal"
replace gdename = "winterthur" if gdename == "winterthur iv"
replace gdename = "maur" if gdename == "maur ebmatingen"
replace gdename = "zuerich" if gdename == "george kempfstr. 25, 8oll zuerich"
replace gdename = "wil sg" if gdename == "rossrueti/bronschhofen sg"
replace gdename = "dietikon" if gdename == "dietikon zh"
replace gdename = "affoltern im emmental" if gdename == "affoltern i.e"
replace gdename = "minusio" if gdename == "locarno minusio"
replace gdename = "sierre" if gdename == "loc/sierre"
replace gdename = "heiden" if gdename == "waesseren, heiden ar"
...
replace gdename = "raperswilen" if gdename == "fischbach (tg)"
replace gdename = "kuesnacht (zh)" if gdename == "goldbach kuesnacht (zh)"
replace gdename = "langnau im emmental" if gdename == "langnau i. e, bosshart co ag, langnau i. e. (0.12)"
replace gdename = "muri bei bern" if gdename == "muri b. b, kaiser co ag, bern (1.2)"
replace gdename = "muri bei bern" if gdename == "muri b. b."
replace gdename = "muri bei bern" if gdename == "muri b. b"
replace gdename = "neuhausen am rheinfall" if gdename == "neuhausen a.r, zollgarage neuhausen ag, neuhausen a.r. (0.05)"
replace gdename = "freienbach" if gdename == "churerstr. 110, freienbach zh"
replace gdename = "duernten" if gdename == "heligeichstr. 8, tann rueti"
replace gdename = "duernten" if gdename == "tann rueti/duernten"
replace gdename = "sainte croix" if gdename == "auberson/ste croix"
replace gdename = "lohn ammannsegg" if gdename == "ammansegg"
replace gdename = "wynigen" if gdename == ", breitenegg/wynigen"

replace gdename = "kuesnacht" if gdename == "itschnach"
replace gdename = "bolligen" if gdename == "reckholtern"
replace gdename = "duedingen" if gdename == "jetschwil"
replace gdename = "collonge-bellerive" if gdename == "bellerive (ge)"
replace gdename = "duedingen" if gdename == "dudingen"
replace gdename = "kuesnacht" if gdename == "taegermoosstr. 20, 870g kuesnacht"
replace gdename = "pierrafortscha" if gdename == "u, granges sur marly"
replace gdename = "kuesnacht" if gdename == "it schnach"
replace gdename = "aeugst am albis" if gdename == "im park, auegst am albfis"
replace gdename = "baar" if gdename == "grundstrasse 29, 634b baar"
replace gdename = "freienbach" if gdename == "hurdenerstr. 38, hurden"
replace gdename = "binningen" if gdename == "alischwilerweg 25, 4.102 binningen"
replace gdename = "zuerich" if gdename == "dr. zuerich"
replace gdename = "villeneuve" if gdename == "byron 7, villenneuve"
replace gdename = "kilchberg" if gdename == "klichberg"
replace gdename = "carouge" if gdename == "r. marche 8, carouge"
replace gdename = "riehen" if gdename == "dr., riehen basel"
replace gdename = "koeniz" if gdename == "aarbergergasse 5. 3028 spiegel bei bern"
replace gdename = "collonge-bellerive" if gdename == "collonge bellerise"
replace gdename = "pully" if gdename == "pully lausanne"
replace gdename = "egnach" if gdename == "neukirch egnach"
replace gdename = "geneve" if gdename == "av. de champel 29 geneve"
replace gdename = "luins" if gdename == "les porrades, 1.184 luins"
replace gdename = "wattwil" if gdename == "schmidberg, ulisbach/wattwil"
replace gdename = "la chaux-de-fonds" if gdename == "la chaux de fonde"
replace gdename = "zuerich" if gdename == "zarich"
replace gdename = "freienbach" if gdename == "seedammstr. 60, hurden"
replace gdename = "wiesendangen" if gdename == "weissendangen"
replace gdename = "arlesheim" if gdename == "aresheim"
replace gdename = "duedingen" if gdename == "diidingen"
replace gdename = "kuesnacht" if gdename == "kuesnacht ziirich"
replace gdename = "kuesnacht" if gdename == "kii, nacht"
replace gdename = "kuesnacht" if gdename == "kuesnacht zuerich"
replace gdename = "zuerich" if gdename == "widmerstr. 27, 80, 2s zuerich"
replace gdename = "zuerich" if gdename == "scheideggstr. 73, zurich"
replace gdename = "zuerich" if gdename == "bluemlisalpstr. 31, 8oll6 zuerich"
replace gdename = "collonge-bellerive" if gdename == "collonge belleeive"
replace gdename = "kuesnacht" if gdename == "ziirich/kuesnacht"
replace gdename = "kuesnacht" if gdename == "kuesnacht (zeh.)"
replace gdename = "zuerich" if gdename == "zuerch"
...
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90c.dta", replace
append using "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90b.dta"
save "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90_noPLZ.dta", replace

use "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90_noPLZ.dta", clear
...
* Jan 2020
replace nCH = 1 if gdename == "dubai are"
replace nCH = 1 if gdename == "koweit city kw"
replace nCH = 1 if gdename == "schwalbach d"
replace nCH = 1 if gdename == "grasse (f)"
replace nCH = 1 if gdename == "korntal dl"
replace nCH = 1 if gdename == "linselles f"
replace nCH = 1 if gdename == "lage d"
replace nCH = 1 if gdename == "dautphetal d"
replace nCH = 1 if gdename == "bry sur marne f"
replace nCH = 1 if gdename == "baindt d"
replace nCH = 1 if gdename == "fort lee usa"
replace nCH = 1 if gdename == "standford (usa)"
replace nCH = 1 if gdename == "salzgitter d"
replace nCH = 1 if gdename == "wheatland, wyoming usa"
replace nCH = 1 if gdename == "the woodlands/texas usa"
replace nCH = 1 if gdename == "leatherhead (gb)"
replace nCH = 1 if gdename == "doom nl"
replace nCH = 1 if gdename == "saint avertin f"
replace nCH = 1 if gdename == "plaisir f"
replace nCH = 1 if gdename == "comstock park michigan usa"
replace nCH = 1 if gdename == "madras/oregon usa"
replace nCH = 1 if gdename == "roseville aus"
replace nCH = 1 if gdename == "kingston upon thames (surrey) gb"
replace nCH = 1 if gdename == "johannesburg sandtom za"
replace nCH = 1 if gdename == "modesto ca usa"
replace nCH = 1 if gdename == "moskau r"
replace nCH = 1 if gdename == "vadstena s"
replace nCH = 1 if gdename == "boissejour f"
replace nCH = 1 if gdename == "spanga s"
replace nCH = 1 if gdename == "borgo priolo i"
replace nCH = 1 if gdename == "camberley/surrey gb"
replace nCH = 1 if gdename == "surbiton gb"
replace nCH = 1 if gdename == "sommerville, new jersey usa"
replace nCH = 1 if gdename == "riyad sau"
replace nCH = 1 if gdename == "markdorf"
replace nCH = 1 if gdename == "hungen d"
replace nCH = 1 if gdename == "beinasco i"
replace nCH = 1 if gdename == "engen d"
replace nCH = 1 if gdename == "istanbul (tuerkei)"
replace nCH = 1 if gdename == "bologna/i"
replace nCH = 1 if gdename == "edina usa"
replace nCH = 1 if gdename == "kfar shmaryaha il"
replace nCH = 1 if gdename == "kaltenleutgeben a"
replace nCH = 1 if gdename == "tacoma wa usa"
replace nCH = 1 if gdename == "banchette i"
replace nCH = 1 if gdename == "penistone gb"
replace nCH = 1 if gdename == "haverhill gb"
replace nCH = 1 if gdename == "amtshausg. 1/drahtseilbahn"
replace nCH = 1 if gdename == "froendenberg (d)"
replace nCH = 1 if gdename == "bagsvaerd dk"
replace nCH = 1 if gdename == "nagano (j)"
replace nCH = 1 if gdename == "celle st cloud (f)"
replace nCH = 1 if gdename == "delft nl"
replace nCH = 1 if gdename == "wynwood usa"
replace nCH = 1 if gdename == "altenkirchen (d)"
replace nCH = 1 if gdename == "alexandrie et"
replace nCH = 1 if gdename == "hattersheim am main d"
replace nCH = 1 if gdename == "bendorf d"
replace nCH = 1 if gdename == "burnham gb"
replace nCH = 1 if gdename == "karlskoga s"
replace nCH = 1 if gdename == "oud beyerland nl"
replace nCH = 1 if gdename == "cheshire (gb)"
replace nCH = 1 if gdename == "amerika"
replace nCH = 1 if gdename == "expoo (finnland)"
replace nCH = 1 if gdename == "helsingoer dk"
replace nCH = 1 if gdename == "nordwalde d"
replace nCH = 1 if gdename == "scheyern d"
replace nCH = 1 if gdename == "bad homburg v.d. hoehe (d)"
replace nCH = 1 if gdename == "buchenbach/breisgau"
replace nCH = 1 if gdename == "st. sampson s (guernsey, channel"
replace nCH = 1 if gdename == "bad hersfeld (d)"
replace nCH = 1 if gdename == "toulon (f)"
replace nCH = 1 if gdename == "oxshott gb"
replace nCH = 1 if gdename == "valtournenche i"
replace nCH = 1 if gdename == "keerbergen (b)"
replace nCH = 1 if gdename == "utrecht (nl)"
replace nCH = 1 if gdename == "milo i"
replace nCH = 1 if gdename == "walton on thames (gb)"
replace nCH = 1 if gdename == "new york, new york, usa"
replace nCH = 1 if gdename == "velbert (d)"
replace nCH = 1 if gdename == "deux montagnes/quebec cdn"
replace nCH = 1 if gdename == "vlotho d"
replace nCH = 1 if gdename == "bad schwartau d"
replace nCH = 1 if gdename == "buckingham gb"
replace nCH = 1 if gdename == "schelklingen d"
replace nCH = 1 if gdename == "burladingen (d)"
replace nCH = 1 if gdename == "samut sakhon (thailand)"
replace nCH = 1 if gdename == "beinstein (d)"
replace nCH = 1 if gdename == "tortola (bvi)"
replace nCH = 1 if gdename == "springfield, adelaide, australia"
replace nCH = 1 if gdename == "hemet (usa)"
replace nCH = 1 if gdename == "blaricum"
replace nCH = 1 if gdename == "trevose usa"
replace nCH = 1 if gdename == "heigenbruecken d"
replace nCH = 1 if gdename == "roslyn usa"
replace nCH = 1 if gdename == "ljubliana"
replace nCH = 1 if gdename == "doetinchem nl"
replace nCH = 1 if gdename == "hingham ma usa"
replace nCH = 1 if gdename == "saint germain en laye (f)"
replace nCH = 1 if gdename == "graz kroisbach (a)"
replace nCH = 1 if gdename == "cottesloe aus"
replace nCH = 1 if gdename == "andernach (d)"
replace nCH = 1 if gdename == "carrara i"
replace nCH = 1 if gdename == "sas van gent nl"
replace nCH = 1 if gdename == "rosheim f"
replace nCH = 1 if gdename == "belmont usa"
replace nCH = 1 if gdename == "ra anana il"
replace nCH = 1 if gdename == "warstein belecke d"
replace nCH = 1 if gdename == "al khobar (sa)"
replace nCH = 1 if gdename == "san francisco california usa"
replace nCH = 1 if gdename == "moscow rus"
replace nCH = 1 if gdename == "unterensingen d"
replace nCH = 1 if gdename == "tolland ct usa"
replace nCH = 1 if gdename == "occhieppo i"
replace nCH = 1 if gdename == "essen/d"
replace nCH = 1 if gdename == "saeroe"
replace nCH = 1 if gdename == "helsinki (fin)"
replace nCH = 1 if gdename == "munchen d"
replace nCH = 1 if gdename == "loerrach/d"
replace nCH = 1 if gdename == "le fay f"
replace nCH = 1 if gdename == "awali brn"
replace nCH = 1 if gdename == "venterol, drome f"
replace nCH = 1 if gdename == "panana pa"
replace nCH = 1 if gdename == "saint valery en caux f"
replace nCH = 1 if gdename == "st. maur f"
replace nCH = 1 if gdename == "vesinet (f)"
replace nCH = 1 if gdename == "libreville gab"
replace nCH = 1 if gdename == "svenborg dk"
replace nCH = 1 if gdename == "erbendorf d"
replace nCH = 1 if gdename == "panama city"
replace nCH = 1 if gdename == "ea paylacura ra"
replace nCH = 1 if gdename == "rosengarten d"
replace nCH = 1 if gdename == "horben d"
replace nCH = 1 if gdename == "neully sur seine f"
replace nCH = 1 if gdename == "vizinada/ohnici kroatien"
replace nCH = 1 if gdename == "dormans park, west sussex gb"
replace nCH = 1 if gdename == "kairo(et)"
replace nCH = 1 if gdename == "chatelineau b"
replace nCH = 1 if gdename == "meerbusch"
replace nCH = 1 if gdename == "narrabre nsw aus"
replace nCH = 1 if gdename == "furlong, pennsylvania usa"
replace nCH = 1 if gdename == "hagen (d)"
replace nCH = 1 if gdename == "jesteburg (d)"
replace nCH = 1 if gdename == "speicher d"
replace nCH = 1 if gdename == "caracas/venezuela"
replace nCH = 1 if gdename == "edinburgh (gb)"
replace nCH = 1 if gdename == "whippany, ny usa"
replace nCH = 1 if gdename == "lisbonne f"
replace nCH = 1 if gdename == "vesoul f"
replace nCH = 1 if gdename == "grating d"
replace nCH = 1 if gdename == "minzier f"
replace nCH = 1 if gdename == "fallersleben d"
replace nCH = 1 if gdename == "belfeld nl"
replace nCH = 1 if gdename == "foehren d"
replace nCH = 1 if gdename == "salt lake city, utah usa"
replace nCH = 1 if gdename == "wiesbaden/d"
replace nCH = 1 if gdename == "nassau, bahamas"
replace nCH = 1 if gdename == "rhenen nl"
replace nCH = 1 if gdename == "montelier f"
replace nCH = 1 if gdename == "dammam (sa)"
replace nCH = 1 if gdename == "noerdlingen d"
replace nCH = 1 if gdename == "cobar (aus)"
replace nCH = 1 if gdename == "laguna hills usa"
replace nCH = 1 if gdename == "toorak victoria aus"
replace nCH = 1 if gdename == "stafelstein d"
replace nCH = 1 if gdename == "rhode saint genese (b)"
replace nCH = 1 if gdename == "nifern oeschelborn d"
replace nCH = 1 if gdename == "the woodlands us"
replace nCH = 1 if gdename == "friedberg (d)"
replace nCH = 1 if gdename == "gentofte (daenemark)"
replace nCH = 1 if gdename == "merion, pennsylvania usa"
replace nCH = 1 if gdename == "garsington gb"
replace nCH = 1 if gdename == "st julien f"
replace nCH = 1 if gdename == "schwandenstadt a"
replace nCH = 1 if gdename == "altenbach d"
replace nCH = 1 if gdename == "chiba japan"
replace nCH = 1 if gdename == "east hanover, new jersey usa"
replace nCH = 1 if gdename == "besancon/france"
replace nCH = 1 if gdename == "schwanewede (d)"
replace nCH = 1 if gdename == "colvilha p"
replace nCH = 1 if gdename == "beyrouth (liban)"
replace nCH = 1 if gdename == "verona (i)"
replace nCH = 1 if gdename == "east grinstead (gb)"
replace nCH = 1 if gdename == "chevry f"
replace nCH = 1 if gdename == "luedenach (d)"
replace nCH = 1 if gdename == "treuchtlingen d"
replace nCH = 1 if gdename == "beuningen nl"
replace nCH = 1 if gdename == "bedford hills, new york usa"
replace nCH = 1 if gdename == "trumbull/conn. usa"
replace nCH = 1 if gdename == "londra"
replace nCH = 1 if gdename == "shanghai (rc)"
replace nCH = 1 if gdename == "culoz (f)"
replace nCH = 1 if gdename == "haifa (israel)"
replace nCH = 1 if gdename == "wycombe gb"
replace nCH = 1 if gdename == "oberwil zg"
replace nCH = 1 if gdename == "moss n"
replace nCH = 1 if gdename == "verden/aller d"
replace nCH = 1 if gdename == "meadowfield bradford gb"
replace nCH = 1 if gdename == "leipzig (d)"
replace nCH = 1 if gdename == "ljubliana (yu)"
replace nCH = 1 if gdename == "bishkek kg"
replace nCH = 1 if gdename == "le mesnil saint denis f"
replace nCH = 1 if gdename == "enzersdorf a"
replace nCH = 1 if gdename == "la haye"
replace nCH = 1 if gdename == "maisons laffitte/yvelines f"
replace nCH = 1 if gdename == "lahti (sf)"
replace nCH = 1 if gdename == "verona, new jersey usa"
replace nCH = 1 if gdename == "wessling d"
replace nCH = 1 if gdename == "weybridge/surrey gb"
replace nCH = 1 if gdename == "lincoln mass. usa"
replace nCH = 1 if gdename == "mahlberg d"
replace nCH = 1 if gdename == "varsovie pl"
replace nCH = 1 if gdename == "la varenne st hilaire f"
replace nCH = 1 if gdename == "sechelt, british columbia cdn"
replace nCH = 1 if gdename == "pesaro i"
replace nCH = 1 if gdename == "noerresundby dk"
replace nCH = 1 if gdename == "zimmern d"
replace nCH = 1 if gdename == "hamilton, bermuda usa"
replace nCH = 1 if gdename == "porte s. saone f"
replace nCH = 1 if gdename == "parsippany, new jersey usa"
replace nCH = 1 if gdename == "dorridge, west midlands (gb)"
replace nCH = 1 if gdename == "munsbach l"
replace nCH = 1 if gdename == "lintgen l"
replace nCH = 1 if gdename == "taufkirchen (d)"
replace nCH = 1 if gdename == "intimiano i"
replace nCH = 1 if gdename == "laufen be"
replace nCH = 1 if gdename == "rocquencourt f"
replace nCH = 1 if gdename == "wadhurst gb"
replace nCH = 1 if gdename == "wansford/peterborough gb"
replace nCH = 1 if gdename == "blankenloch buechig d"
replace nCH = 1 if gdename == "kawasaki j"
replace nCH = 1 if gdename == "russey f"
replace nCH = 1 if gdename == "pulborough, west sussex gb"
replace nCH = 1 if gdename == "perris, calif. usa"
replace nCH = 1 if gdename == "schliersee d"
replace nCH = 1 if gdename == "evian les bains (f)"
replace nCH = 1 if gdename == "prag lipence cz"
replace nCH = 1 if gdename == "berechem, antwerpen (b)"
replace nCH = 1 if gdename == "saint martin d heres f"
replace nCH = 1 if gdename == "brielle nl"
replace nCH = 1 if gdename == "elkhart usa"
replace nCH = 1 if gdename == "hanau dl"
replace nCH = 1 if gdename == "ladenburg (d)"
replace nCH = 1 if gdename == "prad a. stilfersjoch i"
replace nCH = 1 if gdename == "neuhausen auf den fildern d"
replace nCH = 1 if gdename == "heerlen nl"
replace nCH = 1 if gdename == "lemberg d"
replace nCH = 1 if gdename == "kulloby sf"
replace nCH = 1 if gdename == "combloux f"
replace nCH = 1 if gdename == "benissa e"
replace nCH = 1 if gdename == "williamsdale aus"
replace nCH = 1 if gdename == "solikamsk gus"
replace nCH = 1 if gdename == "petaluna usa"
replace nCH = 1 if gdename == "seattle can"
replace nCH = 1 if gdename == "carlisle, massachusetts usa"
replace nCH = 1 if gdename == "california"
replace nCH = 1 if gdename == "glasgow (gb)"
replace nCH = 1 if gdename == "les fins morteau f"
replace nCH = 1 if gdename == "lauf d"
replace nCH = 1 if gdename == "mesnil le roi f"
replace nCH = 1 if gdename == "kuesserberg d"
replace nCH = 1 if gdename == "fontanafredda i"
replace nCH = 1 if gdename == "le mesnil le roi f"
replace nCH = 1 if gdename == "encino, kalifornien usa"
replace nCH = 1 if gdename == "edimbourg gb"
replace nCH = 1 if gdename == "tigliole i"
replace nCH = 1 if gdename == "schwaebisch gmuend (d)"
replace nCH = 1 if gdename == "sark (channel island) gb"
replace nCH = 1 if gdename == "san lazzaro i"
replace nCH = 1 if gdename == "aguilas/murcia e"
replace nCH = 1 if gdename == "st. georges, bermuda"
replace nCH = 1 if gdename == "varsovie"
replace nCH = 1 if gdename == "buckinghamshire (gb)"
replace nCH = 1 if gdename == "leutkirch (d)"
replace nCH = 1 if gdename == "sarnico i"
replace nCH = 1 if gdename == "muscatine, iowa usa"
replace nCH = 1 if gdename == "oerebro (s)"
replace nCH = 1 if gdename == "meulebeke b"
replace nCH = 1 if gdename == "alland a"
replace nCH = 1 if gdename == "salvador (br)"
replace nCH = 1 if gdename == "hueckeswagen (d)"
replace nCH = 1 if gdename == "la honda (california) usa"
replace nCH = 1 if gdename == "alkhobar sa"
replace nCH = 1 if gdename == "les marches f"
replace nCH = 1 if gdename == "conneq usa"
replace nCH = 1 if gdename == "irvine, cal. usa"
replace nCH = 1 if gdename == "kandernholzen d"
replace nCH = 1 if gdename == "neuilly seine f"
replace nCH = 1 if gdename == "planegg d"
replace nCH = 1 if gdename == "rexdale/ontario cdn"
replace nCH = 1 if gdename == "st remy en provence f"
replace nCH = 1 if gdename == "lincoln nebr./usa"
replace nCH = 1 if gdename == "ueberlingen d"
replace nCH = 1 if gdename == "irvine (usa)"
replace nCH = 1 if gdename == "hauppauge (usa)"
replace nCH = 1 if gdename == "roslyn, new york, usa"
replace nCH = 1 if gdename == "rickmansworth"
replace nCH = 1 if gdename == "riverside, connecticut usa"
replace nCH = 1 if gdename == "feucht d"
replace nCH = 1 if gdename == "magaliesburg za"
replace nCH = 1 if gdename == "germantown tn usa"
replace nCH = 1 if gdename == "driebergen rijsenburg nl"
replace nCH = 1 if gdename == "del mar, californien usa"
replace nCH = 1 if gdename == "vogt d"
replace nCH = 1 if gdename == "rishon lezin il"
replace nCH = 1 if gdename == "burgthann d"
replace nCH = 1 if gdename == "yozgat tr"
replace nCH = 1 if gdename == "imperia i"
replace nCH = 1 if gdename == "berentzwiller f"
replace nCH = 1 if gdename == "seekirchen a"
replace nCH = 1 if gdename == "struer (dk)"
replace nCH = 1 if gdename == "saint bardoux f"
replace nCH = 1 if gdename == "korntal (d)"
replace nCH = 1 if gdename == "aschau am inn d"
replace nCH = 1 if gdename == "brehan f"
replace nCH = 1 if gdename == "auburn wa usa"
replace nCH = 1 if gdename == "berchem, antwerpen b"
replace nCH = 1 if gdename == "arthaz pont notre dame f"
replace nCH = 1 if gdename == "douglas (isle of man)"
replace nCH = 1 if gdename == "aggsbach a"
replace nCH = 1 if gdename == "copenhagen dk"
replace nCH = 1 if gdename == "anthy f"
replace nCH = 1 if gdename == "monacao mc"
replace nCH = 1 if gdename == "berchem (b)"
replace nCH = 1 if gdename == "brecht b"
replace nCH = 1 if gdename == "lanzarotte kanar. inseln"
replace nCH = 1 if gdename == "coral harbour, new providence usa"
replace nCH = 1 if gdename == "mainate i"
replace nCH = 1 if gdename == "tornoto ca"
replace nCH = 1 if gdename == "gruenstadt d"
replace nCH = 1 if gdename == "st. germain du plain f"
replace nCH = 1 if gdename == "barbaresco cb i"
replace nCH = 1 if gdename == "heidenheim an der brenz d"
replace nCH = 1 if gdename == "sollentuna (s)"
replace nCH = 1 if gdename == "solihull gb"
replace nCH = 1 if gdename == "hamilton usa"
replace nCH = 1 if gdename == "playa de aro (e)"
replace nCH = 1 if gdename == "berneck d"
replace nCH = 1 if gdename == "wetter/ruhr brd"
replace nCH = 1 if gdename == "amman jor"
replace nCH = 1 if gdename == "johannesbourg"
replace nCH = 1 if gdename == "salzhausen d"
replace nCH = 1 if gdename == "winchester hampshire gb"
replace nCH = 1 if gdename == "sausalito (usa)"
replace nCH = 1 if gdename == "york usa"
replace nCH = 1 if gdename == "djeddah a.s"
replace nCH = 1 if gdename == "giesshuebel a"
replace nCH = 1 if gdename == "grindsted dk"
replace nCH = 1 if gdename == "tegelen nl"
replace nCH = 1 if gdename == "gatto i"
replace nCH = 1 if gdename == "ronnenberg benthe d"
replace nCH = 1 if gdename == "struer d"
replace nCH = 1 if gdename == "brannenburg d"
replace nCH = 1 if gdename == "corona dal mar/calif. usa"
replace nCH = 1 if gdename == "cobham, surrey (gb)"
replace nCH = 1 if gdename == "carrollton (usa)"
replace nCH = 1 if gdename == "bornheim waldberg d"
replace nCH = 1 if gdename == "sceaux/f"
replace nCH = 1 if gdename == "moscou gus"
replace nCH = 1 if gdename == "burladingen d"
replace nCH = 1 if gdename == "mc adenville usa"
replace nCH = 1 if gdename == "bourg en bresse (f)"
replace nCH = 1 if gdename == "prambachkirchen a"
replace nCH = 1 if gdename == "st genese (b)"
replace nCH = 1 if gdename == "king city, ontario cdn"
replace nCH = 1 if gdename == "bahlingen am kaiserstuhl d"
replace nCH = 1 if gdename == "brooklyn usa"
replace nCH = 1 if gdename == "hjaelteby s"
replace nCH = 1 if gdename == "millbrook usa"
replace nCH = 1 if gdename == "oberhausen (d)"
replace nCH = 1 if gdename == "cesena i"
replace nCH = 1 if gdename == "francfort d"
replace nCH = 1 if gdename == "corralejo, fuerteventura e"
replace nCH = 1 if gdename == "djursholm (schweden)"
replace nCH = 1 if gdename == "chelmsford gb"
replace nCH = 1 if gdename == "nieder olm d"
replace nCH = 1 if gdename == "neunkirchen saar d"
replace nCH = 1 if gdename == "cookham dean (gb)"
replace nCH = 1 if gdename == "gaienhofen horn d"
replace nCH = 1 if gdename == "thornton health gb"
replace nCH = 1 if gdename == "bronxville (usa)"
replace nCH = 1 if gdename == "alpharetta (georgia/usa)"
replace nCH = 1 if gdename == "huddersfield gb"
replace nCH = 1 if gdename == "kamakura j"
replace nCH = 1 if gdename == "nassau bhs"
replace nCH = 1 if gdename == "westmount, quebec cdn"
replace nCH = 1 if gdename == "carpaneto i"
replace nCH = 1 if gdename == "winchester, ham. (gb)"
replace nCH = 1 if gdename == "rameldange l"
replace nCH = 1 if gdename == "koenigheim d"
replace nCH = 1 if gdename == "nara city j"
replace nCH = 1 if gdename == "shannonbrook (aus)"
replace nCH = 1 if gdename == "st rhemy en bosses i"
replace nCH = 1 if gdename == "chalfont st. giles (gb)"
replace nCH = 1 if gdename == "fort worth texas usa"
replace nCH = 1 if gdename == "arizona"
replace nCH = 1 if gdename == "glumsloev (s)"
replace nCH = 1 if gdename == "annapolis maryland usa"
replace nCH = 1 if gdename == "bromley, kend gb"
replace nCH = 1 if gdename == "ventura usa"
replace nCH = 1 if gdename == "kappelen b"
replace nCH = 1 if gdename == "lipetsk gus"
replace nCH = 1 if gdename == "campinas br"
replace nCH = 1 if gdename == "muspach le haut f"
replace nCH = 1 if gdename == "palma de majorca (e)"
replace nCH = 1 if gdename == "karlstad/s"
replace nCH = 1 if gdename == "orangenstr. 625, new haven ct"
replace nCH = 1 if gdename == "quinto di treviso i"
replace nCH = 1 if gdename == "camin padova i"
replace nCH = 1 if gdename == "bascharage l"
replace nCH = 1 if gdename == "bloomingdale usa"
replace nCH = 1 if gdename == "oelde stromberg d"
replace nCH = 1 if gdename == "cranbrook gb"
replace nCH = 1 if gdename == "harpenden (gb)"
replace nCH = 1 if gdename == "livingstone usa"
replace nCH = 1 if gdename == "tokyo jpn"
replace nCH = 1 if gdename == "espiche lagos b"
replace nCH = 1 if gdename == "st. john (jersey)"
replace nCH = 1 if gdename == "maskat oman"
replace nCH = 1 if gdename == "new yerne, n.j. usa"
replace nCH = 1 if gdename == "saint pierre en faucigny f"
replace nCH = 1 if gdename == "dromineer, tipperary irl"
replace nCH = 1 if gdename == "montevideo (u)"
replace nCH = 1 if gdename == "calw hirsau (d)"
replace nCH = 1 if gdename == "egelsbach d"
replace nCH = 1 if gdename == "alpirsbach d"
replace nCH = 1 if gdename == "valence e"
replace nCH = 1 if gdename == "brough welton gb"
replace nCH = 1 if gdename == "champigny sur marne f"
replace nCH = 1 if gdename == "marienfeld d"
replace nCH = 1 if gdename == "villeneuve d arca f"
replace nCH = 1 if gdename == "belfaux f"
replace nCH = 1 if gdename == "jabeke b"
replace nCH = 1 if gdename == "avigliana i"
replace nCH = 1 if gdename == "garden city usa"
replace nCH = 1 if gdename == "mittersill (a)"
replace nCH = 1 if gdename == "deilingen d"
replace nCH = 1 if gdename == "orpington gb"
replace nCH = 1 if gdename == "wheathampstead gb"
replace nCH = 1 if gdename == "gruendau d"
replace nCH = 1 if gdename == "boulouris f"
replace nCH = 1 if gdename == "elsenheim f"
replace nCH = 1 if gdename == "attenschwiler, haut rhin f"
replace nCH = 1 if gdename == "bahlingen d"
replace nCH = 1 if gdename == "loegstoer dk"
replace nCH = 1 if gdename == "kandern (d)"
replace nCH = 1 if gdename == "evanston, illinois (usa)"
replace nCH = 1 if gdename == "chamalieres f"
replace nCH = 1 if gdename == "dubai, ver. arab. emirate"
replace nCH = 1 if gdename == "mountain lakes nj usa"
replace nCH = 1 if gdename == "cobar aus"
replace nCH = 1 if gdename == "redhouse usa"
replace nCH = 1 if gdename == "koenigstein/taunus (d)"
replace nCH = 1 if gdename == "winnenden (d)"
replace nCH = 1 if gdename == "weinsberg d"
replace nCH = 1 if gdename == "montigny en ostrevent f"
replace nCH = 1 if gdename == "gruenberg d"
replace nCH = 1 if gdename == "brugnasco/airolo"
replace nCH = 1 if gdename == "cagnes sur mer f"
replace nCH = 1 if gdename == "evesham gb"
replace nCH = 1 if gdename == "erlensee d"
replace nCH = 1 if gdename == "oberammergau d"
replace nCH = 1 if gdename == "occieppo inferiore i"
replace nCH = 1 if gdename == "koeln brd"
replace nCH = 1 if gdename == "traubing d"
replace nCH = 1 if gdename == "chambery france"
replace nCH = 1 if gdename == "mainz/d"
replace nCH = 1 if gdename == "bad neuheim d"
replace nCH = 1 if gdename == "greensboro, nc usa"
replace nCH = 1 if gdename == "oer erkenschwick brd"
replace nCH = 1 if gdename == "harelbeke b"
replace nCH = 1 if gdename == "dunningen d"
replace nCH = 1 if gdename == "fairfield (usa)"
replace nCH = 1 if gdename == "chens le pont (f)"
replace nCH = 1 if gdename == "rankweil (a)"
replace nCH = 1 if gdename == "wildbad d"
replace nCH = 1 if gdename == "springe d"
replace nCH = 1 if gdename == "wittenbach sg"
replace nCH = 1 if gdename == "kane country (illinois) usa"
replace nCH = 1 if gdename == "preetz d"
replace nCH = 1 if gdename == "boskoop (nl)"
replace nCH = 1 if gdename == "midland park usa"
replace nCH = 1 if gdename == "schmallenberg d"
replace nCH = 1 if gdename == "belvaux l"
replace nCH = 1 if gdename == "postbauer d"
replace nCH = 1 if gdename == "contern l"
replace nCH = 1 if gdename == "aspen usa"
replace nCH = 1 if gdename == "avon connecticut usa"
replace nCH = 1 if gdename == "riudarenas e"
replace nCH = 1 if gdename == "wilrijk, anvers b"
replace nCH = 1 if gdename == "ollerton, knutsford cheshire gb"
replace nCH = 1 if gdename == "zagabria"
replace nCH = 1 if gdename == "mclean (virginia. usa)"
replace nCH = 1 if gdename == "portland or usa"
replace nCH = 1 if gdename == "helmond (nl)"
replace nCH = 1 if gdename == "oberursel"
replace nCH = 1 if gdename == "chesieres sur ollon"
replace nCH = 1 if gdename == "colchester (gb)"
replace nCH = 1 if gdename == "conakry usa"
replace nCH = 1 if gdename == "hesperange l"
replace nCH = 1 if gdename == "tai po (hongkong)"
replace nCH = 1 if gdename == "huettenberg d"
replace nCH = 1 if gdename == "rte lac, guevaux"
replace nCH = 1 if gdename == "chobham gb"
replace nCH = 1 if gdename == "strassbourg (f)"
replace nCH = 1 if gdename == "mommenheim d"
replace nCH = 1 if gdename == "savona i"
replace nCH = 1 if gdename == "genemuiden (nl)"
replace nCH = 1 if gdename == "wollbach (d)"
replace nCH = 1 if gdename == "st. kruis nl"
replace nCH = 1 if gdename == "perm csi"
replace nCH = 1 if gdename == "old saybrok ct usa"
replace nCH = 1 if gdename == "nutley gb"
replace nCH = 1 if gdename == "san remo i"
replace nCH = 1 if gdename == "congleton (cheshir.england)"
replace nCH = 1 if gdename == "nogent sur marne"
replace nCH = 1 if gdename == "mendham, new jersey usa"
replace nCH = 1 if gdename == "cellieu (f)"
replace nCH = 1 if gdename == "lnzlingen d"
replace nCH = 1 if gdename == "nitra cs"
replace nCH = 1 if gdename == "yelverton"
replace nCH = 1 if gdename == "aschabad udssr"
replace nCH = 1 if gdename == "mellrichtstadt d"
replace nCH = 1 if gdename == "athis mons f"
replace nCH = 1 if gdename == "mannswoerth (a)"
replace nCH = 1 if gdename == "cork irl"
replace nCH = 1 if gdename == "huninge f"
replace nCH = 1 if gdename == "repentigny, quebec cdn"
replace nCH = 1 if gdename == "stephanskirchen (d)"
replace nCH = 1 if gdename == "bletterens f"
replace nCH = 1 if gdename == "jenkintown, pa usa"
replace nCH = 1 if gdename == "gaborone rb"
replace nCH = 1 if gdename == "retzstadt (d)"
replace nCH = 1 if gdename == "lyaud f"
replace nCH = 1 if gdename == "brown candover gb"
replace nCH = 1 if gdename == "sarnia, ontario cdn"
replace nCH = 1 if gdename == "sanary sur mer f"
replace nCH = 1 if gdename == "hampton ny, usa"
replace nCH = 1 if gdename == "st. brelade (jersey)"
replace nCH = 1 if gdename == "montgeron f"
replace nCH = 1 if gdename == "warrington gb"
replace nCH = 1 if gdename == "riedisheim (f)"
replace nCH = 1 if gdename == "baltimore (usa)"
replace nCH = 1 if gdename == "beacon hill gb"
replace nCH = 1 if gdename == "barcas e"
replace nCH = 1 if gdename == "glasgow gb"
replace nCH = 1 if gdename == "roma/i"
replace nCH = 1 if gdename == "vacoas ms"
replace nCH = 1 if gdename == "schio i"
replace nCH = 1 if gdename == "oxted (gb)"
replace nCH = 1 if gdename == "la trinite f"
replace nCH = 1 if gdename == "kenton gb"
replace nCH = 1 if gdename == "riverside ca usa"
replace nCH = 1 if gdename == "monrovia (lb)"
replace nCH = 1 if gdename == "engelwood, n.j. (usa)"
replace nCH = 1 if gdename == "wakefield gb"
replace nCH = 1 if gdename == "hartsdale ny, usa"
replace nCH = 1 if gdename == "oisterwik nl"
replace nCH = 1 if gdename == "lb oss nl"
replace nCH = 1 if gdename == "san jose (cr)"
replace nCH = 1 if gdename == "beijing"
replace nCH = 1 if gdename == "novare i"
replace nCH = 1 if gdename == "djakarta j"
replace nCH = 1 if gdename == "spenni, triesenberg fl"
replace nCH = 1 if gdename == "chateau beauregard f"
replace nCH = 1 if gdename == "st thibault des vignes f"
replace nCH = 1 if gdename == "san francisco ca usa"
replace nCH = 1 if gdename == "castelfranco, veneto i"
replace nCH = 1 if gdename == "sancourt f"
replace nCH = 1 if gdename == "oakmont usa"
replace nCH = 1 if gdename == "ochsenfurt d"
replace nCH = 1 if gdename == "greenwich (connecticut usa)"
replace nCH = 1 if gdename == "libertyville, illinois usa"
replace nCH = 1 if gdename == "canavieras br"
replace nCH = 1 if gdename == "helensburgh (gb)"
replace nCH = 1 if gdename == "maidenhead berkshire gb"
replace nCH = 1 if gdename == "beaconsfield, quebec cdn"
replace nCH = 1 if gdename == "baltimore, maryland usa"
replace nCH = 1 if gdename == "anzegem b"
replace nCH = 1 if gdename == "senningen l"
replace nCH = 1 if gdename == "sao paolo, brasile"
replace nCH = 1 if gdename == "joenkoeping s"
replace nCH = 1 if gdename == "la gaude f"
replace nCH = 1 if gdename == "menlo oak drive 910, monto park usa"
replace nCH = 1 if gdename == "oostkamp (b)"
replace nCH = 1 if gdename == "ridgmont gb"
replace nCH = 1 if gdename == "outwood gb"
replace nCH = 1 if gdename == "clinton usa"
replace nCH = 1 if gdename == "hellwege d"
replace nCH = 1 if gdename == "erlangen"
replace nCH = 1 if gdename == "asken n"
replace nCH = 1 if gdename == "valencia, venezuela"
replace nCH = 1 if gdename == "idastein d"
replace nCH = 1 if gdename == "petershausen d"
replace nCH = 1 if gdename == "seefeld d"
replace nCH = 1 if gdename == "bergamo 1"
replace nCH = 1 if gdename == "poissy f"
replace nCH = 1 if gdename == "heusden zolder b"
replace nCH = 1 if gdename == "hartenfels d"
replace nCH = 1 if gdename == "bourg en bresse f"
replace nCH = 1 if gdename == "pappinbarra aus"
replace nCH = 1 if gdename == "hierden (nl)"
replace nCH = 1 if gdename == "barrington usa"
replace nCH = 1 if gdename == "gross biberau d"
replace nCH = 1 if gdename == "weston, connecticut usa"
replace nCH = 1 if gdename == "whitchurch, hampshire gb"
replace nCH = 1 if gdename == "wermeiskirchen d"
replace nCH = 1 if gdename == "mirmande f"
replace nCH = 1 if gdename == "stuhr d"
replace nCH = 1 if gdename == "treviglio i"
replace nCH = 1 if gdename == "beaufort l"
replace nCH = 1 if gdename == "granada e"
replace nCH = 1 if gdename == "toronto kanada"
replace nCH = 1 if gdename == "gernsbach d"
replace nCH = 1 if gdename == "anspach d"
replace nCH = 1 if gdename == "ah deurne nl"
replace nCH = 1 if gdename == "luxenbourg l"
replace nCH = 1 if gdename == "glashuetten (d)"
replace nCH = 1 if gdename == "st louis de la chaussee (f)"
replace nCH = 1 if gdename == "chatsworth, los angeles usa"
replace nCH = 1 if gdename == "st.niklausen/horw"
replace nCH = 1 if gdename == "st. paul de vence f"
replace nCH = 1 if gdename == "zwijndrecht"
replace nCH = 1 if gdename == "jarfalla s"
replace nCH = 1 if gdename == "ahlen dolberg d"
replace nCH = 1 if gdename == "barengo i"
replace nCH = 1 if gdename == "schoenenberg a.d.t"
replace nCH = 1 if gdename == "lezignan corbieres f"
replace nCH = 1 if gdename == "hofheim am taunus d"
replace nCH = 1 if gdename == "beverly hills (usa)"
replace nCH = 1 if gdename == "issy l eveque f"
replace nCH = 1 if gdename == "bedford ny usa"
replace nCH = 1 if gdename == "schoten (b)"
replace nCH = 1 if gdename == "fraucheville f"
replace nCH = 1 if gdename == "woudenberg nl"
replace nCH = 1 if gdename == "el congrejo pa"
replace nCH = 1 if gdename == "salavat russland"
replace nCH = 1 if gdename == "lmmenstaad d"
replace nCH = 1 if gdename == "la berthenoux f"
replace nCH = 1 if gdename == "ballston spa, neu york usa"
replace nCH = 1 if gdename == "alberschwende a"
replace nCH = 1 if gdename == "sutton coldfield gb"
replace nCH = 1 if gdename == "chevreux f"
replace nCH = 1 if gdename == "los angeles kalifornien usa"
replace nCH = 1 if gdename == "wittnau d"
replace nCH = 1 if gdename == "goettingen (d)"
replace nCH = 1 if gdename == "eichenzell d"
replace nCH = 1 if gdename == "hillingdon gb"
replace nCH = 1 if gdename == "japan"
replace nCH = 1 if gdename == "kjoto j"
replace nCH = 1 if gdename == "asperg d"
replace nCH = 1 if gdename == "schliersee (d)"
replace nCH = 1 if gdename == "kiboutz neot mordechai il"
replace nCH = 1 if gdename == "henstedt ulzburg d"
replace nCH = 1 if gdename == "hridovre/kopenhagen (dk)"
replace nCH = 1 if gdename == "podbrezova slowakei"
replace nCH = 1 if gdename == "bad oeynhausen d"
replace nCH = 1 if gdename == "new malden gb"
replace nCH = 1 if gdename == "saulx"
replace nCH = 1 if gdename == "newton centre usa"
replace nCH = 1 if gdename == "giverny f"
replace nCH = 1 if gdename == "trumbull/connecticut usa"
replace nCH = 1 if gdename == "leiderdorp nl"
replace nCH = 1 if gdename == "newton blossomville gb"
replace nCH = 1 if gdename == "grez dolceau b"
replace nCH = 1 if gdename == "valleiry"
replace nCH = 1 if gdename == "pfungstadt d"
replace nCH = 1 if gdename == "meadville usa"
replace nCH = 1 if gdename == "champagne sur oise f"
replace nCH = 1 if gdename == "jehanster polleur b"
replace nCH = 1 if gdename == "alpharetta usa"
replace nCH = 1 if gdename == "teneriffa e"
replace nCH = 1 if gdename == "naples i"
replace nCH = 1 if gdename == "winnetka (illinois. usa)"
replace nCH = 1 if gdename == "tonbridge gb"
replace nCH = 1 if gdename == "eckenhaid eschenau d"
replace nCH = 1 if gdename == "hagondange f"
replace nCH = 1 if gdename == "papenburg d"
replace nCH = 1 if gdename == "johannesburg swa"
replace nCH = 1 if gdename == "aylesbury, bucks gb"
replace nCH = 1 if gdename == "palm beach gardens, fl, usa"
replace nCH = 1 if gdename == "marienheide (d)"
replace nCH = 1 if gdename == "munguia e"
replace nCH = 1 if gdename == "lavena ponte tresa i"
replace nCH = 1 if gdename == "wimsheim d"
replace nCH = 1 if gdename == "hellrup dk"
replace nCH = 1 if gdename == "wilmette/illinois usa"
replace nCH = 1 if gdename == "bramhall, cheshire gb"
replace nCH = 1 if gdename == "beirut rl ir"
replace nCH = 1 if gdename == "libreville gabun"
replace nCH = 1 if gdename == "voerde d"
replace nCH = 1 if gdename == "ruemmingen d"
replace nCH = 1 if gdename == "boxtel nl"
replace nCH = 1 if gdename == "haunstetten d"
replace nCH = 1 if gdename == "voorschoten"
replace nCH = 1 if gdename == "los altos hills, cal. (usa)"
replace nCH = 1 if gdename == "wassenberg eulenbusch d"
replace nCH = 1 if gdename == "north hampton, new hampshire usa"
replace nCH = 1 if gdename == "decatur, illinois usa"
replace nCH = 1 if gdename == "kanagawa (j)"
replace nCH = 1 if gdename == "rheda widenbrueck d"
replace nCH = 1 if gdename == "bad breisig d"
replace nCH = 1 if gdename == "st claude f"
replace nCH = 1 if gdename == "bergvik (s)"

// October 2020
replace nCH = 1 if gdename == "brssel"
replace nCH = 1 if gdename == "mnchen"
replace nCH = 1 if gdename == "d"
replace nCH = 1 if gdename == "dsseldorf"
replace nCH = 1 if gdename == "kln"
replace nCH = 1 if gdename == "f"
replace nCH = 1 if gdename == ", paris f"
replace nCH = 1 if gdename == ", st martin d heres"
replace nCH = 1 if gdename == ", eindhoven nl"
replace nCH = 1 if gdename == ", rottach egern d"
replace nCH = 1 if gdename == "freiburg i. b, elektrizitaetswerk rheinau ag, rheinau (20.0)"
replace nCH = 1 if gdename == "berg (d)"
replace nCH = 1 if gdename == "san diego cal, sulzer energieconsulting ag, winterthur (0.5)"
replace nCH = 1 if gdename == "grasse"
replace nCH = 1 if gdename == ", castellina i"
replace nCH = 1 if gdename == ", chambery f"
replace nCH = 1 if gdename == "athnes"
replace nCH = 1 if gdename == "budapest (h)"
replace nCH = 1 if gdename == "leonberg"
replace nCH = 1 if gdename == ", monaco mc"
replace nCH = 1 if gdename == ", london gb"
replace nCH = 1 if gdename == ", chilly mazarin f"
replace nCH = 1 if gdename == "khartoum (sud)"
replace nCH = 1 if gdename == "esbo (sf)"
replace nCH = 1 if gdename == "olivos ra"
replace nCH = 1 if gdename == "sceaux (f)"
replace nCH = 1 if gdename == "aachen brand d"
replace nCH = 1 if gdename == "piove di sacco i"
replace nCH = 1 if gdename == "lentate sul seveso i"
replace nCH = 1 if gdename == "apapa nigeria"
replace nCH = 1 if gdename == "tampa fl usa"
replace nCH = 1 if gdename == "bockenen a"
replace nCH = 1 if gdename == "bergen enkheim d"
replace nCH = 1 if gdename == "boonton usa"
replace nCH = 1 if gdename == "toronto ca"
replace nCH = 1 if gdename == "griechenland"
replace nCH = 1 if gdename == "elversele b"
replace nCH = 1 if gdename == "veigy foncenex (f)"
replace nCH = 1 if gdename == "bratislava (csfr)"
replace nCH = 1 if gdename == "gelnhausen (d)"
replace nCH = 1 if gdename == "pattaya t"
replace nCH = 1 if gdename == "ditzingen heimerdingen (d)"
replace nCH = 1 if gdename == "herrenschwanden"
replace nCH = 1 if gdename == "koeln dl"
replace nCH = 1 if gdename == "rungsted kyst (dk)"
replace nCH = 1 if gdename == "telfs (tirol)"
replace nCH = 1 if gdename == "hochberg d"
replace nCH = 1 if gdename == "nordborg d"
replace nCH = 1 if gdename == "chambourcy (f)"
replace nCH = 1 if gdename == "boxmeer (nl)"
replace nCH = 1 if gdename == "brentwood, tennesse usa"
replace nCH = 1 if gdename == "st helier gb"
replace nCH = 1 if gdename == "narbonne f"
replace nCH = 1 if gdename == "twickenham (middx gb)"
replace nCH = 1 if gdename == "north darmouth usa"
replace nCH = 1 if gdename == "newbury, berkshire gb"
replace nCH = 1 if gdename == "cookham dean, berkshire gb"
replace nCH = 1 if gdename == "leesburg"
replace nCH = 1 if gdename == "sark channel islands gb"
replace nCH = 1 if gdename == "rye, new york usa"
replace nCH = 1 if gdename == "trumbull, connecticut usa"
replace nCH = 1 if gdename == "aartselaar b"
replace nCH = 1 if gdename == "oberwesel d"
replace nCH = 1 if gdename == "hoevelhof espeln d"
replace nCH = 1 if gdename == "minneapolis (usa)"
replace nCH = 1 if gdename == "brockham, surrey gb"
replace nCH = 1 if gdename == "toluca lake, california usa"
replace nCH = 1 if gdename == "volders a"
replace nCH = 1 if gdename == "carnegie usa"
replace nCH = 1 if gdename == "v. monte"
replace nCH = 1 if gdename == "walthamstow(gb)"
replace nCH = 1 if gdename == "kinsale (irl)"
replace nCH = 1 if gdename == "lo barnocha (santiago) rch"
replace nCH = 1 if gdename == "weehawken, n.j. usa"
replace nCH = 1 if gdename == "rio mixoac (mex)"
replace nCH = 1 if gdename == "larkspur usa"
replace nCH = 1 if gdename == "berkely (usa)"
replace nCH = 1 if gdename == "les rousses f"
replace nCH = 1 if gdename == "obersulmetigen (d)"
replace nCH = 1 if gdename == "trissino i"
replace nCH = 1 if gdename == "milan (i)"
replace nCH = 1 if gdename == "trossingen (d)"
replace nCH = 1 if gdename == "les villers le lac f"
replace nCH = 1 if gdename == "libanon rl"
replace nCH = 1 if gdename == "gablitz a"
replace nCH = 1 if gdename == "st.helier, jersey gb"
replace nCH = 1 if gdename == "mezzegra (i)"
replace nCH = 1 if gdename == "levallois (f)"
replace nCH = 1 if gdename == "karlshamn s"
replace nCH = 1 if gdename == "frasnes les anvaing b"
replace nCH = 1 if gdename == "sassenage f"
replace nCH = 1 if gdename == "rossano veneto i"
replace nCH = 1 if gdename == "scheibeek b"
replace nCH = 1 if gdename == "anaheim/californie usa"
replace nCH = 1 if gdename == "retie b"
replace nCH = 1 if gdename == "como 1"
replace nCH = 1 if gdename == "cosefeld brd"
replace nCH = 1 if gdename == "nordenhof d"
replace nCH = 1 if gdename == "stadthagen d"
replace nCH = 1 if gdename == "wyckoff, new jersey usa"
replace nCH = 1 if gdename == "weiz a"
replace nCH = 1 if gdename == "herzogenrath (d)"
replace nCH = 1 if gdename == "lastra a signa i"
replace nCH = 1 if gdename == "haag/sennwald"
replace nCH = 1 if gdename == "bad hofgastein a"
replace nCH = 1 if gdename == "genk b"
replace nCH = 1 if gdename == "saint alban leysse f"
replace nCH = 1 if gdename == "kathmandu nepal"
replace nCH = 1 if gdename == "ludvika s"
replace nCH = 1 if gdename == "waimes (b)"
replace nCH = 1 if gdename == "freudenberg d"
replace nCH = 1 if gdename == "cusano milanino i"
replace nCH = 1 if gdename == "blue bell usa"
replace nCH = 1 if gdename == "steyr saass a"
replace nCH = 1 if gdename == "carlisle, cumbria gb"
replace nCH = 1 if gdename == "lillington gb"
replace nCH = 1 if gdename == "terrassa e"
replace nCH = 1 if gdename == "houston, texas (usa)"
replace nCH = 1 if gdename == "denderleeuw b"
replace nCH = 1 if gdename == "limburg d"
replace nCH = 1 if gdename == "forest hills usa"
replace nCH = 1 if gdename == "ouzouer sur loire f"
replace nCH = 1 if gdename == "st. georgen d"
replace nCH = 1 if gdename == "pigra i"
replace nCH = 1 if gdename == "winhoering (d)"
replace nCH = 1 if gdename == "tourrette levens f"
replace nCH = 1 if gdename == "bruinisse nl"
replace nCH = 1 if gdename == "nir zvi il"
replace nCH = 1 if gdename == "chantraine f"
replace nCH = 1 if gdename == "neuilly s s"
replace nCH = 1 if gdename == "meersburg (d)"
replace nCH = 1 if gdename == "sandhurst rsa"
replace nCH = 1 if gdename == "cermenate i"
replace nCH = 1 if gdename == "bad woerishofen"
replace nCH = 1 if gdename == "citta del messico mex"
replace nCH = 1 if gdename == "heemstede"
replace nCH = 1 if gdename == "gurtis a"
replace nCH = 1 if gdename == "durham nc usa"
replace nCH = 1 if gdename == "schiltach d"
replace nCH = 1 if gdename == "wilhelmshafen d"
replace nCH = 1 if gdename == "taschkent (usbekistan)"
replace nCH = 1 if gdename == "niefern d"
replace nCH = 1 if gdename == "heinsberg d"
replace nCH = 1 if gdename == "rockford usa"
replace nCH = 1 if gdename == "las vegas nevada usa"
replace nCH = 1 if gdename == "marbella/malaga (e)"
replace nCH = 1 if gdename == "calw d"
replace nCH = 1 if gdename == "hall tirol a"
replace nCH = 1 if gdename == "gaggenbach sulzbach d"
replace nCH = 1 if gdename == "rankwell a"
replace nCH = 1 if gdename == "bordeaux cauderan f"
replace nCH = 1 if gdename == "karlsfeld d"
replace nCH = 1 if gdename == "tahiti"
replace nCH = 1 if gdename == "bujumbura, burundi"
replace nCH = 1 if gdename == "montignoso i"
replace nCH = 1 if gdename == "bladel nl"
replace nCH = 1 if gdename == "colle val d elsa i"
replace nCH = 1 if gdename == "remchingen d"
replace nCH = 1 if gdename == "merchtem b"
replace nCH = 1 if gdename == "tullinge s"
replace nCH = 1 if gdename == "kuusankoski sf"
replace nCH = 1 if gdename == "san marino, kalifornien usa"
replace nCH = 1 if gdename == "old westbury, new york, usa"
replace nCH = 1 if gdename == "stuttgart schoenberg"
replace nCH = 1 if gdename == "st. tripoli marine (libanon)"
replace nCH = 1 if gdename == "ferney voltaire/ain f"
replace nCH = 1 if gdename == "salisbury"
replace nCH = 1 if gdename == "marienbaum d"
replace nCH = 1 if gdename == "dortmund dl"
replace nCH = 1 if gdename == "uetliburg/gommiswald"
replace nCH = 1 if gdename == "st. julians (malta)"
replace nCH = 1 if gdename == "churchville, philadelphia usa"
replace nCH = 1 if gdename == "monnetier mornex f"
replace nCH = 1 if gdename == "ipswich (gb)"
replace nCH = 1 if gdename == "bardolino (i)"
replace nCH = 1 if gdename == "sark, channel islands gb"
replace nCH = 1 if gdename == "lima (peru)"
replace nCH = 1 if gdename == "sipplingen d"
replace nCH = 1 if gdename == "neffies f"
replace nCH = 1 if gdename == "teheran, iran"
replace nCH = 1 if gdename == "st. saviours/guernsey gb"
replace nCH = 1 if gdename == "noordwjk"
replace nCH = 1 if gdename == "turnov cz"
replace nCH = 1 if gdename == "grainau d"
replace nCH = 1 if gdename == "offenhausen d"
replace nCH = 1 if gdename == "dalem f"
replace nCH = 1 if gdename == "sorga i"
replace nCH = 1 if gdename == "neustadt (d)"
replace nCH = 1 if gdename == "kehlen l"
replace nCH = 1 if gdename == "cesano boscone (i)"
replace nCH = 1 if gdename == "oslo no"
replace nCH = 1 if gdename == "seraing b"
replace nCH = 1 if gdename == "grossmaischeid d"
replace nCH = 1 if gdename == "novi di modena i"
replace nCH = 1 if gdename == "burnham bucks gb"
replace nCH = 1 if gdename == "bures sur yvette f"
replace nCH = 1 if gdename == "chigwell gb"
replace nCH = 1 if gdename == "almaty kazalkhstan"
replace nCH = 1 if gdename == "freiburg tiengen d"
replace nCH = 1 if gdename == "stein d"
replace nCH = 1 if gdename == "seyssinet pariset f"
replace nCH = 1 if gdename == "upper montclair n.j. usa"
replace nCH = 1 if gdename == "neudenau (d)"
replace nCH = 1 if gdename == "benei beraq il"
replace nCH = 1 if gdename == "gorssel (nl)"
replace nCH = 1 if gdename == "neukeferloh d"
replace nCH = 1 if gdename == "vaihingen d"
replace nCH = 1 if gdename == "knowle, warwickshire gb"
replace nCH = 1 if gdename == "herzliah pituah il"
replace nCH = 1 if gdename == "chappaqua, new york usa"
replace nCH = 1 if gdename == "villers allerand f"
replace nCH = 1 if gdename == "aachen"
replace nCH = 1 if gdename == "klais elmau d"
replace nCH = 1 if gdename == "carthage tn"
replace nCH = 1 if gdename == "publier f"
replace nCH = 1 if gdename == "guildford (gb)"
replace nCH = 1 if gdename == "grenzach (d)"
replace nCH = 1 if gdename == "leimen bei heidelberg (d)"
replace nCH = 1 if gdename == "st.georgen/pinzgau a"
replace nCH = 1 if gdename == "viuz en sallaz (f)"
replace nCH = 1 if gdename == "peking (volksrepublik china)"
replace nCH = 1 if gdename == "ferney voltaire france"
replace nCH = 1 if gdename == "greenwich, connecticut (usa)"
replace nCH = 1 if gdename == "bosisio parini i"
replace nCH = 1 if gdename == "dendermande b"
replace nCH = 1 if gdename == "harvard mass./usa"
replace nCH = 1 if gdename == "manhasset usa"
replace nCH = 1 if gdename == "ann arbor usa"
replace nCH = 1 if gdename == "wolfenbuettel (d)"
replace nCH = 1 if gdename == "appiano gentile i"
replace nCH = 1 if gdename == "midland usa"
replace nCH = 1 if gdename == "reigate, surrey gb"
replace nCH = 1 if gdename == "pessano con bornago i"
replace nCH = 1 if gdename == "princeton (usa)"
replace nCH = 1 if gdename == "stemwede d"
replace nCH = 1 if gdename == "albstadt laufen d"
replace nCH = 1 if gdename == "bocholt (d)"
replace nCH = 1 if gdename == "aichwald d"
replace nCH = 1 if gdename == "giungano i"
replace nCH = 1 if gdename == "north kingstown, rhode island usa"
replace nCH = 1 if gdename == "satolas et bonce f"
replace nCH = 1 if gdename == "kyrkslatt sf"
replace nCH = 1 if gdename == "neuilly s seine (f)"
replace nCH = 1 if gdename == "finchley london gb"
replace nCH = 1 if gdename == "georgetown grand cayman, cayman"
replace nCH = 1 if gdename == "palos heights usa"
replace nCH = 1 if gdename == "zittau d"
replace nCH = 1 if gdename == "hailsham, east sussex gb"
replace nCH = 1 if gdename == "woodbury, california usa"
replace nCH = 1 if gdename == "edgecliff aus"
replace nCH = 1 if gdename == "nueziders a"
replace nCH = 1 if gdename == "kokomo usa"
replace nCH = 1 if gdename == "efringen d"
replace nCH = 1 if gdename == "bronxvill ny, usa"
replace nCH = 1 if gdename == "bretigny s morrens"
replace nCH = 1 if gdename == "victoria island, lagos (nig)"
replace nCH = 1 if gdename == "clamart (f)"
replace nCH = 1 if gdename == "jungingen d"
replace nCH = 1 if gdename == "seeboden a"
replace nCH = 1 if gdename == "colmar (f)"
replace nCH = 1 if gdename == "neuershausen d"
replace nCH = 1 if gdename == "hotel hohe"
replace nCH = 1 if gdename == "altrincham gb"
replace nCH = 1 if gdename == "bedford gb"
replace nCH = 1 if gdename == "vallauris f"
replace nCH = 1 if gdename == "crans gevrier f"
replace nCH = 1 if gdename == "ebersburg weyhers d"
replace nCH = 1 if gdename == "cranleigh, surey, gb"
replace nCH = 1 if gdename == "locust valley new york usa"
replace nCH = 1 if gdename == "st. wolfgang am wolfgangsee a"
replace nCH = 1 if gdename == "voorburg (nl)"
replace nCH = 1 if gdename == "anieres f"
replace nCH = 1 if gdename == "limonest f"
replace nCH = 1 if gdename == "cowden gb"
replace nCH = 1 if gdename == "istanbul (tk)"
replace nCH = 1 if gdename == "daisendorf d"
replace nCH = 1 if gdename == "santa barbara usa"
replace nCH = 1 if gdename == "mions f"
replace nCH = 1 if gdename == "creney f"
replace nCH = 1 if gdename == "rijsbergen nl"
replace nCH = 1 if gdename == "nideggen d"
replace nCH = 1 if gdename == "wilton, connecticut usa"
replace nCH = 1 if gdename == "grunbach d"
replace nCH = 1 if gdename == "kokola sf"
replace nCH = 1 if gdename == "muellheim (d)"
replace nCH = 1 if gdename == "costa d en blanes, mallorca e"
replace nCH = 1 if gdename == "changis sur marne f"
replace nCH = 1 if gdename == "leatherhead/surrey gb"
replace nCH = 1 if gdename == "untersiggingen d"
replace nCH = 1 if gdename == "ashford, kent gb"
replace nCH = 1 if gdename == "frankfurt a/main d"
replace nCH = 1 if gdename == "corsano i"
replace nCH = 1 if gdename == "angelholm s"
replace nCH = 1 if gdename == "unterkirnach d"
replace nCH = 1 if gdename == "montpellier (f)"
replace nCH = 1 if gdename == "untergruppenbach d"
replace nCH = 1 if gdename == "liphoo, hantshire gb"
replace nCH = 1 if gdename == "polleur b"
replace nCH = 1 if gdename == "datteln brd"
replace nCH = 1 if gdename == "nordhausen d"
replace nCH = 1 if gdename == "coatesville pennsylvania usa"
replace nCH = 1 if gdename == "cloudside cheshire gb"
replace nCH = 1 if gdename == "fournets luisans f"
replace nCH = 1 if gdename == "london cdn"
replace nCH = 1 if gdename == "berg am starnberger see d"
replace nCH = 1 if gdename == "manama town brn"
replace nCH = 1 if gdename == "santa cruz bol"
replace nCH = 1 if gdename == "elda/spanien"
replace nCH = 1 if gdename == "norwalk, connecticut (usa)"
replace nCH = 1 if gdename == "copenhague"
replace nCH = 1 if gdename == "woerth d"
replace nCH = 1 if gdename == "fauvillers b"
replace nCH = 1 if gdename == "kummerfeld d"
replace nCH = 1 if gdename == "montfoort nl"
replace nCH = 1 if gdename == "weil (d)"
replace nCH = 1 if gdename == "heusenstamm (d)"
replace nCH = 1 if gdename == "yokohama"
replace nCH = 1 if gdename == "einbeck (d)"
replace nCH = 1 if gdename == "los angeles, california usa"
replace nCH = 1 if gdename == "turramurra, sidney australien"
replace nCH = 1 if gdename == "providence/rhode island usa"
replace nCH = 1 if gdename == "horseheads usa"
replace nCH = 1 if gdename == "colombey f"
replace nCH = 1 if gdename == "colwyn bay, n. wales gb"
replace nCH = 1 if gdename == "denia (s)"
replace nCH = 1 if gdename == "berkeley ca usa"
replace nCH = 1 if gdename == "irving texas usa"
replace nCH = 1 if gdename == "altmuenster a"
replace nCH = 1 if gdename == "paget parish vgb"
replace nCH = 1 if gdename == "genova 1"
replace nCH = 1 if gdename == "weinstadt grossheppach d"
replace nCH = 1 if gdename == "andorre f"
replace nCH = 1 if gdename == "vigonza i"
replace nCH = 1 if gdename == "warren, new jersey usa"
replace nCH = 1 if gdename == "remscheid (d)"
replace nCH = 1 if gdename == "la hulpe (b)"
replace nCH = 1 if gdename == "paris(f)"
replace nCH = 1 if gdename == "toluca lake usa"
replace nCH = 1 if gdename == "tunbridge gb"
replace nCH = 1 if gdename == "cannon park, coventry gb"
replace nCH = 1 if gdename == "chicago il usa"
replace nCH = 1 if gdename == "bagnaria arsa i"
replace nCH = 1 if gdename == "sciez (f)"
replace nCH = 1 if gdename == "kolding (dk)"
replace nCH = 1 if gdename == "melzo i"
replace nCH = 1 if gdename == "stuhr varrel d"
replace nCH = 1 if gdename == "felino i"
replace nCH = 1 if gdename == "ordino e"
replace nCH = 1 if gdename == "spaarnwoude nl"
replace nCH = 1 if gdename == "boise idaho usa"
replace nCH = 1 if gdename == "sunnyvale ca usa"
replace nCH = 1 if gdename == "fort lauderale, florida usa"
replace nCH = 1 if gdename == "enumclaw (washington) usa"
replace nCH = 1 if gdename == "birmingham, michigan usa"
replace nCH = 1 if gdename == "konstanz/d"
replace nCH = 1 if gdename == "moerfelden bei frankfurt am main d"
replace nCH = 1 if gdename == "portola valley, kalifornien usa"
replace nCH = 1 if gdename == "nederhorst den berg nl"
replace nCH = 1 if gdename == "etterbeck b"
replace nCH = 1 if gdename == "el cerrito, california (usa)"
replace nCH = 1 if gdename == "orgeval, yvelines f"
replace nCH = 1 if gdename == "schwenningen brd"
replace nCH = 1 if gdename == "remscheid lennep (d)"
replace nCH = 1 if gdename == "malgesso i"
replace nCH = 1 if gdename == "salonique"
replace nCH = 1 if gdename == "neuenburg d"
replace nCH = 1 if gdename == "wombach d"
replace nCH = 1 if gdename == "redhill gb"
replace nCH = 1 if gdename == "sunning dale, berkshire gb"
replace nCH = 1 if gdename == "harmelen (nl)"
replace nCH = 1 if gdename == "bonneuil sur marne f"
replace nCH = 1 if gdename == "honore labande, rue 2, monaco"
replace nCH = 1 if gdename == "flushing new york usa"
replace nCH = 1 if gdename == "ak diemen nl"
replace nCH = 1 if gdename == "skodje n"
replace nCH = 1 if gdename == "guilford gb"
replace nCH = 1 if gdename == "woodland hills cal. usa"
replace nCH = 1 if gdename == ", moskau rus"
replace nCH = 1 if gdename == "voehrenbach d"
replace nCH = 1 if gdename == "new port beach usa"
replace nCH = 1 if gdename == "prague cs"
replace nCH = 1 if gdename == "hertz gb"
replace nCH = 1 if gdename == "guernsey gbg"
replace nCH = 1 if gdename == "chicago (illinois usa)"
replace nCH = 1 if gdename == "amersham, buckinghamshire (gb)"
replace nCH = 1 if gdename == "yateley gb"
replace nCH = 1 if gdename == "brembate di sopra i"
replace nCH = 1 if gdename == "bad saeckingen rippolingen d"
replace nCH = 1 if gdename == "weston turvill gb"
replace nCH = 1 if gdename == "neuenrade d"
replace nCH = 1 if gdename == "fayetteville usa"
replace nCH = 1 if gdename == "kiev ukr"
replace nCH = 1 if gdename == "mouveaux f"
replace nCH = 1 if gdename == "birkeroed (dk)"
replace nCH = 1 if gdename == "annemasse fr"
replace nCH = 1 if gdename == "peterborough (gb)"
replace nCH = 1 if gdename == "bruehl (d)"
replace nCH = 1 if gdename == "beveren leie b"
replace nCH = 1 if gdename == "stuttgart bad cannstatt d"
replace nCH = 1 if gdename == "cheddington gb"
replace nCH = 1 if gdename == "missouri city, texas usa"
replace nCH = 1 if gdename == "stockport"
replace nCH = 1 if gdename == "bielsko biala pl"
replace nCH = 1 if gdename == "lexington, ma usa"
replace nCH = 1 if gdename == "st jullen en genevois f"
replace nCH = 1 if gdename == "lustenau (a)"
replace nCH = 1 if gdename == "burlingame usa"
replace nCH = 1 if gdename == "relegem b"
replace nCH = 1 if gdename == "rosta i"
replace nCH = 1 if gdename == "londres (gb)"
replace nCH = 1 if gdename == "nuland nl"
replace nCH = 1 if gdename == "voerendaal nl"
replace nCH = 1 if gdename == "fujisawa city, kanagawa pref"
replace nCH = 1 if gdename == "raalte nl"
replace nCH = 1 if gdename == "veigy foncenez (f)"
replace nCH = 1 if gdename == "marnes la coquette, haut de se"
replace nCH = 1 if gdename == "salvador"
replace nCH = 1 if gdename == "buedingen b. frankfurt d"
replace nCH = 1 if gdename == "mashad iran"
replace nCH = 1 if gdename == "lublin pl"
replace nCH = 1 if gdename == "bentley gb"
replace nCH = 1 if gdename == "bath gb"
replace nCH = 1 if gdename == "summit, new jersey, usa"
replace nCH = 1 if gdename == "italia"
replace nCH = 1 if gdename == "sao paulo brasile"
replace nCH = 1 if gdename == "bonducs f"
replace nCH = 1 if gdename == "bad endort d"
replace nCH = 1 if gdename == "ingoldstadt (d)"
replace nCH = 1 if gdename == "koenigstein/d"
replace nCH = 1 if gdename == "sulzbach/main d"
replace nCH = 1 if gdename == "paoli (pennsylvania) usa"
replace nCH = 1 if gdename == "taastrup d"
replace nCH = 1 if gdename == "rickmansworth (gb)"
replace nCH = 1 if gdename == "arbent/oyonnax f"
replace nCH = 1 if gdename == "amsterdam/nl"
replace nCH = 1 if gdename == "sharjah vae"
replace nCH = 1 if gdename == "champanges f"
replace nCH = 1 if gdename == "treviso"
replace nCH = 1 if gdename == "standon gb"
replace nCH = 1 if gdename == "caldy gb"
replace nCH = 1 if gdename == "sorrento i"
replace nCH = 1 if gdename == "achen (d)"
replace nCH = 1 if gdename == "rosenthal d"
replace nCH = 1 if gdename == "rastatt (d)"
replace nCH = 1 if gdename == "bothell usa"
replace nCH = 1 if gdename == "sea point cape south africa"
replace nCH = 1 if gdename == "meerssen nl"
replace nCH = 1 if gdename == "gladwyne usa"
replace nCH = 1 if gdename == "patos br"
replace nCH = 1 if gdename == "heenstede nl"
replace nCH = 1 if gdename == "sulz d"
replace nCH = 1 if gdename == "broomley, kent gb"
replace nCH = 1 if gdename == "mendham new jersey usa"
replace nCH = 1 if gdename == "talheim d"
replace nCH = 1 if gdename == "francfort sur le main"
replace nCH = 1 if gdename == "tiana barcelona e"
replace nCH = 1 if gdename == "paraguay py"
replace nCH = 1 if gdename == "ostrach d"
replace nCH = 1 if gdename == "tanger ma"
replace nCH = 1 if gdename == "mexico city (mex)"
replace nCH = 1 if gdename == "singen/d"
replace nCH = 1 if gdename == "oestringen odenheim d"
replace nCH = 1 if gdename == "titisee neustadt d"
replace nCH = 1 if gdename == "south croydon (gb)"
replace nCH = 1 if gdename == "rielasingen worblingen (d)"
replace nCH = 1 if gdename == "residence bellevue e 4, divonne les bains"
replace nCH = 1 if gdename == "mequon wisconsin usa"
replace nCH = 1 if gdename == "hassocks (gb)"
replace nCH = 1 if gdename == "remagen (d)"
replace nCH = 1 if gdename == "montabaur/d"
replace nCH = 1 if gdename == "montabaur d"
replace nCH = 1 if gdename == "jakarta selatan idn"
replace nCH = 1 if gdename == "bargteheide d"
replace nCH = 1 if gdename == "osaka (j)"
replace nCH = 1 if gdename == "thursley godalming gb"
replace nCH = 1 if gdename == "echenevex (f)"
replace nCH = 1 if gdename == "galwas knocknacarra (irland)"
replace nCH = 1 if gdename == "chimoio (mocambique)"
replace nCH = 1 if gdename == "quebec, can"
replace nCH = 1 if gdename == "aoste"
replace nCH = 1 if gdename == "la nucia/sevilla e"
replace nCH = 1 if gdename == "marssac sur tarn f"
replace nCH = 1 if gdename == "guadalajara mex"
replace nCH = 1 if gdename == "dschidda sa"
replace nCH = 1 if gdename == "apapa (wan)"
replace nCH = 1 if gdename == "kuessaberg/ettikon d"
replace nCH = 1 if gdename == "buehl vimbuch"
replace nCH = 1 if gdename == "breda (nl)"
replace nCH = 1 if gdename == "sexau d"
replace nCH = 1 if gdename == "neukirchen (d)"
replace nCH = 1 if gdename == "hurstpierpoint/sussex gb"
replace nCH = 1 if gdename == "therens f"
replace nCH = 1 if gdename == "hartley winthey"
replace nCH = 1 if gdename == "carnate i"
replace nCH = 1 if gdename == "gran canaria e"
replace nCH = 1 if gdename == "paramus new jersey usa"
replace nCH = 1 if gdename == "beaconsfield cdn"
replace nCH = 1 if gdename == "tel aviv isr"
replace nCH = 1 if gdename == "luton gb"
replace nCH = 1 if gdename == "penn, wolverhampton, west midlands"
replace nCH = 1 if gdename == "los gatos usa"
replace nCH = 1 if gdename == "columbia, maryland usa"
replace nCH = 1 if gdename == "st. ives aus"
replace nCH = 1 if gdename == "drottningholm, malmwik s"
replace nCH = 1 if gdename == "bombay in"
replace nCH = 1 if gdename == "sudbury usa"
replace nCH = 1 if gdename == "bjerringbro"
replace nCH = 1 if gdename == "ebeltoft dk"
replace nCH = 1 if gdename == "haag"
replace nCH = 1 if gdename == "lebanon new jersy usa"
replace nCH = 1 if gdename == "oberboihingen d"
replace nCH = 1 if gdename == "rosny sous bois"
replace nCH = 1 if gdename == "nettal d"
replace nCH = 1 if gdename == "canterbury gb"
replace nCH = 1 if gdename == "novi sad yu"
replace nCH = 1 if gdename == "cogny f"
replace nCH = 1 if gdename == "dunkeld za"
replace nCH = 1 if gdename == "herzelia il"
replace nCH = 1 if gdename == "bayrischzell d"
replace nCH = 1 if gdename == "sint truiden b"
replace nCH = 1 if gdename == "willich anrath d"
replace nCH = 1 if gdename == "saint maurice f"
replace nCH = 1 if gdename == "arlington heights usa"
replace nCH = 1 if gdename == "broadstairs, kent gb"
replace nCH = 1 if gdename == "haifa israel"
replace nCH = 1 if gdename == "le vert, channel islands"
replace nCH = 1 if gdename == "st martin/chezard st m"
replace nCH = 1 if gdename == "upper montclaire (new jersey/usa)"
replace nCH = 1 if gdename == "marchington gb"
replace nCH = 1 if gdename == "bad saeckingen brd"
replace nCH = 1 if gdename == "lion (f)"
replace nCH = 1 if gdename == "montevideo ra"
replace nCH = 1 if gdename == "wellington (suedafrika)"
replace nCH = 1 if gdename == "saint antoine f"
replace nCH = 1 if gdename == "rotthalmuenster d"
replace nCH = 1 if gdename == "sint michialsgestel nl"
replace nCH = 1 if gdename == "bourbach le bas f"
replace nCH = 1 if gdename == "borromeo i"
replace nCH = 1 if gdename == "coueron f"
replace nCH = 1 if gdename == "schwalbach (d)"
replace nCH = 1 if gdename == "senden aufheim d"
replace nCH = 1 if gdename == "washington d.c, usa"
replace nCH = 1 if gdename == "pannessieres f"
replace nCH = 1 if gdename == "putzbrunn solalinden d"
replace nCH = 1 if gdename == "oudemaarde b"
replace nCH = 1 if gdename == "mierlo (nl)"
replace nCH = 1 if gdename == "isle d adam f"
replace nCH = 1 if gdename == "cernex f"
replace nCH = 1 if gdename == "weilstetten/d"
replace nCH = 1 if gdename == "wentzwiller (f)"
replace nCH = 1 if gdename == "winzenhohl d"
replace nCH = 1 if gdename == "barbizon f"
replace nCH = 1 if gdename == "burgdorf ehlershausen d"
replace nCH = 1 if gdename == "stamford, connecticut usa"
replace nCH = 1 if gdename == "yalta, ukraine"
replace nCH = 1 if gdename == "gray f"
replace nCH = 1 if gdename == "marnheim d"
replace nCH = 1 if gdename == "viry/f"
replace nCH = 1 if gdename == "indiana (usa)"
replace nCH = 1 if gdename == "toronto cn"
replace nCH = 1 if gdename == "appleton (gb)"
replace nCH = 1 if gdename == "south laguna, california usa"
replace nCH = 1 if gdename == "fujisawa"
replace nCH = 1 if gdename == "farum dm"
replace nCH = 1 if gdename == "knegsel nl"
replace nCH = 1 if gdename == "white plains ny usa"
replace nCH = 1 if gdename == "helensburgh"
replace nCH = 1 if gdename == "palm beach usa"
replace nCH = 1 if gdename == "lafayette california usa"
replace nCH = 1 if gdename == "jyskae fin"
replace nCH = 1 if gdename == "bournemouth (gb)"
replace nCH = 1 if gdename == "nashua, new hampshire usa"
replace nCH = 1 if gdename == "gleichen brehmke d"
replace nCH = 1 if gdename == "vejen dk"
replace nCH = 1 if gdename == "pittsburgh, penn. usa"
replace nCH = 1 if gdename == "wettenberg d"
replace nCH = 1 if gdename == "petts wood gb"
replace nCH = 1 if gdename == "bad duerrheim d"
replace nCH = 1 if gdename == "ginsheim gustavbur s"
replace nCH = 1 if gdename == "paget bermudas"
replace nCH = 1 if gdename == "burlingame ca usa"
replace nCH = 1 if gdename == "bishop auckland gb"
replace nCH = 1 if gdename == "hemel hempstead, hertfordshire gb"
replace nCH = 1 if gdename == "jork d"
replace nCH = 1 if gdename == "reit im winkel d"
replace nCH = 1 if gdename == "oehningen wangen d"
replace nCH = 1 if gdename == "dayton/ohio usa"
replace nCH = 1 if gdename == "lahore pk"
replace nCH = 1 if gdename == "homburg (d)"
replace nCH = 1 if gdename == "villanova di camposampiero i"
replace nCH = 1 if gdename == "vasteras s"
replace nCH = 1 if gdename == "ober roden d"
replace nCH = 1 if gdename == "paraguay"
replace nCH = 1 if gdename == "cremenaga i"
replace nCH = 1 if gdename == "altea la vieja/alicante e"
replace nCH = 1 if gdename == "horb d"
replace nCH = 1 if gdename == "klein winternheim d"
replace nCH = 1 if gdename == "cabinda pan"
replace nCH = 1 if gdename == "stetten d"
replace nCH = 1 if gdename == "freiburg opfingen d"
replace nCH = 1 if gdename == "bagshot, surrey gb"
replace nCH = 1 if gdename == "rixensart (b)"
replace nCH = 1 if gdename == "ville la gland f"
replace nCH = 1 if gdename == "speyer d"
replace nCH = 1 if gdename == "mcewen/tn, tennesse usa"
replace nCH = 1 if gdename == "cosenza i"
replace nCH = 1 if gdename == "hosenruck/wuppenau"
replace nCH = 1 if gdename == "lier b"
replace nCH = 1 if gdename == "kemerton gb"
replace nCH = 1 if gdename == "freiburg i. b. (d)"
replace nCH = 1 if gdename == "suresnes (f)"
replace nCH = 1 if gdename == "le muraz f"
replace nCH = 1 if gdename == "wittinsburg bl"
replace nCH = 1 if gdename == "froendenberg d"
replace nCH = 1 if gdename == "simbury, conn. usa"
replace nCH = 1 if gdename == "wlen (a)"
replace nCH = 1 if gdename == "biberach a.d.r. d"
replace nCH = 1 if gdename == "angelmodde (d)"
replace nCH = 1 if gdename == "steinheim am main d"
replace nCH = 1 if gdename == "fredensborg"
replace nCH = 1 if gdename == "schongau d"
replace nCH = 1 if gdename == "rumont f"
replace nCH = 1 if gdename == "winwick, cheshire gb"
replace nCH = 1 if gdename == "petworth w. sussex gb"
replace nCH = 1 if gdename == "hardheim d"
replace nCH = 1 if gdename == "huenstetten d"
replace nCH = 1 if gdename == "la celle st cloud (f)"
replace nCH = 1 if gdename == "meiningen a"
replace nCH = 1 if gdename == "bergneustadt (d)"
replace nCH = 1 if gdename == "spencer, indiana usa"
replace nCH = 1 if gdename == "barby f"
replace nCH = 1 if gdename == "hartbert"
replace nCH = 1 if gdename == "allensbach (d)"
replace nCH = 1 if gdename == "bad krozingen (d)"
replace nCH = 1 if gdename == "lagoa p"
replace nCH = 1 if gdename == "maasland (nl)"
replace nCH = 1 if gdename == "fresno/calif. usa"
replace nCH = 1 if gdename == "koge dk"
replace nCH = 1 if gdename == "santiago"
replace nCH = 1 if gdename == "greisch l"
replace nCH = 1 if gdename == "osaka"
replace nCH = 1 if gdename == "paceco i"
replace nCH = 1 if gdename == "wernhout nl"
replace nCH = 1 if gdename == "cockeysville usa"
replace nCH = 1 if gdename == "simmertal d"
replace nCH = 1 if gdename == "mihara j"
replace nCH = 1 if gdename == "meersen nl"
replace nCH = 1 if gdename == "berleur b"
replace nCH = 1 if gdename == "prittlbach/hebertshausen d"
replace nCH = 1 if gdename == "holzgerlingen d"
replace nCH = 1 if gdename == "lolingen (b)"
replace nCH = 1 if gdename == "segny (f)"
replace nCH = 1 if gdename == "st. lucia wl"
replace nCH = 1 if gdename == "engelsbrand d"
replace nCH = 1 if gdename == "lancs gb"
replace nCH = 1 if gdename == "fort myers (usa)"
replace nCH = 1 if gdename == "igels a"
replace nCH = 1 if gdename == "lancashire gb"
replace nCH = 1 if gdename == "steinsoultz f"
replace nCH = 1 if gdename == "ornez f"
replace nCH = 1 if gdename == "saddle river usa"
replace nCH = 1 if gdename == "omaruru (republik"
replace nCH = 1 if gdename == "mindelheim d"
replace nCH = 1 if gdename == "wembley (gb)"
replace nCH = 1 if gdename == "potomac (usa)"
replace nCH = 1 if gdename == "keerberger b"
replace nCH = 1 if gdename == "rijen nl"
replace nCH = 1 if gdename == "vaestra froelunda s"
replace nCH = 1 if gdename == "philadelphie usa"
replace nCH = 1 if gdename == "altenberge (d)"
replace nCH = 1 if gdename == "arendonk b"
replace nCH = 1 if gdename == "haus alpenblick, voa"
replace nCH = 1 if gdename == "welwyn garden city gb"
replace nCH = 1 if gdename == "praha cz"
replace nCH = 1 if gdename == "bad godesberg d"
replace nCH = 1 if gdename == "graz/a"
replace nCH = 1 if gdename == "oldenzaal nl"
replace nCH = 1 if gdename == "columbus/ind. usa"
replace nCH = 1 if gdename == "eppingen muehlbach d"
replace nCH = 1 if gdename == "aarhus (dk)"
replace nCH = 1 if gdename == "wartberg a"
replace nCH = 1 if gdename == "taunusstein (d)"
replace nCH = 1 if gdename == "pietarsaari sf"
replace nCH = 1 if gdename == "kenwood, california usa"
replace nCH = 1 if gdename == "st louis de la chaussee f"
replace nCH = 1 if gdename == "gr. canaria"
replace nCH = 1 if gdename == "schopflheim d"
replace nCH = 1 if gdename == "hastings gb"
replace nCH = 1 if gdename == "san antonio, texas usa"
replace nCH = 1 if gdename == "weinstadt (d)"
replace nCH = 1 if gdename == "guernsey (gb)"
replace nCH = 1 if gdename == "baierbrunn d"
replace nCH = 1 if gdename == "venelles f"
replace nCH = 1 if gdename == "muelheim an der ruhr (d)"
replace nCH = 1 if gdename == "kleinblittersdorf d"
replace nCH = 1 if gdename == "esloev s"
replace nCH = 1 if gdename == "dalton (massachusetts/usa)"
replace nCH = 1 if gdename == "voecklabruck a"
replace nCH = 1 if gdename == "san juan capistrano usa"
replace nCH = 1 if gdename == "hof d"
replace nCH = 1 if gdename == "yvoir b"
replace nCH = 1 if gdename == "osaka, jap"
replace nCH = 1 if gdename == "herzlyia il"
replace nCH = 1 if gdename == "fleet (gb)"
replace nCH = 1 if gdename == "gorliz e"
replace nCH = 1 if gdename == "akron usa"
replace nCH = 1 if gdename == "zagreb y"
replace nCH = 1 if gdename == "tonneins f"
replace nCH = 1 if gdename == "weinheim an der bergstrasse d"
replace nCH = 1 if gdename == "sienne i"
replace nCH = 1 if gdename == "betzdorf/sieg d"
replace nCH = 1 if gdename == "fino mornasco i"
replace nCH = 1 if gdename == "dunham cdn"
replace nCH = 1 if gdename == "richemond (usa)"
replace nCH = 1 if gdename == "miami fl usa"
replace nCH = 1 if gdename == "glenorie aus"
replace nCH = 1 if gdename == "san maurizio d opaglio i"
replace nCH = 1 if gdename == "fillingen schwenningen d"
replace nCH = 1 if gdename == "boulogne/f"
replace nCH = 1 if gdename == "vorra d"
replace nCH = 1 if gdename == "machida city j"
replace nCH = 1 if gdename == "pryn mawr, pennsylvania usa"
replace nCH = 1 if gdename == "valdosta georgie usa"
replace nCH = 1 if gdename == "honolulu usa"
replace nCH = 1 if gdename == "davidson usa"
replace nCH = 1 if gdename == "burgdorf (d)"
replace nCH = 1 if gdename == "santa monica, california usa"
replace nCH = 1 if gdename == "buchschlag b. frankfurt a.m"
replace nCH = 1 if gdename == "sorrento aus"
replace nCH = 1 if gdename == "bellevue hill (aus)"
replace nCH = 1 if gdename == "beckum d"
replace nCH = 1 if gdename == "bonne f"
replace nCH = 1 if gdename == "colorado springs usa"
replace nCH = 1 if gdename == "betanzos e"
replace nCH = 1 if gdename == "s. francisco usa"
replace nCH = 1 if gdename == "aelmhut s"
replace nCH = 1 if gdename == "ador, quai"
replace nCH = 1 if gdename == "bovisio masciago i"
replace nCH = 1 if gdename == "charnay les macon f"
replace nCH = 1 if gdename == "einhausen d"
replace nCH = 1 if gdename == "herrliberg zh"
replace nCH = 1 if gdename == "stockholm sv"
replace nCH = 1 if gdename == "reeuwijk (nl)"
replace nCH = 1 if gdename == "maidenhead, berkshire gb"
replace nCH = 1 if gdename == "heerbrugg/au"
replace nCH = 1 if gdename == "mauren li"
replace nCH = 1 if gdename == "misrah kola, attard (malta)"
replace nCH = 1 if gdename == "vineuil f"
replace nCH = 1 if gdename == "palos verdes estates usa"
replace nCH = 1 if gdename == "warsawa pl"
replace nCH = 1 if gdename == "corroy le grand b"
replace nCH = 1 if gdename == "falun s"
replace nCH = 1 if gdename == "lynge dk"
replace nCH = 1 if gdename == "kelkheim eppenhain (d)"
replace nCH = 1 if gdename == "bellegarde f"
replace nCH = 1 if gdename == "schwanberg a"
replace nCH = 1 if gdename == "katonah, new york usa"
replace nCH = 1 if gdename == "aulendorf d"
replace nCH = 1 if gdename == "braishfield gb"
replace nCH = 1 if gdename == "ruggel fl"
replace nCH = 1 if gdename == "puria i"
replace nCH = 1 if gdename == "bonheiden b"
replace nCH = 1 if gdename == "st omobono imagna i"
replace nCH = 1 if gdename == "falsterbo (s)"
replace nCH = 1 if gdename == "st. dizier f"
replace nCH = 1 if gdename == "temecula usa"
replace nCH = 1 if gdename == "palatine, illinois usa"
replace nCH = 1 if gdename == "tucson usa"
replace nCH = 1 if gdename == "noblesville/ind. usa"
replace nCH = 1 if gdename == "neuilly s/seine/f"
replace nCH = 1 if gdename == "ryadh ksa"
replace nCH = 1 if gdename == "onchan, isle of man"
replace nCH = 1 if gdename == "neuilly s/seine (f)"
replace nCH = 1 if gdename == "rosa i"
replace nCH = 1 if gdename == "portola valley usa"
replace nCH = 1 if gdename == "wilton (usa)"
replace nCH = 1 if gdename == "miesbach d"
replace nCH = 1 if gdename == "abu dhabi uea"
replace nCH = 1 if gdename == "markham, ontario cdn"
replace nCH = 1 if gdename == "columbia usa"
replace nCH = 1 if gdename == "oehningen (d)"
replace nCH = 1 if gdename == "st. margreth s, middlesex gb"
replace nCH = 1 if gdename == "acton usa"
replace nCH = 1 if gdename == "mannendorf d"
replace nCH = 1 if gdename == "lonzee b"
replace nCH = 1 if gdename == "tacoma usa"
replace nCH = 1 if gdename == "kettenring dayton/ohio usa"
replace nCH = 1 if gdename == "vigevano i"
replace nCH = 1 if gdename == "lennestadt d"
replace nCH = 1 if gdename == "bad schwalbach d"
replace nCH = 1 if gdename == "grays gb"
replace nCH = 1 if gdename == "pinestows, cobham surrey gb"
replace nCH = 1 if gdename == "biedenkopf/lahn d"
replace nCH = 1 if gdename == ", vaduz fl"
replace nCH = 1 if gdename == "hayes, middlesex gb"
replace nCH = 1 if gdename == "visby s"
replace nCH = 1 if gdename == "felimo s. michele i"
replace nCH = 1 if gdename == "messina (i)"
replace nCH = 1 if gdename == "oesterreich"
replace nCH = 1 if gdename == "meudon, hauts de seine f"
replace nCH = 1 if gdename == "cocquino i"
replace nCH = 1 if gdename == "mainz rfa"
replace nCH = 1 if gdename == "moensheim (d)"
replace nCH = 1 if gdename == "buchschlag offenbach d"
replace nCH = 1 if gdename == "belleville usa"
replace nCH = 1 if gdename == "creteil f"
replace nCH = 1 if gdename == "necker/oberhelfenschwil"
replace nCH = 1 if gdename == "neuilly/seine f"
replace nCH = 1 if gdename == "liebenswiller f"
replace nCH = 1 if gdename == "sernaglia della battaglia i"
replace nCH = 1 if gdename == "schlangenbad georgenborn d"
replace nCH = 1 if gdename == "villareal, castellon e"
replace nCH = 1 if gdename == "gibralta"
replace nCH = 1 if gdename == "asuncion py"
replace nCH = 1 if gdename == "kennesay(georgia.usa)"
replace nCH = 1 if gdename == "haberskirch d"
replace nCH = 1 if gdename == "wezep nl"
replace nCH = 1 if gdename == "excenevex f"
replace nCH = 1 if gdename == "detmold (d)"
replace nCH = 1 if gdename == "nauders (a)"
replace nCH = 1 if gdename == "oakside usa"
replace nCH = 1 if gdename == "st sixt, haute savoie f"
replace nCH = 1 if gdename == "portland (oregon) usa"
replace nCH = 1 if gdename == "maroubra aus"
replace nCH = 1 if gdename == "morsbach d"
replace nCH = 1 if gdename == "schorndorf schlichten d"
replace nCH = 1 if gdename == "st jean de beauregard f"
replace nCH = 1 if gdename == "l etrat f"
replace nCH = 1 if gdename == "durban, natal"
replace nCH = 1 if gdename == "tiel nl"
replace nCH = 1 if gdename == "wrightsville, pennsylvania usa"
replace nCH = 1 if gdename == "woking (gb)"
replace nCH = 1 if gdename == "livingston usa"
replace nCH = 1 if gdename == "oberkirch/baden d"
replace nCH = 1 if gdename == "volgograd rus"
replace nCH = 1 if gdename == "panama (pa)"
replace nCH = 1 if gdename == "neusaess b augsburg d"
replace nCH = 1 if gdename == "dubai emirats arabes unis"
replace nCH = 1 if gdename == "somerton, somerset gb"
replace nCH = 1 if gdename == "pleasanton usa"
replace nCH = 1 if gdename == "casellina scandicci i"
replace nCH = 1 if gdename == "rancia i"
replace nCH = 1 if gdename == "lilienthal d"
replace nCH = 1 if gdename == "seattle (washington. usa)"
replace nCH = 1 if gdename == "la celle st. cloud f"
replace nCH = 1 if gdename == "emmelrood nl"
replace nCH = 1 if gdename == "freshfield scaynes hill gb"
replace nCH = 1 if gdename == "horbourg f"
replace nCH = 1 if gdename == "laxey, isle of man gbm"
replace nCH = 1 if gdename == "weston/conn. usa"
replace nCH = 1 if gdename == "mount laurel usa"
replace nCH = 1 if gdename == "five oaks, cobham surrey gb"
replace nCH = 1 if gdename == "red bank (usa)"
replace nCH = 1 if gdename == "sofia (bulgarien)"
replace nCH = 1 if gdename == "schwechat a"
replace nCH = 1 if gdename == "blaubeuren d"
replace nCH = 1 if gdename == "san carlos usa"
replace nCH = 1 if gdename == "nykoeping s"
replace nCH = 1 if gdename == "hayward heath, w. sussex (gb)"
replace nCH = 1 if gdename == "pommard f"
replace nCH = 1 if gdename == "grandate i"
replace nCH = 1 if gdename == "passignano i"
replace nCH = 1 if gdename == "schloss hotel, stukenbrock d"
replace nCH = 1 if gdename == "neullly sur seine f"
replace nCH = 1 if gdename == "faaborg dk"
replace nCH = 1 if gdename == "messina i"
replace nCH = 1 if gdename == "aci sant antonio i"
replace nCH = 1 if gdename == "st. peter port, guernsey gb"
replace nCH = 1 if gdename == "friedrichsfehn d"
replace nCH = 1 if gdename == "grand rapids michigan usa"
replace nCH = 1 if gdename == "new dehli"
replace nCH = 1 if gdename == "plankstadt d"
replace nCH = 1 if gdename == "nordwood, new jersey usa"
replace nCH = 1 if gdename == "estoril (p)"
replace nCH = 1 if gdename == "lissabon (portugal"
replace nCH = 1 if gdename == "arlesheim (d)"
replace nCH = 1 if gdename == "hopfgarten"
replace nCH = 1 if gdename == "messtetten tieringen d"
replace nCH = 1 if gdename == "abidjan f"
replace nCH = 1 if gdename == "capri i"
replace nCH = 1 if gdename == "wavre/thielle w"
replace nCH = 1 if gdename == "horsell surrey gb"
replace nCH = 1 if gdename == "maerstetten dorf"
replace nCH = 1 if gdename == "morion pa. usa"

replace nCH = 1 if gdename == "paris (fr)"
replace nCH = 1 if gdename == "mailand (it)"
replace nCH = 1 if gdename == "new york (us)"
replace nCH = 1 if gdename == "muenchen (de)"
replace nCH = 1 if gdename == "wien (at)"
replace nCH = 1 if gdename == "hamburg (de)"
replace nCH = 1 if gdename == "rom (it)"
replace nCH = 1 if gdename == "duesseldorf (de)"
replace nCH = 1 if gdename == "bruessel (be)"
replace nCH = 1 if gdename == "stuttgart (de)"
replace nCH = 1 if gdename == "tokio (jp)"
replace nCH = 1 if gdename == "milano (it)"
replace nCH = 1 if gdename == "madrid (es)"
replace nCH = 1 if gdename == "oslo (no)"
replace nCH = 1 if gdename == "koeln (de)"
replace nCH = 1 if gdename == "stockholm (se)"
replace nCH = 1 if gdename == "turin (it)"
replace nCH = 1 if gdename == "berlin (de)"
replace nCH = 1 if gdename == "frankfurt (de)"
replace nCH = 1 if gdename == "beirut (lb)"
replace nCH = 1 if gdename == "genua (it)"
replace nCH = 1 if gdename == "los angeles (us)"
replace nCH = 1 if gdename == "bruxelles (be)"
replace nCH = 1 if gdename == "essen (de)"
replace nCH = 1 if gdename == "konstanz (de)"
replace nCH = 1 if gdename == "neuilly (fr)"
replace nCH = 1 if gdename == "wiesbaden (de)"
replace nCH = 1 if gdename == "toronto (ca)"
replace nCH = 1 if gdename == "neuilly sur seine (fr)"
replace nCH = 1 if gdename == "barcelona (es)"
replace nCH = 1 if gdename == "bremen (de)"
replace nCH = 1 if gdename == "helsinki (fi)"
replace nCH = 1 if gdename == "como (it)"
replace nCH = 1 if gdename == "montreal (ca)"
replace nCH = 1 if gdename == "marseille (fr)"
replace nCH = 1 if gdename == "kuweit (kw)"
replace nCH = 1 if gdename == "lyon (fr)"
replace nCH = 1 if gdename == "heidelberg (de)"
replace nCH = 1 if gdename == "mannheim (de)"
replace nCH = 1 if gdename == "hannover (de)"
replace nCH = 1 if gdename == "rotterdam (nl) (nl)"
replace nCH = 1 if gdename == "goeteborg (se)"
replace nCH = 1 if gdename == "frankfurt am main (de)"
replace nCH = 1 if gdename == "campione (it)"
replace nCH = 1 if gdename == "pforzheim (de)"
replace nCH = 1 if gdename == "le vesinet (fr)"
replace nCH = 1 if gdename == "dortmund (de)"
replace nCH = 1 if gdename == "ratingen (de)"
replace nCH = 1 if gdename == "annemasse (fr)"
replace nCH = 1 if gdename == "houston (us)"
replace nCH = 1 if gdename == "ulm (de)"
replace nCH = 1 if gdename == "salzburg (at)"
replace nCH = 1 if gdename == "roma (it)"
replace nCH = 1 if gdename == "krefeld (de)"
replace nCH = 1 if gdename == "nuernberg (de)"
replace nCH = 1 if gdename == "new jersey (us)"
replace nCH = 1 if gdename == "karlsruhe (de)"
replace nCH = 1 if gdename == "chicago (us)"
replace nCH = 1 if gdename == "dallas (us)"
replace nCH = 1 if gdename == "bielefeld (de)"
replace nCH = 1 if gdename == "beyrouth (lb)"
replace nCH = 1 if gdename == "linz (at)"
replace nCH = 1 if gdename == "alpen d"
replace nCH = 1 if gdename == "loerrach (de)"
replace nCH = 1 if gdename == "florenz (it)"
replace nCH = 1 if gdename == "grenoble (fr)"
replace nCH = 1 if gdename == "lund (se)"
replace nCH = 1 if gdename == "dornbirn (at)"
replace nCH = 1 if gdename == "glashuetten"
replace nCH = 1 if gdename == "reichenbach/alptal d"
replace nCH = 1 if gdename == "taeby (se)"
replace nCH = 1 if gdename == "augsburg (de)"
replace nCH = 1 if gdename == "wuerzburg"
replace nCH = 1 if gdename == "waldshut (de)"
replace nCH = 1 if gdename == "como/i"
replace nCH = 1 if gdename == "bogota (kolumbien)"
replace nCH = 1 if gdename == "darmstadt (de)"
replace nCH = 1 if gdename == "duisburg (de)"
replace nCH = 1 if gdename == "bregenz (at)"
replace nCH = 1 if gdename == "hoexter/d"
replace nCH = 1 if gdename == "bonn (de)"
replace nCH = 1 if gdename == "forno canavese"
replace nCH = 1 if gdename == "waterloo (be)"
replace nCH = 1 if gdename == "suenzhausen (d)"
replace nCH = 1 if gdename == "antwerpen (be)"
replace nCH = 1 if gdename == "meerbusch (de)"
replace nCH = 1 if gdename == "st. oswald ob eibiswald a"
replace nCH = 1 if gdename == "monza (it)"
replace nCH = 1 if gdename == "saint rhemy"
replace nCH = 1 if gdename == "baden baden (de)"
replace nCH = 1 if gdename == "wuppertal (de)"
replace nCH = 1 if gdename == "freiburg i. br."

replace nCH = 1 if gdename == "funchal (p)"
replace nCH = 1 if gdename == "arenzano (i)"
replace nCH = 1 if gdename == "taegermoosstr. 20, 870g kuesnacht"
replace nCH = 1 if gdename == "the old forge (sark, channel"
replace nCH = 1 if gdename == "oeflingen/wehr d"
replace nCH = 1 if gdename == "cheuebougeries"
replace nCH = 1 if gdename == "san salvador es"
replace nCH = 1 if gdename == "sta. gertrudis ibiza e"
replace nCH = 1 if gdename == "im hasenacker 34, schaan fl"
replace nCH = 1 if gdename == "oxted (surrey. gb)"
replace nCH = 1 if gdename == "vandwuvres"
replace nCH = 1 if gdename == "vando uvres"
replace nCH = 1 if gdename == "the old forge (sark, channel islands)"
replace nCH = 1 if gdename == "landstr. 210, ruggell fl"
replace nCH = 1 if gdename == "weesp nl"
replace nCH = 1 if gdename == "fontvieille mc"
replace nCH = 1 if gdename == "the old forge, sark ci"
replace nCH = 1 if gdename == "oberpallen l"
replace nCH = 1 if gdename == "porto cristo novo (mallorca)"
replace nCH = 1 if gdename == "porto cristo novo (e)"
replace nCH = 1 if gdename == "mombasa kenia"
replace nCH = 1 if gdename == "n. massapequa usa"
replace nCH = 1 if gdename == "roquebrune, cap martin (f)"
replace nCH = 1 if gdename == "listowel, ontario cdn"
replace nCH = 1 if gdename == "arenzano (1)"
replace nCH = 1 if gdename == "arenzano (it)"
replace nCH = 1 if gdename == "rue de geneve 21, ferney voltaire f"
replace nCH = 1 if gdename == "u, san jose (e)"
replace nCH = 1 if gdename == "san jose e"
replace nCH = 1 if gdename == "aroslo (no)"
replace nCH = 1 if gdename == "bn el"
replace nCH = 1 if gdename == "collonges soussaleve f"
replace nCH = 1 if gdename == "bringthon gb"
replace nCH = 1 if gdename == "devon/pennsylvania usa"
replace nCH = 1 if gdename == ", leymen f"
replace nCH = 1 if gdename == "san donato milanese"
replace nCH = 1 if gdename == "felsoeoers h"
replace nCH = 1 if gdename == "shrewsbury ma, usa"
replace nCH = 1 if gdename == "eivissa e"
replace nCH = 1 if gdename == "new canaan connecticut usa"
replace nCH = 1 if gdename == "koenigswiesen d"
replace nCH = 1 if gdename == "havanna c"
replace nCH = 1 if gdename == "roslyn heights n.y. usa"
replace nCH = 1 if gdename == "uebach palenberg d"
replace nCH = 1 if gdename == "u, arenzano i"
replace nCH = 1 if gdename == ", franklin lakes, new"
replace nCH = 1 if gdename == "lauenfoerde d"
replace nCH = 1 if gdename == "jersey city, new jersey usa"
replace nCH = 1 if gdename == "kirkcudbrightshire gb"
replace nCH = 1 if gdename == "cowes, isle of wight (gb)"
replace nCH = 1 if gdename == "raleigh north carolina usa"
replace nCH = 1 if gdename == "genova/i"
replace nCH = 1 if gdename == "ch. cret leyron, la"
replace nCH = 1 if gdename == "saarbruecken buebingen d"
replace nCH = 1 if gdename == "gimli cdn"
replace nCH = 1 if gdename == "the old forge, sark cl"
replace nCH = 1 if gdename == "beetsterzwaag nl"
replace nCH = 1 if gdename == "framingham usa"
replace nCH = 1 if gdename == "haestveda s"
replace nCH = 1 if gdename == "jakarta idn"
replace nCH = 1 if gdename == "villa de mazo e"
replace nCH = 1 if gdename == "ardmore, pennsylvania usa"
replace nCH = 1 if gdename == "wangs/vilters"
replace nCH = 1 if gdename == "clackshire, schottland gb"
replace nCH = 1 if gdename == "harding township, new jersey usa"
replace nCH = 1 if gdename == "grace, avenue princess 24, monaco mc"
replace nCH = 1 if gdename == "jardin exotique, bvd du 77, monaco"
replace nCH = 1 if gdename == "egelsbach (d)"
replace nCH = 1 if gdename == "berkel enschot nl"
replace nCH = 1 if gdename == "cossaye f"
replace nCH = 1 if gdename == "frankfurt a.m."
replace nCH = 1 if gdename == "santa barbara (usa)"
replace nCH = 1 if gdename == "one di fonte i"
replace nCH = 1 if gdename == "chevreuse (f)"
replace nCH = 1 if gdename == "lochau (a)"
replace nCH = 1 if gdename == "rivendell gb"
replace nCH = 1 if gdename == "weidenau d"
replace nCH = 1 if gdename == "montclair, new jersey (usa)"
replace nCH = 1 if gdename == "ploermel f"
replace nCH = 1 if gdename == "syosset ny usa"
replace nCH = 1 if gdename == "capstone (gb)"
replace nCH = 1 if gdename == "vancouver, can"
replace nCH = 1 if gdename == "spring hill aus"
replace nCH = 1 if gdename == "spa balmoral b"
replace nCH = 1 if gdename == "grand laney (ge)"
replace nCH = 1 if gdename == "camarillo usa"
replace nCH = 1 if gdename == "hauppauge usa"
replace nCH = 1 if gdename == "st imier (be)"
replace nCH = 1 if gdename == "fontvieille f"
replace nCH = 1 if gdename == "ditton, surrey gb"
replace nCH = 1 if gdename == "joplin usa"
replace nCH = 1 if gdename == "im lett 870, vaduz"
replace nCH = 1 if gdename == "rye new york usa"
replace nCH = 1 if gdename == "breidenbach wiesenbach d"
replace nCH = 1 if gdename == "seven oaks gb"
replace nCH = 1 if gdename == "menthonnex en bornes f"
replace nCH = 1 if gdename == "lizzano in belvedere i"
replace nCH = 1 if gdename == "steinegg al"
replace nCH = 1 if gdename == "elburg nl"
replace nCH = 1 if gdename == "surbridge"
replace nCH = 1 if gdename == "antigua / west indies"
replace nCH = 1 if gdename == "dubai (vab)"
replace nCH = 1 if gdename == "falkland gb"
replace nCH = 1 if gdename == "nuembrecht berkenroth d"
replace nCH = 1 if gdename == "carthage usa"
replace nCH = 1 if gdename == "great brickhill gb"
replace nCH = 1 if gdename == "putte b"
replace nCH = 1 if gdename == "ocean ridge florida usa"
replace nCH = 1 if gdename == "guernsey ci"
replace nCH = 1 if gdename == "cailloux sur fontaine f"
replace nCH = 1 if gdename == "kandy cl"
replace nCH = 1 if gdename == "tiefenbach b. bruchsal d"
replace nCH = 1 if gdename == "montperreux f"
replace nCH = 1 if gdename == "rabigh ksa"
replace nCH = 1 if gdename == "short hills (usa)"
replace nCH = 1 if gdename == "village f"
replace nCH = 1 if gdename == "hod hasharon il"
replace nCH = 1 if gdename == "ready token gb"
replace nCH = 1 if gdename == "fresno usa"
replace nCH = 1 if gdename == "claygate gb"
replace nCH = 1 if gdename == "santa ursula, teneriffa (e)"
replace nCH = 1 if gdename == "arona (i)"
replace nCH = 1 if gdename == "schmitten im taunus (d)"
replace nCH = 1 if gdename == "estepona e"
replace nCH = 1 if gdename == "unterfeld 606 b, triesen fl"
replace nCH = 1 if gdename == "eteaux f"
replace nCH = 1 if gdename == "chain (b)"
replace nCH = 1 if gdename == "chypre zy"
replace nCH = 1 if gdename == "illmuenster d"
replace nCH = 1 if gdename == "sark, channel, islands gb"
replace nCH = 1 if gdename == "palm harbor, florida usa"
replace nCH = 1 if gdename == "hohokus, new jersey usa"
replace nCH = 1 if gdename == "mombasa (eak)"
replace nCH = 1 if gdename == "ibiza (e)"
replace nCH = 1 if gdename == "kokkedal (dk)"
replace nCH = 1 if gdename == "cointrin (ge)"
replace nCH = 1 if gdename == "jacob bellecombette f"
replace nCH = 1 if gdename == "loevenich (d)"
replace nCH = 1 if gdename == "muenster (westf.d)"
replace nCH = 1 if gdename == "yardley pa usa"
replace nCH = 1 if gdename == "windsor, ontario cdn"
replace nCH = 1 if gdename == "windhoek (swa)"
replace nCH = 1 if gdename == "rue nicolas le blanc 32, bourges f"
replace nCH = 1 if gdename == "heidelberger d"
replace nCH = 1 if gdename == "st. florian bei linz a"
replace nCH = 1 if gdename == "warlingham, surry gb"
replace nCH = 1 if gdename == "st jean de tholome f"
replace nCH = 1 if gdename == "hemhofen d"
replace nCH = 1 if gdename == "torno (i)"
replace nCH = 1 if gdename == "flossmoor usa"
replace nCH = 1 if gdename == "cerrito usa"
replace nCH = 1 if gdename == "brisbane aus"
replace nCH = 1 if gdename == "somosaguas e"
replace nCH = 1 if gdename == "frimley green gb"
replace nCH = 1 if gdename == "dauchingen d"
replace nCH = 1 if gdename == "luxemburg lu"
replace nCH = 1 if gdename == "bloomfield hills, michigan usa"
replace nCH = 1 if gdename == "halton village buckinghamshire gb"
replace nCH = 1 if gdename == "airuno it"
replace nCH = 1 if gdename == "schwaig dl"
replace nCH = 1 if gdename == "mils im tirol a"
replace nCH = 1 if gdename == "north caldwell usa"
replace nCH = 1 if gdename == "cadegliano viconago i"
replace nCH = 1 if gdename == "gussendorf a"
replace nCH = 1 if gdename == "schweich d"
replace nCH = 1 if gdename == "watermael b"
replace nCH = 1 if gdename == "peron (f)"
replace nCH = 1 if gdename == "johannesburg (south"
replace nCH = 1 if gdename == "heerhrugg (sg)"
replace nCH = 1 if gdename == "jeddah (arabie saoudite)"
replace nCH = 1 if gdename == "oak park, illinois usa"
replace nCH = 1 if gdename == "rheinstetten (d)"
replace nCH = 1 if gdename == "cernay f"
replace nCH = 1 if gdename == "u, san jose e"
replace nCH = 1 if gdename == "bromma (se)"
replace nCH = 1 if gdename == "honeoye falls, new york usa"
replace nCH = 1 if gdename == "bunbury (gb)"
replace nCH = 1 if gdename == "willemstad, curacao na"
replace nCH = 1 if gdename == "knowle gb"
replace nCH = 1 if gdename == "greenwich, connocticut usa"
replace nCH = 1 if gdename == "ozzano monferrato i"
replace nCH = 1 if gdename == "lousada douro (p)"
replace nCH = 1 if gdename == "laxey (ile de man)"
replace nCH = 1 if gdename == "dunstable/bedfordshire gb"
replace nCH = 1 if gdename == "kaufbeuren"
replace nCH = 1 if gdename == "aalborg dk"
replace nCH = 1 if gdename == "horsted keynes, sussex gb"
replace nCH = 1 if gdename == "centerport usa"
replace nCH = 1 if gdename == "bad kleinkirchheim a"
replace nCH = 1 if gdename == "kingston jka"
replace nCH = 1 if gdename == "etting f"
replace nCH = 1 if gdename == "stockholm se"
replace nCH = 1 if gdename == "oxford (nz)"
replace nCH = 1 if gdename == "waltersdorf d"
replace nCH = 1 if gdename == "cannoblo i"
replace nCH = 1 if gdename == "bromley/gb"
replace nCH = 1 if gdename == "mont saint aignan f"
replace nCH = 1 if gdename == "neuilly s s f"
replace nCH = 1 if gdename == "fair lawn nj usa"
replace nCH = 1 if gdename == "pl. de milan 18, jerusalem il"
replace nCH = 1 if gdename == "trichiana i"
replace nCH = 1 if gdename == "stellenbosch (south africa)"
replace nCH = 1 if gdename == "lake park, florida usa"
replace nCH = 1 if gdename == "loano i"
replace nCH = 1 if gdename == "rome 1"
replace nCH = 1 if gdename == "bad kleinkrichheim"
replace nCH = 1 if gdename == "riverside cal. usa"
replace nCH = 1 if gdename == "fuerth stadeln d"
replace nCH = 1 if gdename == "ardning a"
replace nCH = 1 if gdename == "altea la vieja e"
replace nCH = 1 if gdename == "gaggini da"
replace nCH = 1 if gdename == "braedstrup"
replace nCH = 1 if gdename == "rom (1)"
replace nCH = 1 if gdename == "weesp (nl)"
replace nCH = 1 if gdename == "salmiyah kwt"
replace nCH = 1 if gdename == "west palm beach, florida usa"
replace nCH = 1 if gdename == "don mills ontario cdn"
replace nCH = 1 if gdename == "frankfurt a main d"
replace nCH = 1 if gdename == "buseum nl"
replace nCH = 1 if gdename == "kurasi (ghana)"
replace nCH = 1 if gdename == "hjaeltevad (schweden)"
replace nCH = 1 if gdename == "limasson cy"
replace nCH = 1 if gdename == "la jolla (california) usa"
replace nCH = 1 if gdename == "inverness gb"
replace nCH = 1 if gdename == "albertirsa h"
replace nCH = 1 if gdename == "grosse ill, michigan usa"
replace nCH = 1 if gdename == "daytona beach, florida usa"
replace nCH = 1 if gdename == "carshalton beeches"
replace nCH = 1 if gdename == "jupiter fl usa"
replace nCH = 1 if gdename == "stutensee (d)"
replace nCH = 1 if gdename == "mosman aus"
replace nCH = 1 if gdename == "menthon st bernard f"
replace nCH = 1 if gdename == "weiler bei bingen d"
replace nCH = 1 if gdename == "neuenhain (d)"
replace nCH = 1 if gdename == "u, bergen nl"
replace nCH = 1 if gdename == "anjalankoski sf"
replace nCH = 1 if gdename == "marsh baidon, oxfordshire gb"
replace nCH = 1 if gdename == "eningen unter achalm d"
replace nCH = 1 if gdename == "high wycombe uk"
replace nCH = 1 if gdename == "tiassale (elfenbeinkueste)"
replace nCH = 1 if gdename == "calahonda, mijas e"
replace nCH = 1 if gdename == "barrington hills usa"
replace nCH = 1 if gdename == "beuren d"
replace nCH = 1 if gdename == "woodbridge cdn"
replace nCH = 1 if gdename == "barrington rhode island (usa)"
replace nCH = 1 if gdename == "kayalpatman"
replace nCH = 1 if gdename == "hochheim am main d"
replace nCH = 1 if gdename == "madison, new jersey usa"
replace nCH = 1 if gdename == "setauket, new york usa"
replace nCH = 1 if gdename == "oststeinbeck d"
replace nCH = 1 if gdename == "ade f"
replace nCH = 1 if gdename == "chain (belgique)"
replace nCH = 1 if gdename == "mill valley, kalifornien usa"
replace nCH = 1 if gdename == "muehldorf eichfeld (d)"
replace nCH = 1 if gdename == "fourqueux (f)"
replace nCH = 1 if gdename == "corte madera, ca usa"
replace nCH = 1 if gdename == "hundested (dk)"
replace nCH = 1 if gdename == "nizza"
replace nCH = 1 if gdename == "solihull engl"
replace nCH = 1 if gdename == "summit nj usa"
replace nCH = 1 if gdename == "koeln/d"
replace nCH = 1 if gdename == "porto s. stefano (i)"
replace nCH = 1 if gdename == "laxey, isle de man gbm"
replace nCH = 1 if gdename == "waldburg d"
replace nCH = 1 if gdename == "asa s"
replace nCH = 1 if gdename == "stollberg d"
replace nCH = 1 if gdename == "penshurst close (gb)"
replace nCH = 1 if gdename == "stegen l"
replace nCH = 1 if gdename == "fetcham leatherhead (gb)"
replace nCH = 1 if gdename == "edinburgh schottland gb"
replace nCH = 1 if gdename == "djakarta"
replace nCH = 1 if gdename == "haus calanda, vaduz"
replace nCH = 1 if gdename == "senlis/oise f"
replace nCH = 1 if gdename == "leonding bei linz a"
replace nCH = 1 if gdename == "cannobio it"
replace nCH = 1 if gdename == "hartsdale (usa)"
replace nCH = 1 if gdename == "santanyi, mallorca e"
replace nCH = 1 if gdename == "benariffa e"
replace nCH = 1 if gdename == "raleigh, north carolina usa"
replace nCH = 1 if gdename == "parc st. roman monaco"
replace nCH = 1 if gdename == "vallauris (fr)"
replace nCH = 1 if gdename == "charlton ny usa"
replace nCH = 1 if gdename == "rapallo i"
replace nCH = 1 if gdename == "alma ata (kazakhstan)"
replace nCH = 1 if gdename == "weston, massachusetts usa"
replace nCH = 1 if gdename == "north vancouver british"
replace nCH = 1 if gdename == "aydoilles f"
replace nCH = 1 if gdename == "kronberg (de)"
replace nCH = 1 if gdename == "fishers indiana usa"
replace nCH = 1 if gdename == "wayne, pennsylvanie usa"
replace nCH = 1 if gdename == "la celle st cloud"
replace nCH = 1 if gdename == "castlelyons/lo (irland)"
replace nCH = 1 if gdename == "liedekerke b"
replace nCH = 1 if gdename == "washinomiya machi j"
replace nCH = 1 if gdename == "dolans bay au"
replace nCH = 1 if gdename == "aguilas/murcla e"
replace nCH = 1 if gdename == "olympia usa"
replace nCH = 1 if gdename == "chobham, surrey gb"
replace nCH = 1 if gdename == "lathom, ormskirk, lancashire gb"
replace nCH = 1 if gdename == "pagueira baleares"
replace nCH = 1 if gdename == "uccle bruxelles"
replace nCH = 1 if gdename == "unterfoehringn d"
replace nCH = 1 if gdename == "andraltx, mallorca e"
replace nCH = 1 if gdename == "ganderkesee d"
replace nCH = 1 if gdename == "seelze d"
replace nCH = 1 if gdename == "la hupe b"
replace nCH = 1 if gdename == "emmerich brd"
replace nCH = 1 if gdename == "los angeles ca usa"
replace nCH = 1 if gdename == "rosenfeld (d)"
replace nCH = 1 if gdename == "sakai shi j"
replace nCH = 1 if gdename == "buenos aires, argentinien"
replace nCH = 1 if gdename == "kew gb"
replace nCH = 1 if gdename == "parts f"
replace nCH = 1 if gdename == "jeddah sau"
replace nCH = 1 if gdename == "zarieb"
replace nCH = 1 if gdename == "freiburg i.br"
replace nCH = 1 if gdename == "laren nh nl"
replace nCH = 1 if gdename == "boissise le roi f"
replace nCH = 1 if gdename == "ahlen (d)"
replace nCH = 1 if gdename == "punta paítiila pa"
replace nCH = 1 if gdename == "neuilly sur selne f"
replace nCH = 1 if gdename == "laxey, isle of man"
replace nCH = 1 if gdename == "chene bougerles"
replace nCH = 1 if gdename == "stanmore (middlesex. engl.)"
replace nCH = 1 if gdename == "ohain, lasne (b)"
replace nCH = 1 if gdename == "hofheim/taunus"
replace nCH = 1 if gdename == "andraitx, mallorca e"
replace nCH = 1 if gdename == "malpas gb"
replace nCH = 1 if gdename == "gruenwald b. muenchen d"
replace nCH = 1 if gdename == "kerby cross, essex (gb)"
replace nCH = 1 if gdename == "glenburnie/kingston, ontario cdn"
replace nCH = 1 if gdename == "novare (i)"
replace nCH = 1 if gdename == "weil a.rh. d"
replace nCH = 1 if gdename == "nicosie cyp"
replace nCH = 1 if gdename == "marblehead (usa)"
replace nCH = 1 if gdename == "eisenerz a"
...
replace GdeNr_E_CNTR = 2667300 if PID == 106906 & year == 1992
replace GdeNr_E_CNTR = 2613300 if PID == 112413 & year == 1991
replace GdeNr_E_CNTR = 2630000 if PID == 158857 & year == 1989
replace GdeNr_E_CNTR = 2638300 if PID == 2436 & year == 1995
replace GdeNr_E_CNTR = 2654100 if PID == 3948 & year == 1986
replace GdeNr_E_CNTR = 2685800 if PID == 45255 & year == 1969
replace GdeNr_E_CNTR = 2675700 if PID == 56200 & year == 1992
replace GdeNr_E_CNTR = 2683100 if PID == 77810 & year == 1995
replace GdeNr_E_CNTR = 2604900 if PID == 87936 & year == 1988
replace GdeNr_E_CNTR = 2604900 if PID == 93820 & year == 1989
replace GdeNr_E_CNTR = 2611300 if PID == 9682 & year == 1969

replace GdeNr_N_CNTR = 1249100 if PID == 106906 & year == 1992
replace GdeNr_N_CNTR = 1258800 if PID == 112413 & year == 1991
replace GdeNr_N_CNTR = 1236100 if PID == 158857 & year == 1989
replace GdeNr_N_CNTR = 1237700 if PID == 2436 & year == 1995
replace GdeNr_N_CNTR = 1259400 if PID == 3948 & year == 1986
replace GdeNr_N_CNTR = 1243800 if PID == 45255 & year == 1969
replace GdeNr_N_CNTR = 1209500 if PID == 56200 & year == 1992
replace GdeNr_N_CNTR = 1247100 if PID == 77810 & year == 1995
replace GdeNr_N_CNTR = 1231000 if PID == 87936 & year == 1988
replace GdeNr_N_CNTR = 1231000 if PID == 93820 & year == 1989
replace GdeNr_N_CNTR = 1267600 if PID == 9682 & year == 1969

replace gdename = "kuenten" if PID == 106906 & year == 1992
replace gdename = "dornach" if PID == 112413 & year == 1991
replace gdename = "fulenbach" if PID == 158857 & year == 1989
replace gdename = "zofingen" if PID == 2436 & year == 1995
replace gdename = "unterboezberg" if PID == 3948 & year == 1986
replace gdename = "zollikon" if PID == 45255 & year == 1969
replace gdename = "weggis" if PID == 56200 & year == 1992
replace gdename = "zuerich" if PID == 77810 & year == 1995
replace gdename = "oberdorf" if PID == 87936 & year == 1988
replace gdename = "oberdorf" if PID == 93820 & year == 1989
replace gdename = "basel" if PID == 9682 & year == 1969

replace geo = 94 if PID == 106906 & year == 1992
replace geo = 94 if PID == 112413 & year == 1991
replace geo = 94 if PID == 158857 & year == 1989
replace geo = 94 if PID == 2436 & year == 1995
replace geo = 94 if PID == 3948 & year == 1986
replace geo = 94 if PID == 45255 & year == 1969
replace geo = 94 if PID == 56200 & year == 1992
replace geo = 94 if PID == 77810 & year == 1995
replace geo = 94 if PID == 87936 & year == 1988
replace geo = 94 if PID == 93820 & year == 1989
replace geo = 94 if PID == 9682 & year == 1969
...
keep PID year ID_dupl titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page ///
geo_merge countPLZ4 gdenr_2018 gdename GdeNr_E_CNTR GdeNr_N_CNTR nCH gde_options options4a options4b options_manual

order PID year ID_dupl gde_options options4a options4b options_manual titlejob firstname lastname address address2 alternateaddress PLZ4 city companiesid functions page ///
...
label var options4b "potential gde-matches in 4b (after split & gdename options w/o canton info)"
label var options_manual "potential gde-matches after manual coding"
label var gde_options "Indicator for multiple options in Gde (e.g. Rueti)"
label var geo_merge "Step in Geo-Merge"

sort PID year ID_dupl gdenr_2018

// add cantonal info
preserve
use "$path\02_Processed_data\10_Directors_1934_2003\Gde_GdeNr2018_Centroid_1931-2018_tmp.dta", clear
keep gdenr_2018 year ctn
duplicates drop gdenr_2018 year ctn, force
sort gdenr_2018 ctn year
save "$path\02_Processed_data\10_Directors_1934_2003\gdenr_2018-ctn.dta", replace
restore

merge m:1 gdenr_2018 year using "$path\02_Processed_data\10_Directors_1934_2003\gdenr_2018-ctn.dta"
drop if _merge == 2
drop _merge 
...
label var language_w "majority language Wohnort"
...
/*
legend for coding in geo_merge
1  : PLZ match
...
42 : no string match, split gdename, loop and merge on cell fragments (without cantonal information in gdenames): manual checks
91 : no matches on PLZ / gdename & year - manual correction of PLZ with subsequent merging on PLZ4
...
erase "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4a_notmerged.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo4b_notmerged.dta"
...
erase "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90c.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\Persons-Geo90_noPLZ.dta"
...
erase "$path\02_Processed_data\10_Directors_1934_2003\check_4b.dta"
erase "$path\02_Processed_data\10_Directors_1934_2003\gdenr_2018-ctn.dta"
...
** Import corrected unique first and last names: aggregate corrections
...
*br * if mergelast==1
replace lastname = "Jacquenoud" if lastname == "'°Jacquenoud"
replace lastname = "Bass-Hefti" if lastname == "Baß-Hefti"
replace lastname = "Hauser Schaupp" if lastname == "HauserSchaupp"
replace lastname = "Hilleker" if lastname == "Hllleke"
replace lastname = "Künzli" if lastname == "Künzii"
replace lastname = "Koelliker-Faust" if lastname == "Kœlliker-Faust"
replace lastname = "Patroncini-Hemmeler" if lastname == "Patronclnl-Hemmeler"
replace lastname = "Pauli-Gloor" if lastname == "Pauli-Gioor"
replace lastname = "Pedotti-Polar" if lastname == "Pedottl-Polar"
replace lastname = "Pedrazzini-Beeler" if lastname == "Pedrazzini-Beeier"
replace lastname = "Peereboom-Schulz" if lastname == "Peereboom-Schuiz"
replace lastname = "Peereboom-Schulz" if lastname == "Peereboom-Schuiz"
replace lastname = "Aga Khan" if lastname == "Aga"
replace firstname = "Karim" if firstname == "Khan Karim"
...
** Import manual corrections of raw-data entries
sort PID year ID_dupl gde_options

preserve
use "$path\02_Processed_data\10_Directors_1934_2003\06_Round3_Infiles\Original_check_corrections.dta", clear
sort PID year ID_dupl gde_options
save "$path\02_Processed_data\10_Directors_1934_2003\06_Round3_Infiles\Original_check_corrections.dta", replace
restore

merge m:1 PID year ID_dupl gde_options using "$path\02_Processed_data\10_Directors_1934_2003\06_Round3_Infiles\Original_check_corrections.dta", gen(mergenamefinal)
...
drop tempID newfirstname_f newlastname_f indfirst mergefirst newlastname_l newfirstname_l indlast mergelast cases lastname_ch3 firstname_ch3 finallastname finalfirstname mergenamefinal
...
*** Gender Coding ***
*use "$path\02_Processed_data\10_Directors_1934_2003\Sugarcube_Person_CleanName-Geo.dta", clear
sort firstname 

* read-in coded gender variable (for details see do-files in "...Dropbox\Projekt Nationalräte\03_Code\10_Directors_1934_2003\gender coding")
preserve
import delim using "$path\02_Processed_data\10_Directors_1934_2003\07_GenderCoding\GenderRecodedFinal.csv", delim(";") encoding("utf-8") clear
duplicates report originalname
rename originalname firstname
sort firstname
save "$path\02_Processed_data\10_Directors_1934_2003\07_GenderCoding\GenderRecodedFinal.dta", replace
restore

merge m:1 firstname using "$path\02_Processed_data\10_Directors_1934_2003\07_GenderCoding\GenderRecodedFinal.dta"
drop if _merge == 2 // 3 observations: "Khan Karim", "am BrienzerseeHauswerner", "firstname" 
drop _merge


*** Introduce Geo_Identifier ***
// It serves to establish the link with "Ground Truth Dataset" from record linkage task with NR data
// take centroid, delete decimals (for those observations that create centroid as average over several gde-centroids (in case PLZ spans various municipalities))

gen id_sug = PID 
gen year_sug= year 
replace year_sug = year_sug - 1 if year_sug <= 1964 // adjust year-ID ("year_sug") for Sandro: he worked with the origninal (erroneous) years: 1933 (instead 1934), 1942 (43), 1959 (60), 1961 (62), 1962 (63), 1963 (64)
gen e_id_sug = int(GdeNr_E_CNTR) 
gen n_id_sug = int(GdeNr_N_CNTR)
label var id_sug "PID for link w/ Ground Truth"
label var year_sug "Year for link w/ Ground Truth"
label var e_id_sug "Geo-ID East for link w/ Ground Truth"
label var n_id_sug "Geo-ID Nort for link w/ Ground Truth"

label var ID_dupl "Duplicate ID in Sugarcube original data (unique ID: PID & ID_dupl)"
label var gde_options "Geographical options if municipality unclear (e.g., Rüti)"
