******************************************************
* (A) Set directories and macros, generate variables
******************************************************
...
global margin votemargin_rel1
local margin $margin // only for file saving

global covs elected_F1 participation_F1
...
gen votemargin_rel1= votemargin/eligible_cant
gen votemargin_rel2= votemargin/voters_cant
gen votemargin_rel3= votemargin*no_seats/eligible_cant 
gen votemargin_rel4= votemargin*no_seats/voters_cant
...
label var votemargin_rel1 "Vote margin (eligibles)"
label var votemargin_rel2 "Vote margin (voters)"
label var votemargin_rel3 "Vote margin (eligibles/seats)"
label var votemargin_rel4 "Vote margin (voters/seats)"
...
rddensity $margin, kernel($kernel) plot // help rddensity: "Bias correction is only used for the construction of confidence intervals/bands, but not for point estimation. [...] Sometimes the density point estimates may lie outside of the confidence intervals/bands, which can happen if the underlying distribution exhibits high curvature at some evaluation point(s).  One possible solution in this case is to increase the polynomial order p(#) or to employ a smaller bandwidth."
rddensity $margin, h(`e(h_l)'/2 `e(h_r)'/2 ) kernel($kernel) plot

******************************************************
* (C) RD Estimation
******************************************************

* (i) Summary statistics and bandwidth definition
...
if "`z'" == "elected_F1" {
	noisily esttab p1 p1_div2 p2 p2_div2 using "$path\05_Texts_and_presns\01_Running_variable\tables\\`margin'_balance.tex", ///
...
if "`z'" == "party_svp" {
	noisily esttab p1 p1_div2 p2 p2_div2 using "$path\05_Texts_and_presns\01_Running_variable\tables\\`margin'_balance.tex", ///
	keep(Robust) b(%9.3f) se(%9.3f) coef(Robust "`label'") varwidth(21) fragment ///
	sca("h_l Bandwidth" "N_h_l Left obs" "N_h_r Right obs")  star(* 0.10 ** 0.05 *** 0.01) ///
	sfmt(%9.3f %9.0f %9.0f) noobs nogap noli nomti nonum nonotes append
	}
}
}
