...
rdrobust mandates_lead4 `1', p(1) bwselect(mserd) kernel($kernel)  ///
...
rdrobust mandates_lead4 `1', p(2) bwselect(mserd) kernel($kernel)  ///
	vce($secl ID_pers) all rho(1)
...
cap rdplot `z' `1'  ///
	if -$bw_opt_p1 <= `1' & `1' <= $bw_opt_p1 & mandates_lead4 < ., ///
...
graph export "$path\05_Texts_and_presns\03_Political_Rents\figures\\`1'_`z'_RD_Plot.pdf", ///
	as(pdf) replace
}
...
coefplot matrix(pointestimate), ci(CI) vertical nolabel yline(0) ///
	...
	graphregion(fcolor(white) lcolor(white)) legend(off)
graph export "$path\05_Texts_and_presns\03_Political_Rents\figures\\`1'_`z'_Bw_Plot.pdf", ///
	as(pdf) replace
clear matrix
...
if "`z'" == "elected_F1" {
	esttab p1 p1_div2 p2 p2_div2 using "$path\05_Texts_and_presns\03_Political_Rents\tables\\`1'_balance.tex", ///
...
esttab matrix(rrates) using "$path\05_Texts_and_presns\03_Political_Rents\tables\\`1'_balance.tex",  ///
